...
void ModelDataIteratorImpl::destroy() {
...
CORBA::Boolean ModelDataIteratorImpl::next(
    CKvalObs::CService::ModelDataList_out modelDataList) {
  ...
  if (!dbCon) {
    LOGERROR(
        "next:  No db connection (returning false)." << endl << toString(*whichData));
    return false;
  }
  ...
  if (!active) {
    LOGWARN(
        "next: deactivated ( returning false)." << endl << toString(*whichData));
    return false;
  }
  ...
  do {
    if (iData >= whichData->length()) {
      LOGDEBUG(
          "ModelDataIteratorImpl::next: End of data reached (return false)!\n" << endl << toString( *whichData ));
      return false;
    }
    ...
    try {
      ...
      LOGERROR(
          "ModelDataIteratorImpl::next: EXCEPTION: \n" << "   " << ex.what() << endl << toString( *whichData ));
      return false;
    } catch (...) {
      LOGERROR(
          "ModelDataIteratorImpl::next: UNKNOWN EXCEPTION." << endl << toString( *whichData ));
      return false;
    }
  ...
  while (it != dataList.end()) {
    if (it->obstime() != thisTime) {
      ...
      modelDataList->length(obsi + 1);
      LOGDEBUG(
          "ModelDataIteratorImpl::next: obsDataList[" << obsi-1 << "].dataList.length()=" << (*modelDataList)[obsi-1].dataList.length() << endl);
    }
  ...
  LOGDEBUG(
      "ModelDataIteratorImpl::next: obsDataList->length()=" << modelDataList->length() << endl);

  return true;
}

bool ModelDataIteratorImpl::findData(
    list<kvModelData> &data, const CKvalObs::CService::WhichData &wData) {
  kvDbGate gate(dbCon);
  boost::posix_time::ptime stime = boost::posix_time::time_from_string_nothrow(
      (const char *) wData.fromObsTime);
  boost::posix_time::ptime etime;

  if (strlen(wData.toObsTime) > 0)
    etime = boost::posix_time::time_from_string_nothrow(
        (const char *) wData.toObsTime);
  ...
  if (gate.select(data,
                  kvQueries::selectModelData(wData.stationid, stime, etime))) {
    LOGDEBUG(
        "ModelDataIteratorImpl::findData: nElements=" << data.size() << " (return true)\n");
    return true;
  }
 ...
 */
void ModelDataIteratorImpl::cleanUp() {
  boost::mutex::scoped_lock lock(mutex);
