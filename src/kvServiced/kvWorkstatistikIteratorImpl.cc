...
void WorkstatistikIteratorImpl::fillData(
    std::list<kvalobs::kvWorkelement> &toData,
...
bool WorkstatistikIteratorImpl::findData(
    std::list<kvalobs::kvWorkelement> &data) {
...
WorkstatistikIteratorImpl::WorkstatistikIteratorImpl(
    dnmi::db::Connection *dbCon_, const miutil::miTime &fromTime_,
    const miutil::miTime &toTime_, WorkstatistikTimeType timeType_,
    ServiceApp &app_)
    : dbCon(dbCon_),
      currentEnd(fromTime_),
      toTime(toTime_),
      dataIt(dataList.end()),
      app(app_) {
  switch (timeType_) {
...
void WorkstatistikIteratorImpl::destroy() {
  boost::mutex::scoped_lock lock(mutex);
  LogContext context("service/getWorkstatistikIterator");
...
CORBA::Boolean WorkstatistikIteratorImpl::next(
    CKvalObs::CService::WorkstatistikElemList_out wsList) {
  ...
  for (CORBA::Long datai = 0; it != dataList.end(); datai++, it++) {
    ...
    (*wsList)[datai].processStart =
        to_kvalobs_string(it->process_start()).c_str();
    (*wsList)[datai].qaStart = to_kvalobs_string(it->qa_start()).c_str();
    (*wsList)[datai].qaStop = to_kvalobs_string(it->qa_stop()).c_str();
    (*wsList)[datai].serviceStart =
        to_kvalobs_string(it->service_start()).c_str();
    (*wsList)[datai].serviceStop =
        to_kvalobs_string(it->service_stop()).c_str();
  }
 ...
 */
void WorkstatistikIteratorImpl::cleanUp() {
  boost::mutex::scoped_lock lock(mutex);
