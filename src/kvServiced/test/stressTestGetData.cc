...
class GetDataReceiver : public kvservice::KvGetDataReceiver {
 ...
 public:
  ...
  virtual bool next(kvservice::KvObsDataList &datalist) {
    ...
    for (KvObsDataList::iterator it = datalist.begin(); it != datalist.end();
        ++it) {
      KvObsData::kvDataList &data = it->dataList();
      KvObsData::kvTextDataList textData = it->textDataList();

      for (KvObsData::kvDataList::iterator dit = data.begin();
          dit != data.end(); ++dit) {
        ...
        nParams++;
        cerr << dit->stationID() << "," << dit->obstime() << ","
             << dit->typeID() << "," << dit->paramID() << "," << dit->original()
             << "," << dit->corrected() << ": Data" << endl;
      }

      for (KvObsData::kvTextDataList::iterator dit = textData.begin();
          dit != textData.end(); ++dit) {
        cerr << dit->stationID() << "," << dit->obstime() << ","
             << dit->typeID() << "," << dit->paramID() << "," << dit->original()
             << ": TextData" << endl;
      }
 ...
 public:
  ...
  void operator()() {
    ...
    whichData.addStation(
        0, boost::posix_time::time_from_string("2010-02-22 06:00:00"),
        boost::posix_time::time_from_string("2010-02-22 06:00:00"));
    ...
    for (int N = 1; N > 0; N--) {
      ...
      if (!app->getKvData(dataReceiver, whichData)) {
        ...
        } else if (nObstimes != dataReceiver.nObstimes
            || nParams != dataReceiver.nParams) {
          results->nUnexpected++;
        }

        cerr << results->tid << " (" << N << ")" << " nObstimes: "
             << dataReceiver.nObstimes << " nParams: " << dataReceiver.nParams
             << endl;
      }
    }
  }
...
int main(int argn, char **argv) {
  ...
  for (std::list<GetThread*>::iterator it = threadList.begin();
      it != threadList.end(); ++it) {
    cerr << (*it)->results->tid << ": runs: " << (*it)->results->nRuns
         << " Failed: " << (*it)->results->nFailed << " Unexpected: "
         << (*it)->results->nUnexpected << endl;
  }
