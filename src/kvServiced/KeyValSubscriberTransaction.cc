...
KeyValSubscriberTransaction::insert( const std::string &val )
{
    ostringstream q;
    q << "INSERT INTO key_val VALUES("
...
bool
KeyValSubscriberTransaction::
update( const std::string &val )
{
    ostringstream q;
    q << "UPDATE key_val"
      << " SET val='" << val << "'"
      << " WHERE package=" << SubScriberID << " AND key='" << subscriberid << "'";
    err << "query: <" << q.str() << ">. ";
    con->exec( q.str() ); //Throws on error
    return true;
}
...
bool
KeyValSubscriberTransaction::
get( std::list<kvalobs::kvKeyVal> &vals, const std::string &subid )
{
    vals.clear();
    ostringstream q;
    q << "SELECT * FROM key_val"
      << " WHERE package=" << SubScriberID;
...
: action( UPDATE_LAST_CALL ),
  ...
  subscriberid( subid ),
  lastCall( lastCall_ ), con( 0 ), ok( false )
{
}

KeyValSubscriberTransaction::
KeyValSubscriberTransaction( const std::string &subid, const std::string &content_ )
: action( INSERT_OR_UPDATE ), SubScriberID("'KvServiceSubcriberIDS'"),
  subscriberid( subid ),
  content(content_), con( 0 ), ok( false )
{
}

KeyValSubscriberTransaction::
KeyValSubscriberTransaction( const std::string &subid, Action act )
: action( act ), SubScriberID("'KvServiceSubcriberIDS'"),
  subscriberid( subid ), con( 0 ), ok( false )
{
}
...
operator()(dnmi::db::Connection *conection)
{
    ...
    con = conection;
    switch( action ) {
        case INSERT_OR_UPDATE: err << "<INSERT_OR_UPDATE> ";
            ok = insertOrUpdate(); break;
        case UPDATE_LAST_CALL: err << "<UPDATE_LAST_CALL> ";
            ok = updateLastCall(); break;
        case DELETE_SUBSCRIBER: err << "<DELETE_SUBSCRIBER> ";
            ok = removeSubscriber(); break;
        case GET_SUBSCRIBER: err << "<GET_SUBSCRIBER> ";
            ok = get( keyVals, subscriberid ); break;
    }
    return true;
}
