...
bool isHourDataCheck(const list<kvData> &dataList) {
...
DataIteratorImpl::DataIteratorImpl(dnmi::db::Connection *dbCon_,
                                   WhichDataList *whichData_, ServiceApp &app_)
    : dbCon(dbCon_),
...
CORBA::Boolean DataIteratorImpl::next(
    CKvalObs::CService::ObsDataList_out obsDataList) {
  ...
  while (it != dataList.end()) {
    if (it->obstime() != thisTime) {
      ...
      LOGDEBUG(
          "next: obsDataList[" << obsi-1 << "].dataList.length()=" << (*obsDataList)[obsi-1].dataList.length() << endl);
    }
    ...
    (*obsDataList)[obsi].dataList[datai].obstime =
    ...
    (*obsDataList)[obsi].dataList[datai].tbtime =
    ...
    (*obsDataList)[obsi].dataList[datai].controlinfo = it->controlinfo()
        .flagstring().c_str();
    (*obsDataList)[obsi].dataList[datai].useinfo = it->useinfo().flagstring()
        .c_str();
  ...
  LOGDEBUG("next: obsDataList->length()=" << obsDataList->length() << endl);
  ...
  while (tit != textDataList.end()) {
    ...
    textData[datai].obstime = to_kvalobs_string_without_decimal_secound(
        tit->obstime()).c_str();
    textData[datai].original = tit->original().c_str();
    textData[datai].paramID = tit->paramID();
    textData[datai].tbtime = to_kvalobs_string_without_decimal_secound(
        tit->tbtime()).c_str();
...
list<kvData>::iterator DataIteratorImpl::filterData(
    list<kvData>::iterator start, list<kvData> &dataList,
 ...
 */
bool DataIteratorImpl::findData(list<kvData> &data,
  ...
  LOGDEBUG(
      "stationid: " << wData.stationid << " currentEndTime: " << currentEndTime << " endTime: " << endTime << " iData: " << iData << " isHourData: " << isHourData);

  if (currentEndTime.is_not_a_date_time()) {
    stime = boost::posix_time::time_from_string_nothrow(
        std::string(wData.fromObsTime));

    if (strlen(wData.toObsTime) > 0)
      endTime = boost::posix_time::time_from_string_nothrow(
          std::string(wData.toObsTime));
  ...
  LOGDEBUG(
      "select(" << wData.stationid << ", " << stime << ", " << etime << "): isHourData: " << isHourData);
  ...
  if (gate.select(data, kvQueries::selectData(wData.stationid, stime, etime))) {
    ...
    LOGERROR(
        "Error fetching <data>! stationid: " << wData.stationid << " timeinterval:" << stime << " - " << etime);
    ret = false;
  }
  ...
  if (gate.select(textData,
                  kvQueries::selectTextData(wData.stationid, stime, etime))) {
    LOGDEBUG("textData: nElements=" << textData.size());
  } else {
    LOGWARN(
        "Error fetching <textData>! stationid: " << wData.stationid << " timeinterval:" << stime << " - " << etime);
  }
...
void DataIteratorImpl::insertTextData(
    CKvalObs::CService::ObsDataList *obsDataList,
  ...
  textTime = boost::posix_time::time_from_string_nothrow(
      std::string(textData[0].obstime));
  ...
  for (i = 0; i < obsDataList->length(); i++) {
    ...
    obsTime = boost::posix_time::time_from_string_nothrow(
        std::string((*obsDataList)[i].dataList[0].obstime));
