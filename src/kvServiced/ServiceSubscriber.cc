...
string buildDataQuery(const kvalobs::kvStationInfoExt &st) {
  ostringstream q;
  list<kvalobs::kvStationInfoExt::Param> params = st.params();

  q << " WHERE stationid=" << st.stationID() << " AND typeid=" << st.typeID()
...
string buildTextDataQuery(const kvalobs::kvStationInfoExt &st) {
  ostringstream q;
  list<kvalobs::kvStationInfoExt::Param> params = st.params();
...
ServiceSubscriber::ServiceSubscriber(ServiceApp &app_,
...
ServiceSubscriber::ServiceSubscriber(const ServiceSubscriber &s)
    : app(s.app),
      inputque(s.inputque),
      dbCon(s.dbCon) {
}
...
void ServiceSubscriber::updateWorkelementServiceStart(
    const kvalobs::kvStationInfoExt &st, dnmi::db::Connection *con,
    const std::string &logid) {
  kvDbGate gate(con);
  ostringstream ost;

  ost << "UPDATE workque SET service_start='" << miTime::nowTime()
      << "' WHERE stationid=" << st.stationID() << "  AND obstime='"
      << to_kvalobs_string(st.obstime()) << "' AND typeid=" << st.typeID();

  if (!gate.exec(ost.str())) {
    LOGERROR(
        "DBERROR: Cant update workque!" << endl << "Reason: " << gate.getErrorStr());
...
void ServiceSubscriber::updateWorkelementServiceStop(
    const kvalobs::kvStationInfoExt &st, dnmi::db::Connection *con,
    const std::string &logid) {
  kvDbGate gate(con);
  ostringstream ost;
  ...
  ost << "UPDATE workque SET service_stop='" << miTime::nowTime()
      << "' WHERE stationid=" << st.stationID() << "  AND obstime='"
      << to_kvalobs_string(st.obstime()) << "' AND typeid=" << st.typeID();
...
void ServiceSubscriber::callDataNotifySubscribers(
    const kvalobs::kvStationInfoExt &si, const std::string &logid) {
  ...
  if (!gate.select(
      dataList,
      kvQueries::selectDataFromType(si.stationID(), si.typeID(),
                                    si.obstime()))) {
    if (logid.empty()) {
      LOGWARN(
          "NODATA (notifysub) source <?>: stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    } else {
      LOGWARN(
          "NODATA (notifysub) source <" << logid << ">: stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    }

    if (!logid.empty()) {
      IDLOGWARN(
          logid,
          "NODATA (notifysub): stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    }
    return;
  }
  ...
  LOGDEBUG(
      "CALL DataNotifySubscribers: stationID(" << si.stationID() << ")\n" << si);
...
void ServiceSubscriber::callDataSubscribers(const kvalobs::kvStationInfoExt &si,
                                            const std::string &logid) {
  long stationID;
  ...
  if (dataList.empty() && textDataList.empty()) {
    if (logid.empty()) {
      LOGWARN(
          "NODATA (datasub) source <?>: stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    } else {
      LOGWARN(
          "NODATA (datasub) source <" << logid << ">: stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    }

    if (!logid.empty()) {
      IDLOGWARN(
          logid,
          "NODATA (datasub): stationid: " << si.stationID() << " typeid: " << si.typeID() << " obstime: " << si.obstime());
    }
  ...
  LOGDEBUG(
      "CALL DataSubscribers: stationID: "<< si.stationID() << " obstime: " << si.obstime() << " typeID: " << si.typeID());
  if (!logid.empty()) {
    IDLOGDEBUG(
        logid,
        "CALL DataSubscribers: stationID: "<< si.stationID() << " obstime: " << si.obstime() << " typeID: " << si.typeID());
...
void ServiceSubscriber::removeDeadConnections() {
  app.subscribers.removeDeadSubscribers(60);
}

void ServiceSubscriber::operator()() {
  ...
  while (!app.shutdown()) {
    ...
    if (!logName.empty()) {
      IDLOGDEBUG(logName,
                 "Number of datasets: " << stInfoCmd->getStationInfo().size());
    }
    ...
    for (; it != stInfoCmd->getStationInfoExt().end(); it++) {
      if (!logName.empty()) {
        IDLOGDEBUG(
            logName,
            "Observation: stationid: " << it->stationID() << " typeid: " << it->typeID() << " obstime: " << it->obstime());
      }
    ...
    if (fromKvManager)
      app.sendToManager(stInfoCmd->getStationInfo(), stInfoCmd->getCallback());
...
void DataNotifyFunc::func(KvDataNotifySubscriberPtr ptr) {
  ...
  try {
    ...
    LOGERROR(
        "EXCEPTION: (timeout?) Can't send <DataNotify> event to subscriber!" << endl << "Subscriberid: " << ptr->subscriberid() << ">!");
  } catch (...) {
    ptr->connection(false);
    LOGERROR(
        "EXCEPTION: Can't send <DataNotify> event to subscriber!" << endl << "Subscriberid: " << ptr->subscriberid() << ">!");
  }
}

bool DataNotifyFunc::buildWhatList(
  ...
  for (list<kvData>::const_iterator it = dataList.begin(); it != dataList.end();
      it++) {
...
bool DataNotifyFunc::fqcLevel(CKvalObs::CService::QcId qcId,
                              unsigned char flag) {
  switch (qcId) {
...
bool DataNotifyFunc::checkStatusAndQc(KvDataNotifySubscriberPtr ptr) {
  unsigned char flag;
...
bool DataFunc::fqcLevel(CKvalObs::CService::QcId qcId, unsigned char flag) {
  switch (qcId) {
    case CKvalObs::CService::QC1:
      if (flag & QC1_mask)
        return true;
...
bool DataFunc::checkStatusAndQc(KvDataSubscriberPtr ptr) {
  CORBA::Long it;
  CORBA::Long i;
  unsigned char flag;
...
DataFunc::DataFunc(const DataToSendList &dataList) {
  ...
  for (CIDataToSendList itd = dataList.begin(); itd != dataList.end(); itd++) {
    hasData = false;
    ...
    if (itd->dataList.size() > 0) {
      ...
      for (list<kvalobs::kvData>::const_iterator it = itd->dataList.begin();
          it != itd->dataList.end(); it++) {
        /* DEBUG
         LOGDEBUG("DS: " << *it );
         */
        data[obsi].dataList[datai].stationID = it->stationID();
        data[obsi].dataList[datai].obstime =
        ...
        data[obsi].dataList[datai].tbtime =
            to_kvalobs_string_without_decimal_secound(it->tbtime()).c_str();
        ...
        data[obsi].dataList[datai].controlinfo = it->controlinfo().flagstring()
            .c_str();
    ...
    if (itd->textDataList.size() > 0) {
      ...
      for (list<kvalobs::kvTextData>::const_iterator it = itd->textDataList
          .begin(); it != itd->textDataList.end(); it++) {
        data[obsi].textDataList[datai].stationID = it->stationID();
        data[obsi].textDataList[datai].obstime =
            to_kvalobs_string_without_decimal_secound(it->obstime()).c_str();
        data[obsi].textDataList[datai].original = it->original().c_str();
        data[obsi].textDataList[datai].paramID = it->paramID();
        data[obsi].textDataList[datai].tbtime =
            to_kvalobs_string_without_decimal_secound(it->tbtime()).c_str();
...
void DataFunc::func(KvDataSubscriberPtr ptr) {
  using namespace CKvalObs::CService;
  ...
  try {
    ...
    ptr->connection(false, true);
    LOGERROR(
        "EXCEPTION: (timeout?) Can't send <Data> event to subscriber!" << endl << "Subscriberid: " << ptr->subscriberid() << ">!");
  } catch (...) {
    ptr->connection(false);
    LOGERROR(
        "EXCEPTION: Can't send <Data> event to subscriber!" << endl << "Subscriberid: " << ptr->subscriberid() << ">!");
  }
}
