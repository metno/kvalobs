...
SubscriberThread::SubscriberThread(const SubscriberThread &s)
    : app(s.app),
      inputquePtr(s.inputquePtr),
      join_(s.join_) {
}

SubscriberThread::SubscriberThread(ServiceApp &app_,
...
bool SubscriberThread::join() {
  return *join_;
}

void SubscriberThread::operator()() {
}
