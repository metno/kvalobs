/*
  ...
  $Id: kvDataSubscriberInfo.h,v 1.1.2.7 2007/09/27 09:02:40 paule Exp $                                                       
...
class KvDataSubscriberInfo : public KvSubscriberBase 
{
 ...
 public:
  ...
  enum {RETRY=60, MAX_RETRY=3600} RetryConst;
...
protected:
  ...
  //This is used for retry and timeout controll comuniction 
  //with susbscribers.
  time_t                    retry;  
  time_t                    maxRetry;
  time_t                    tick;
...
public:
  ...
  bool        qcAll()const{ return qc_.size()==0;}
  bool        hasQc(CKvalObs::CService::QcId qc)const; 
  TQc         qc()const{ return qc_;}
  ITQc        qcbegin(){ return qc_.begin();}
  CITQc       qcbegin()const { return qc_.begin();}
  ITQc        qcend(){ return qc_.end();}
  CITQc       qcend()const { return qc_.end();}

  bool        thisStation(int stationid)const;

  ITStations  stbegin() { return stations_.begin();}                
  CITStations stbegin()const { return stations_.begin();}                
  ITStations  stend() { return stations_.end();}                
  CITStations stend()const { return stations_.end();}                
