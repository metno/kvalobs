/*
  ...
  $Id: SubscriberData.cc,v 1.1.2.6 2007/09/27 09:02:39 paule Exp $                                                       
...
createSubscriberid(TSubType st)
{
  ...
  switch(st){
  case DataSub:       servicename="datanotify"; break;
  case DataNotifySub: servicename="data";       break;
...
bool 
SubscriberData::
readSubscriberFile(const std::string &fname)
{
  ...
  //CODE:
  //Remeber to create the subscriber threads.
...
createDataSubscriber(KvDataSubscriber *subscriber)
{
  ...
  string id=createSubscriberid(DataSub);
...
createDataNotifySubscriber(KvDataNotifySubscriber *subscriber)
{
  ...
  string id=createSubscriberid(DataNotifySub);
...
createHintSubscriber(CKvalObs::CService::kvHintSubscriber_ptr s)
{
  ...
  mutex::scoped_lock lock(mutex);
  string id=createSubscriberid(HintSub);
