...
bool ServiceApp::createGlobalLogger(std::string &id, milog::LogLevel ll) {
  try {
...
kvalobs::kvStation ServiceApp::lookUpStation(const std::string &kvQuerie) {
...
void ServiceApp::notifyAllKvHintSubscribers() {
...
bool ServiceApp::sendToManager(kvalobs::kvStationInfoList &retList,
  ...
  for (k = 0; it != retList.end(); it++, k++) {
    ...
    stInfo[k].obstime = CORBA::string_dup(
...
bool ServiceApp::isMaxClientReached() {
  ...
  LOGDEBUG(
      "isMaxClientReached: # : " << reaperObjList.size() << " max: " << MAX_CLIENTS);
...
void ServiceApp::removeReaperObj(ReaperBase *rb) {
  ...
  LOGDEBUG(
      "removeReaperObj: No ReaperObj found in the reaperList! (This must be an error!)");
}
...
void ServiceApp::cleanUpReaperObj() {
  ...
  while (it != reaperObjList.end()) {
    ...
    LOGDEBUG(
        "cleanUpReaperObj: active: " << ((*it)->isActive()?"true":"false") << " running: " << (running?"true":"false") << " lastAccess: " << miutil::miTime(lastAccess) << " timedif: " << now -lastAccess);
    ...
    if (!(*it)->isActive()) {
      ...
      LOGDEBUG(
          "COLLECT before # : " << n << " after # : " << reaperObjList.size() << " max: " << MAX_CLIENTS);
      obj->cleanUp();
      obj->removeRef();
      ...
      if ((now - lastAccess) >= (*it)->getTimeToLive()) {
        LOGWARN(
            "TIMEOUT, a ReaperBase object is scheduled for garbage collection!");
