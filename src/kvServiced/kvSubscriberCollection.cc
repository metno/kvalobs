...
KvSubscriberCollection::KvSubscriberCollection(ServiceApp &app_)
    : app(app_),
      isInitialized(false) {
}
...
dnmi::db::Connection*
KvSubscriberCollection::getDbConnection() {
...
void KvSubscriberCollection::releaseThisThreadsDbConnection() {
  DbConnectionHolder *dbHolder = dbConectionHolder.get();
...
void KvSubscriberCollection::updateSubscriberInDb(const std::string &subid,
                                                  const std::string &content) {
  dnmi::db::Connection *con = getDbConnection();

  if (!con) {
    LOGERROR(
        "KvSubscriberCollection: No db connection. Cant read subscribers from the database.");
    return;
  }
  ...
  try {
    ...
    if (!tran.isOk()) {
      LOGERROR(
          "KvSubscriberCollection: Insert/Update subscriber <" << subid << "> FAILED." << endl << "Reason: " << tran.getMsg());
      return;
    } else {
      LOGDEBUG(
          "KvSubscriberCollection: Inserted/Updated subscriber <" << subid << ">." << endl << "Msg: " << tran.getMsg());
    }
  } catch (const std::exception &ex) {
    LOGERROR(
        "KvSubscriberCollection: Insert/Update subscriber <" << subid << "> FAILED." << endl << "Reason: (exception)" << ex.what() << endl << tran.getMsg());
    return;
  }
}

bool KvSubscriberCollection::createSubscriberid(
    KvSubscriberBasePtr p, const std::string &servicename) {
  int COUNT_MAX = 1000;
  int count = 0;
...
void KvSubscriberCollection::removeSubscriberid(KvSubscriberBasePtr p) {
  std::set<KvSubscriberBasePtr, KvSubscriberBasePtrOps>::iterator it;
  ...
  if (it != subscribers_.end()) {
    ...
    LOGDEBUG(
        "KvSubscriberCollection::removeSubscriberid: " << p->subscriberid() << endl);
  ...
  LOGDEBUG(
      "KvSubscriberCollection::removeSubscriberid: " << p->subscriberid() << " is not among the subscribers!");
}

void KvSubscriberCollection::removeSubscriberid(
  ...
  for (it = subscribers_.begin(); it != subscribers_.end(); it++) {
    ...
    if ((*it)->subscriberid() == subscriberid) {
      LOGDEBUG(
          "KvSubscriberCollection::removeSubscriberid: " << (*it)->subscriberid() << endl);
      subscribers_.erase(it);
  ...
  LOGDEBUG(
      "KvSubscriberCollection::removeSubscriberid: " << subscriberid << " is not among the subscribers. (FATAL?)");

}

bool KvSubscriberCollection::addDataNotifySubscriber(
...
bool KvSubscriberCollection::addDataNotifySubscriber(
    KvDataNotifySubscriberPtr p, long stationid) {
  //This method is called from kvServiceImpl only.
  //We release the database connection before
  //we return.
  boost::mutex::scoped_lock lock(mutex);
...
void KvSubscriberCollection::removeDataNotifySubscriber(
    const std::string &subscriberid) {
...
void KvSubscriberCollection::removeHintSubscriber(
    const std::string &subscriberid) {
...
void KvSubscriberCollection::removeDataSubscriber(
    const std::string &subscriberid) {
...
void KvSubscriberCollection::doRemoveSubscriber(
    const std::string &subscriberid) {
...
void KvSubscriberCollection::removeSubscriber(const std::string &subscriberid) {
  boost::mutex::scoped_lock lock(mutex);
  readAllSubscribersFromDb();
 ...
 */
void KvSubscriberCollection::removeDeadSubscribers(int durationInSeconds) {
  ...
  for (std::list<KvDataNotifySubscriberPtr>::iterator it =
      allStationsDataNotifySubscribers.begin();
      it != allStationsDataNotifySubscribers.end(); it++) {
    if ((*it)->removeThisSubscriber(durationInSeconds)) {
      subList.push_back((*it)->subscriberid());
    }
  }

  for (std::multimap<long, KvDataNotifySubscriberPtr>::iterator it =
      stationDataNotifySubscribers.begin();
      it != stationDataNotifySubscribers.end(); it++) {
    if (it->second->removeThisSubscriber(durationInSeconds)) {
      subList.push_back(it->second->subscriberid());
    }
  }

  for (std::list<KvDataSubscriberPtr>::iterator it = allStationsDataSubscribers
      .begin(); it != allStationsDataSubscribers.end(); it++) {
    if ((*it)->removeThisSubscriber(durationInSeconds)) {
      subList.push_back((*it)->subscriberid());
    }
  }

  for (std::multimap<long, KvDataSubscriberPtr>::iterator it =
      stationDataSubscribers.begin(); it != stationDataSubscribers.end();
      it++) {
    if (it->second->removeThisSubscriber(durationInSeconds)) {
      subList.push_back(it->second->subscriberid());
    }
...
void KvSubscriberCollection::forAllDataNotifySubscribers(
    DataNotifySubscriberFuncBase &obj, long stationid) {
  list<KvDataNotifySubscriberPtr>::iterator it;
...
bool KvSubscriberCollection::addDataSubscriber(KvDataSubscriberPtr p) {
  //This method is called from kvServiceImpl only.
  //We release the database connection before
  //we return.
  boost::mutex::scoped_lock lock(mutex);
...
bool KvSubscriberCollection::addKvHintSubscriber(KvHintSubscriberPtr sub) {
  //This method is called from kvServiceImpl only.
  //We release the database connection before
  //we return.
...
void KvSubscriberCollection::sendKvHintUp() {
  using namespace CKvalObs::CService;
  ...
  while (it != hintSubscriberList.end()) {
    try {
      ...
      it++;
      LOGINFO(
          "SUBSCRIBER NOT LISTNING: Deletes the subscriber <" << (*itTmp)->subscriberid() << "> from the list of\n" << "kvHint subscribers.");
      doRemoveSubscriber((*itTmp)->subscriberid());
    }
  }
}

void KvSubscriberCollection::sendKvHintDown() {
  using namespace CKvalObs::CService;
  kvHintSubscriber_var sub;
  std::list<KvHintSubscriberPtr>::iterator it;
  boost::mutex::scoped_lock lock(mutex);
  ...
  for (; it != hintSubscriberList.end(); it++) {
    try {
      ...
      LOGINFO(
          "SUBSCRIBER NOT LISTNING: Deletes the subscriber <" << (*it)->subscriberid() << "> from the list of\n" << "kvHint subscribers.");
...
void KvSubscriberCollection::forAllDataSubscribers(DataSubscriberFuncBase &obj,
                                                   long stationid) {
  list<KvDataSubscriberPtr>::iterator it;
...
bool KvSubscriberCollection::hasDataSubscribers() {
  boost::mutex::scoped_lock lock(mutex);
  readAllSubscribersFromDb();
  if (allStationsDataSubscribers.empty() && stationDataSubscribers.empty())
    return false;

  return true;
}

bool KvSubscriberCollection::hasDataNotifySubscribers() {
  boost::mutex::scoped_lock lock(mutex);
  readAllSubscribersFromDb();
...
std::ostringstream*
KvSubscriberCollection::writeHeader(const std::string &subscriberid,
  ...
  (*fs) << "Last call: " << tNow.isoTime() << endl;

  (*fs) << "Created: " << tNow.isoTime() << endl << "Subid: " << subscriberid
      << endl << "CORBA ref: " << corbaref << endl << "StatusId: ";
...
void KvSubscriberCollection::readAllSubscribersFromDb() {
  if (!isInitialized) {
    dnmi::db::Connection *con = getDbConnection();
    ...
    //Get all subscribers stored in the KeyVal table in the database.
    KeyValSubscriberTransaction tran(
        "", KeyValSubscriberTransaction::GET_SUBSCRIBER);

    try {
      con->perform(tran);
      if (!tran.isOk()) {
        LOGERROR(
            "KvSubscriberCollection: Read all from subscribers from the database FAILED." << endl << "Reason: " << tran.getMsg());
        return;
      } else {
        LOGDEBUG(
            "KvSubscriberCollection: Read all from subscribers from the database OK." << endl << "Msg: " << tran.getMsg());
      }
    } catch (const std::exception &ex) {
      LOGERROR(
          "KvSubscriberCollection: Read all from subscribers from the database FAILED." << endl << "Reason: (exception)" << ex.what() << endl << tran.getMsg());
      return;
    }
...
bool KvSubscriberCollection::readSubscriberFromDb(
    const std::string &subscriberid, const std::string &content) {
  ...
  while (!fs.eof()) {
    ...
    if (i == string::npos) {
      LOGERROR("FORMAT ERROR: readSubscriber <" << subscriberid << "> failed!");
      return false;
    }
    ...
    if (key.find("Last call") != string::npos) {
      ...
      hasQcIdKey = true;
      boost::split(qcIdList, val, boost::algorithm::is_space(),
      ...
      if (val.empty()) {
        ...
        allStations = false;
        boost::split(stationList, val, boost::algorithm::is_space(),
                     boost::algorithm::token_compress_on);
      }
    } else {
      LOGWARN(
          "Uknown key <" << key << ">: readSubscriber <" << subscriberid << ">!");
    }
  ...
  if (subid.empty() || cref.empty()) {
    LOGERROR(
        "Format error: readSubscriber <" << subscriberid << "> " << "missing key <Subid> or <CORBA ref>!");
    return false;
  }
  ...
  if (i == string::npos) {
    LOGERROR(
        "Ivalid subscriberid [missing subscriber type]: " << subid << ">,  readSubscriber <" << subscriberid << ">!");
    return false;
  }
  ...
  if (subtype == "datanotify") {
    if (!hasStationKey) {
      LOGERROR(
          "Missing key <Stations> for datanotify subscriber: " << "readSubscriber <" << subid << ">!");
      return false;
    }

    res = addDataNotifyFromDb(subid, cref, statusid, qcIdList, stationList);

  } else if (subtype == "data") {
    if (!hasStationKey) {
      LOGERROR(
          "Missing key <Stations> for data subscriber: " << "readSubscriber <" << subid << ">!");
      return false;
    }

    res = addDataFromDb(subid, cref, statusid, qcIdList, stationList);
  ...
  if (res) {
    LOGINFO("Added subscriber <" << subtype << ">  id <" << subid << ">!");
    return true;
  } else {
    LOGERROR(
        "Could not add subscriber <" << subtype << "> id <" << subid << ">!");
    return false;
  }
}

bool KvSubscriberCollection::writeSubscriberFromDb(const std::string &subid) {
  ...
  if (subtype == "datanotify") {
    ...
    LOGERROR(
        "Unknown subscriber type: " << subtype << " [subscriberid: " << subid << "]");
    return false;
  }
...
bool KvSubscriberCollection::writeDataNotifyToDb(const std::string &subid) {
...
bool KvSubscriberCollection::writeDataToDb(const std::string &subid) {
  CorbaHelper::CorbaApp *cApp = CorbaHelper::CorbaApp::getCorbaApp();
  ...
  for (itAllStations = allStationsDataSubscribers.begin();
      itAllStations != allStationsDataSubscribers.end(); itAllStations++) {
  ...
  for (it = stationDataSubscribers.begin(); it != stationDataSubscribers.end();
      it++) {
...
bool KvSubscriberCollection::writeKvHintToDb(const std::string &subid) {
  CorbaHelper::CorbaApp *cApp = CorbaHelper::CorbaApp::getCorbaApp();
  ...
  for (it = hintSubscriberList.begin(); it != hintSubscriberList.end(); it++) {
...
bool KvSubscriberCollection::addDataNotifyFromDb(
    const std::string &subid, const std::string &cref,
  ...
  try {
    ...
    CORBA::release(cptrSub);
    LOGERROR(
        "OUT OF MEMMORY: KvSubscriberCollection::addDataNotifyFromFileFile!");
    return false;
  }
  ...
  for (it = stationList.begin(); it != stationList.end(); it++) {
    ...
    try {
      ...
      LOGERROR(
          "UNKNOWN Exception: trying stationid=boost::numeric_cast<long>(*it)!");
      continue;
    }
...
kvalobs::KvDataSubscriberInfo KvSubscriberCollection::createKvDataInfo(
    const std::string &statusid, const std::vector<std::string> &qcIdList) {
  ...
  for (CORBA::Long i = 0; it != qcIdList.end(); it++, i++) {
...
bool KvSubscriberCollection::addDataFromDb(
    const std::string &subid, const std::string &cref,
    const std::string &statusid, const std::vector<std::string> &qcIdList,
    const std::vector<std::string> &stationList) {
  CorbaHelper::CorbaApp *cApp = CorbaHelper::CorbaApp::getCorbaApp();
...
bool KvSubscriberCollection::addKvHintFromDb(const std::string &subid,
...
bool KvSubscriberCollection::removeSubscriberFromDb(
    const std::string &subscriberid) {
  dnmi::db::Connection *con = getDbConnection();
  ...
  KeyValSubscriberTransaction tran(
      subscriberid, KeyValSubscriberTransaction::DELETE_SUBSCRIBER);

  try {
    con->perform(tran);
    if (!tran.isOk()) {
      LOGERROR(
          "KvSubscriberCollection: can't remove <" << subscriberid << "> from the database." << endl << "Reason: " << tran.getMsg());
      return false;
    } else {
      LOGDEBUG(
          "KvSubscriberCollection: Removed <" << subscriberid << "> from the database." << endl << "Msg: " << tran.getMsg());
    }
    return true;
  } catch (const std::exception &e) {
    LOGERROR(
        "KvSubscriberCollection: can't remove <" << subscriberid << "> from the database." << endl << "Reason: (exception): " << e.what() << endl << "Msg: " << tran.getMsg());
  }
  return true;
}

std::ostream& operator<<(std::ostream& os, const KvSubscriberCollection &c) {
  boost::mutex::scoped_lock lock(const_cast<KvSubscriberCollection&>(c).mutex);
  os << "\nKvSubscriberCollection: \n";
  os << "  allStationDataNotifySubscribers: Antall: "
     << c.allStationsDataNotifySubscribers.size() << endl;
  ...
  os << "\n  stationsDataNotifySubscribers: Antall: "
     << c.stationDataNotifySubscribers.size() << endl;
