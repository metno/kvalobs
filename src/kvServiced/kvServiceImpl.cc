...
KvServiceImpl::KvServiceImpl(ServiceApp &app_)
    : app(app_) {
}
...
void KvServiceImpl::addToObsPgmList(CKvalObs::CService::Obs_pgmList &pgmList,
  ...
  for (; it != obsPgms.end(); it++) {
    ...
    if (it->fromtime().is_not_a_date_time())
      ...
      pgm.fromtime = to_kvalobs_string_without_decimal_secound(it->fromtime())
          .c_str();
    ...
    if (it->totime().is_not_a_date_time())
      ...
      pgm.totime =
          to_kvalobs_string_without_decimal_secound(it->totime()).c_str();
...
bool KvServiceImpl::appendWhichDataFromStationRange(
    dnmi::db::Connection *dbCon, const CKvalObs::CService::WhichDataList &wData,
    CORBA::Long startIndex, CKvalObs::CService::WhichDataList *whichData) {
  if (!dbCon) {
  ...
  if ((startIndex + 1) >= wData.length()) {
    LOGERROR(
        "Get range of stations! " << "Starting from: " << labs( wData[startIndex].stationid) << ", but it is no ending station in the range." << endl <<"Too few WhichData elements.");
    return false;
  }
  ...
  if (!gate.select(
      ...
      kvQueries::selectStationsByRange(labs(wData[startIndex].stationid),
                                       ...
                                       true))) {
    LOGERROR(
        "Failed to get the station range [" << labs( wData[startIndex].stationid ) << "," <<labs( wData[startIndex+1].stationid ) << "> from the database!" << endl << "Reason: " << gate.getErrorStr());
    return false;
  }

  LOGDEBUG(
      "Got the station range [" << labs( wData[startIndex].stationid ) << "," <<labs( wData[startIndex+1].stationid ) << "> from the database!");
  ...
  for (CORBA::Long i = insertFrom; it != stations.end(); it++, i++) {
...
KvServiceImpl::createWhichDataListForAllStations(
    dnmi::db::Connection *dbCon, const CKvalObs::CService::WhichData &wData) {
  ...
  if (!gate.select(stations, kvQueries::selectAllStations("stationid"))) {
    LOGERROR(
        "Failed to get the station's from the database!" << gate.getErrorStr());
    return 0;
  }
  ...
  for (CORBA::Long i = 0; it != stations.end(); it++, i++) {
    (*wDataList)[i] = wData;
...
char*
KvServiceImpl::subscribeDataNotify(const DataSubscribeInfo& info,
...
char*
KvServiceImpl::subscribeData(const DataSubscribeInfo& info,
...
char*
KvServiceImpl::subscribeKvHint(CKvalObs::CService::kvHintSubscriber_ptr sub) {
...
void KvServiceImpl::unsubscribe(const char *subid) {
...
CORBA::Boolean KvServiceImpl::getData(const WhichDataList& whichData,
                                      DataIterator_out it) {
  ...
  if (whichData[0].stationid == 0) {
    LOGINFO(
        "It is asked for all stations!" << endl << "fromtime: " << miTime(whichData[0].fromObsTime) << " (" << whichData[0].fromObsTime << ")" << endl << "totime: " << miTime(whichData[0].toObsTime) << " (" << whichData[0].toObsTime << ")");
    ...
    try {
      ...
      while (i < whichData.length()) {
        if (whichData[i].stationid < 0) {
          if (!appendWhichDataFromStationRange(pCon, whichData, i,
                                               pWhichData)) {
            delete pWhichData;
            it = DataIterator::_nil();
            app.releaseDbConnection(pCon);
...
CORBA::Boolean KvServiceImpl::getModelData(
    const CKvalObs::CService::WhichDataList& whichData,
  ...
  if (!pCon) {
    LOGWARN(
        "Failed: No database connection .... (Increase max_connections in postgresql.conf?)." << endl << toString( whichData ));
    return false;
  }
  ...
  try {
    ...
    LOGERROR(
        "OUT OF MEMMORY ... Failed to make a copy of WhichDataList." << endl << toString( whichData ));
  ...
  try {
    ...
    LOGERROR(
        "OUT OF MEMMORY. Failed to create a ModelDataIterator." << endl << toString(whichData));
    app.releaseDbConnection(pCon);
    delete pWhichData;
    it = ModelDataIterator::_nil();
    return false;
  ...
  try {
    ...
    LOGERROR(
        "Cant register the DataIteratorImpl in the poa!" << endl << toString( whichData ));
    delete dataIt;
    it = ModelDataIterator::_nil();
    return false;
  }
  ...
  try {
    ...
    LOGERROR(
        "Cant obtain a referanse to the DataIterator!" << endl << toString( whichData ));
    delete dataIt;
    it = ModelDataIterator::_nil();
    return false;
  }
  ...
  if (CORBA::is_nil(it)) {
    LOGERROR(
        "cant instatiate (DataIterator)!!!!" << endl << toString( whichData ));
    delete dataIt;
    return false;
  }
...
CORBA::Boolean KvServiceImpl::getRejectdecode(
    const CKvalObs::CService::RejectDecodeInfo& info,
...
CORBA::Boolean KvServiceImpl::getReferenceStation(
  ...
  res = gate.select(stations,
                    kvQueries::selectReferenceStation(stationid, paramsetid));
  app.releaseDbConnection(pCon);
...
CORBA::Boolean KvServiceImpl::getStations(
    CKvalObs::CService::StationList_out stationList) {
...
CORBA::Boolean KvServiceImpl::getParams(ParamList_out paramList) {
  list<kvParam> params;
  ...
  for (CORBA::Long i = 0; it != params.end(); it++, i++) {
...
CORBA::Boolean KvServiceImpl::getObsPgm(
    CKvalObs::CService::Obs_pgmList_out obs_pgmList,
...
CORBA::Boolean KvServiceImpl::getTypes(
    CKvalObs::CService::TypeList_out typeList) {
  ...
  for (CORBA::Long i = 0; it != types.end(); it++, i++) {
...
CORBA::Boolean KvServiceImpl::getOperator(
    CKvalObs::CService::OperatorList_out operatorList) {
  ...
  for (CORBA::Long i = 0; it != operators.end(); it++, i++) {
...
CORBA::Boolean KvServiceImpl::getStationParam(
  ...
  if (day >= 0)
    s << " and fromday<=" << day << " and today>=" << day;
...
char*
KvServiceImpl::registerDataNotify(const char* id,
                                  kvDataNotifySubscriberExt_ptr sub) {
...
CORBA::Boolean KvServiceImpl::unregisterSubscriber(const char *id) {
  LogContext context("service/unregister");
  LOGWARN("service/unregister: NOT SUPPORTED");
  return false;
}

CORBA::Boolean KvServiceImpl::getDataExt(const WhichDataExtList& whichData,
                                         DataIterator_out it) {
...
CORBA::Boolean KvServiceImpl::getStationMetaData(
    Station_metadataList_out stMeta, CORBA::Long stationid,
    const char* obstime_, const char* metadataName) {
  ...
  LOGDEBUG(
      "request: obstime: " << (obstime.undef()?"undef" : obstime.isoTime()) << " stationid: " << stationid << " metadataName: " << metadataName);
  ...
  try {
    ...
    LOGERROR(
        "Unexpected EXCEPTION: Query:" << q.str() << endl << "Reason: Unknown.");
    retRes = false;
  }
...
CORBA::Boolean KvServiceImpl::getWorkstatistik(
    CKvalObs::CService::WorkstatistikTimeType timeType, const char *fromTime_,
    const char *toTime_, CKvalObs::CService::WorkstatistikIterator_out it) {
  ...
  try {
    //dataIt becomes the owner of the pointers 'con' and 'pWhichData'
    //and will delete them when dataIt is deleted.
    dataIt = new WorkstatistikIteratorImpl(pCon, fromTime, toTime, timeType,
                                           app);
  } catch (...) {
    LOGERROR("OUT OF MEMMORY (2)");
    app.releaseDbConnection(pCon);
