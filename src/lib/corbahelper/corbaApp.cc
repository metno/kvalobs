...
bool ServiceHost::decode(const std::string &serviceWithPort, int defaultPort) {
...
std::string ServiceHost::toString() const {
  ostringstream ost;
...
bool CorbaApp::isOk() const {
  return app != 0;
}

bool CorbaApp::putObjInNS(CORBA::Object_ptr objref, const std::string &name_) {
  ServiceHost ns;
  ...
  try {
    ...
    try {
      ...
      if (CORBA::is_nil(rootContext)) {
        CERR(
            "Failed to narrow the root naming context (CORBA Nameservice)" << endl);
        return false;
      }
    ...
    if (!nameContext.empty()) {
      try {
        ...
        CERR(
            "Context <" << nameContext << "> already exists , resolve context!\n");
        CORBA::Object_var obj;
        ...
        if (CORBA::is_nil(toContext)) {
          CERR(
              "ERROR: Failed to narrow naming context <" << nameContext << ">\n");
          return false;
        }
    ...
    CERR(
        "Caught system exception COMM_FAILURE -- unable to contact the " << "naming service." << endl);
    return false;
  } catch (CORBA::SystemException&) {
    CERR(
        "Caught a CORBA::SystemException while using the naming service." << endl);
    return false;
  }
...
std::string CorbaApp::corbaRef(CORBA::Object_ptr ptr) {
  CORBA::String_var s;
...
CORBA::Object_ptr CorbaApp::corbaRef(const std::string &ref) {
...
CORBA::Object_ptr CorbaApp::getObjFromNS(const std::string &name_) {
  ServiceHost ns;
...
std::string CorbaApp::setNameservice(const std::string &host) {
