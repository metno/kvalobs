...
bool createGlobalLogger(const std::string &path, const std::string &prefix,
                        const std::string &id, milog::LogLevel ll, int size,
                        int nFiles, StdLayout1 *layout) {
  try {
    ...
    if (logs->open(ost.str())) {
      ...
      if (!LogManager::createLogger(id, logs)) {
        LOGERROR(
            "Failed to create logger '" << id << "'. Logfile: '" << ost.str()<<"'.");
        delete logs;
        return false;
      }
