...
void 
milog::FLogStream::write(const std::string &message)
{
...
milog::FLogStream::FLogStream()
    : nRotate_(1), maxSize_(100*1024), fd(0)
{
  try{
...
milog::FLogStream::FLogStream(Layout *layout, 
			      int    nRotate, 
			      int    maxSize)
  :LogStream(layout), currentSize_(0), fd(0)
{
...
bool 
milog::FLogStream::open(const std::string &fname)
{
    string fileName;
    string pathName;
    string::size_type i;
