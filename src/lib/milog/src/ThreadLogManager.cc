...
namespace milog{
	...
	namespace{
	    milog::thread::ThreadLocalDataHolder<ThreadLogManager> tLogMgr_("__milog_key__");
	}
	...
	bool    
	ThreadLogManager::setLogger(const std::string &id, Logger *logger)
	{
	    if(!logger)
		return false;
	...
	void    
	ThreadLogManager::removeLogger(const std::string &id)
	{
	    ITLoggerList it=loggerList.find(id);
	...
	void 
	ThreadLogManager::updateContextStr()
	{
	    ostringstream ost;
	...
	void 
	ThreadLogManager::push(const std::string &context)
	{
