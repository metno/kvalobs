...
Logger::
Logger()
    :thrLocaleLoggers_(0), ownerOfLogger(true),level_(NOTSET), enabled_(true)
{
    StdErrStream *strm=0;
...
Logger(LogStream *logStream)
    :logger_(0), thrLocaleLoggers_(0), level_(NOTSET), enabled_(true), ownerOfLogger(true)
{
    try{
...
milog::
Logger::
setDefaultLogger(LogStream *logStrm)
{
...
milog::
Logger::
createLogger(const std::string &id, LogStream *ls)
{
...
milog::
Logger::
removeLogger(const std::string &id)
{
  priv::ThreadLogManager *tlm=priv::ThreadLogManager::instance();
...
milog::LogLevel 
milog::Logger::logLevel(LogLevel ll)
{
  LogLevel tmp=level_;
  level_=ll;
  return tmp;
...
milog::
Logger::
log(LogLevel ll, const std::string &msg)
{
	if(!logger_)
		return;
