...
{
  public:
    kv2kvDecoder( dnmi::db::Connection &con,
    ...
    void logMessages( bool doLog ) { doLog_ = doLog; }
    bool logMessages() const { return doLog_; }

  protected:
    kvDbGate dbGate;

    void parse( serialize::KvalobsData & data, const miutil::miString & obs ) const;
    void verifyAndAdapt( serialize::KvalobsData & data, std::list<kvalobs::kvData> & out );
//     void save( const serialize::KvalobsData & data );
    void save( const std::list<kvalobs::kvData> & dl, const std::list<kvalobs::kvTextData> & tdl );
    ...
    void invalidatePreviousData( serialize::KvalobsData & data, const std::list<serialize::KvalobsData::InvalidateSpec> & inv  );
    void invalidatePreviousTextData( serialize::KvalobsData & data, const std::list<serialize::KvalobsData::InvalidateSpec> & inv  );

    struct DecoderError
          : public std::runtime_error
    {
      const kvalobs::decoder::DecoderBase::DecodeResult res;
      DecoderError( kvalobs::decoder::DecoderBase::DecodeResult res_, const std::string & reason )
