...
int AExecClient::getExitCode(const std::string &buf_) {
  string buf = buf_;
  string tmp;
  ...
  try {
    ...
    errMsg = "FORMATERROR: EXITCODE: '" + buf_
        + "'. expecting 'exitcode executiontime'";
    return -1;
  }
}

pid_t AExecClient::exec(const std::string &command, const std::string &logfile,
                        int timeoutInSecond) {
  char sint[100];
  ostringstream cmd;
  string buf;
  ...
  if (*rit != '\n') {
    errMsg =
        "INVALIDFORMAT, the response from the server was invalid formatted.";
    return 0;
  }
...
int AExecClient::wait(int timeoutInSecondBetweenChar) {
  string buf;
  timeout_ = false;
