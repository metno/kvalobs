...
int socketGetCh(int fd, char &ch, bool &timedout, int timeoutInSec) {
  fd_set r, w, x;
  timeval tv;
  int rc;
  int ret;
  ...
  do {
    ...
    if (ret < 0) {
      std::cerr
          << "sockethelper: socketGetCh: select returned -1 errno!=EINTR!\n";
      return -1;
    } else if (ret == 0) {
...
int socketGetChExt(int fd, char &ch, bool &timedout, int timeoutInMillimSec) {
  fd_set r, w, x;
  timeval tv;
  int rc;
  int ret;
...
int readString(int fd, std::string &str, int timeoutInSec) {
  fd_set r, w, x;
  timeval tv;
  int n = 1;
  int rc;
...
int readStringExt(int fd, std::string &str, int timeoutInMillimSec) {
  fd_set r, w, x;
  timeval tv;
  int n = 1;
  int rc;
...
int writeString(int fd, const std::string &str) {
...
bool socketSetLinger(int sd, bool enable, int timeoutInSec) {
  struct linger li;
  ...
  if (setsockopt(sd,
  SOL_SOCKET,
                 SO_LINGER, (char *) &li, sizeof(linger)) == 0)
    return true;

  return false;
}

socket_status_codes::ErrorTypes setupConnection(const std::string &servname,
                                                int port, int& fd) {
  int err, enable;
  ...
  err = setsockopt(sd,
  SOL_SOCKET,
                   SO_REUSEADDR, (char *) &enable, sizeof(int));
