...
void SimpleSocket::setfd(int fd_) {
  if (fd_ <= 0)
    return;
...
SimpleSocket::SimpleSocket(int fd_)
    : fd(fd_),
      mConnected(fd_ > 0),
      timeout_(false) {
}
...
int SimpleSocket::getCh(char &ch, bool &timedout, int timeoutInSec) {
  if (!mConnected) {
...
int SimpleSocket::getChExt(char &ch, bool &timedout, int timeoutInMilliSec) {
  if (!mConnected) {
    //std::cerr << "SimpleSocket::getCh: NOT CONNECTED!\n";
    return -1;
  }
...
int SimpleSocket::writeBuf(const std::string &str) {
  if (!mConnected)
    return -1;
...
int SimpleSocket::writeln(const std::string &str) {
  if (!mConnected)
    return -1;
...
int SimpleSocket::readln(std::string &str,
                         int timeoutInSecBetweenReceivedBytes) {
  str = "";
...
int SimpleSocket::readlnExt(std::string &str,
...
bool SimpleSocket::setLinger(bool enable, int timeoutInSec) {
  if (mConnected == 0)
    return false;
...
bool SimpleSocket::getLinger(bool &enabled, int &timeout) {
  struct linger li;
  socklen_t len;
...
SimpleSocketClient::SimpleSocketClient(const std::string &servername, int port)
    : SimpleSocket() {
  mErrorType = socket_status_codes::NOT_CONNECTED_ERR;
...
bool SimpleSocketClient::connect(const std::string &servername, int port) {
  int fd_;
...
bool SimpleSocketServer::bind(int port_) {
  int sockfd;
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char *) &flag, sizeof(int))
      != 0) {
...
bool SimpleSocketServer::listen(int backlog) {
  if (!connected())
    return false;
...
SimpleSocket*
SimpleSocketServer::accept(bool &interupted, int timeout) {
  int newsockfd;
  socklen_t clilen;
  ...
  if (setsockopt(newsockfd, SOL_SOCKET, SO_REUSEADDR, (char *) &flag,
                 sizeof(int)) != 0) {
    mErrorType = socket_status_codes::SET_SOCKOPT_ERR;
    ::close(newsockfd);
    return 0;
