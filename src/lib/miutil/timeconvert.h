...
inline std::string to_kvalobs_string(const date & d) {
  std::ostringstream s;
  s << d.year() << '-' << std::setfill('0') << std::setw(2) << std::right
    << d.month().as_number() << '-' << std::setfill('0') << std::setw(2)
    << std::right << d.day();
  return s.str();
}
}
...
inline std::string to_kvalobs_string(const ptime & t, char separator = ' ') {
  ...
  s << to_kvalobs_string(t.date()) << separator
    << to_kvalobs_string(t.time_of_day());
  return s.str();
}

inline std::string to_kvalobs_string_without_decimal_secound(const ptime & t,
                                                             char separator =
                                                                 ' ') {
  std::ostringstream s;
  time_duration td = t.time_of_day();
  s << to_kvalobs_string(t.date()) << separator << std::setfill('0')
    << std::setw(2) << std::right << td.hours() << ":" << std::setfill('0')
    << std::setw(2) << std::right << td.minutes() << ":" << std::setfill('0')
    << std::setw(2) << std::right << td.seconds();
...
inline boost::posix_time::ptime time_from_string_nothrow(
  ...
  try {
    ...
    if (pos != std::string::npos) {
      ...
      if (s[pos] != 'Z') {
        ...
        if (so.length() == 3)
          ...
          offset = boost::posix_time::time_duration(
              boost::lexical_cast<int>(so.substr(0, 3)),
      ...
      return boost::posix_time::time_from_string(boost::trim_copy(time))
          - offset;
    }
    ...
    switch (s.length()) {
      case 14:
        if (std::sscanf(s.c_str(), "%4d%2d%2d%2d%2d%2d", &yy, &mm, &dd, &h, &m,
                        &sec) != 6)
          return boost::posix_time::ptime();
        break;
      case 12:
        if (std::sscanf(s.c_str(), "%4d%2d%2d%2d%2d", &yy, &mm, &dd, &h, &m)
            != 5)
          return boost::posix_time::ptime();
        break;
      case 10:
        if (std::sscanf(s.c_str(), "%4d%2d%2d%2d", &yy, &mm, &dd, &h) != 4)
          return boost::posix_time::ptime();
        break;
      case 8:
        if (std::sscanf(s.c_str(), "%4d%2d%2d", &yy, &mm, &dd) != 3)
          return boost::posix_time::ptime();
        break;
      default:
#ifdef LOGWARN
