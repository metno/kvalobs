...
dnmi::thread::ThreadPoolQue::ThreadPoolQue(unsigned int size_, CommandQue &q,
                                           int timeoutInSec)
    : que(q),
      size(size_),
      doKill(false),
...
void dnmi::thread::ThreadPoolQue::run() {
  for (int i = 0; i < size; i++) {
    thrGroup.create_thread(ThrPoolQueThread(que, *this));
  }
}

bool dnmi::thread::ThreadPoolQue::killSignal() {
...
void dnmi::thread::ThreadPoolQue::join() {
  thrGroup.join_all();
}
