...
bool kvalobs::kvStation::set( int   stationid__,
			      float lat__,
			      float lon__,
			      float height__,
			      float maxspeed__,
...
bool kvalobs::kvStation::set(const dnmi::db::DRow& r_)
{
  ...
  for(;it!=names.end(); it++){
    try{
      ...
      else if( *it=="height"    )  height_     = atoi(buf.c_str());
      else if( *it=="maxspeed"  )  maxspeed_   = atoi(buf.c_str());
      else if( *it=="name"      )  name_       = buf;
      else if( *it=="wmonr"     )  wmonr_      = atoi(buf.c_str());
      else if( *it=="nationalnr")  nationalnr_ = atoi(buf.c_str());
      else if( *it=="icaoid"    )  ICAOid_     = (buf.size() == 4 ? buf : "");
      else if( *it=="call_sign" )  call_sign_  = buf;
      else if( *it=="stationstr")  stationstr_ = buf;
      else if( *it=="environmentid"  ) environmentid_   = atoi(buf.c_str());
      else if( *it=="static"    )  static_     = (buf=="t");
      else if( *it=="fromtime"  )  fromtime_   = miTime(buf);
    }
    catch(...){
