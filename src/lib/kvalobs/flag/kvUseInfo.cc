...
bool kvUseInfo::setUseFlags(const kvControlInfo& cinfo) {
  ...
  //if ( cinfo.qc1Done() )
  {
    ...
    else if ((fd == 2 || fd == 4 || fd == 7 || fd == 8 || fd == 9 || fd == 0xA)
        && ui[7] == 0)
      ui[1] = 3;
  ...
  //if ( cinfo.qc1Done() or cinfo.qc2Done() or cinfo.hqcDone() )
  {
    ...
    else if (fagg > 4 || fr == 6 || fr == 0xA || fcc >= 0xA || fcp >= 0xA
        || fs >= 8 || ftime == 1 || ftime == 2 || fnum == 6 || fw == 0xA
        || fpos >= 4 || fd == 2 || fd == 4 || fd >= 7 || fpre >= 4 || fclim == 3
        || fcombi >= 9 || fhqc == 4 || fhqc >= 6)
      ui[2] = 3;

    else if (fagg == 3 || ((fr == 4 || fr == 5) && fcombi != 2 && fcombi != 1)
        || fcc == 3 || fcc == 4 || fcc == 6 || fcc == 7 || fcp == 3 || fcp == 4
        || fcp == 6 || fcp == 7 || fs == 3 || fw == 3 || fpos == 3 || fstat == 2
        || fd == 3)
      ui[2] = 2;

    else if (fagg == 2 || fr == 2 || fr == 3 || fcc == 2 || fcp == 2 || fs == 2
        || fs == 7 || fw == 2 || fclim == 2 || fcombi == 2)
      ui[2] = 1;

    else if (fagg == 1 || fr == 1 || fcc == 1 || fcp == 1 || fs == 1 || fs == 4
        || fw == 1 || fpos == 1 || fstat == 1 || fclim == 1 || fd == 1
        || fcombi == 1)
      ui[2] = 0;
  }
  ...
  //if ( cinfo.qc1Done() or cinfo.qc2Done() or cinfo.hqcDone() )
  {
    ...
    else if (fd == 9 || fd == 0xA || fagg == 8)
      ui[3] = 5;

    else if (fd == 7 || fd == 8 || fagg == 9)
      ui[3] = 6;
    ...
    else if (fagg == 6 || fcc == 0xA || fcc == 0xB || fcp == 0xA || fcp == 0xB
        || fs == 0xA || fpos == 4 || fpre == 4 || fclim == 3
        || ((fnum == 6 || ftime == 2) && fmis == 4))
      ui[3] = 3;
  ...
  //if ( cinfo.qc1Done() or cinfo.qc2Done() or cinfo.hqcDone() )
  {
    if (ui[2] == 0)
    ...
    else if ((fr == 0xA || (7 > fr && fr > 1)) && fcc <= 1 && fcp <= 1
        && fs <= 1 && fnum <= 5 && fpos <= 1 && ftime <= 1 && fw <= 1
        && fstat <= 1 && fclim <= 1)
      ui[4] = 1;

    else if ((fcc == 2 || fcc == 3 || fcc == 6 || fcc == 9 || fcc == 0xA
        || fcc == 0xD || fcp == 2 || fcp == 3 || fcp == 0xA) && fs <= 1
        && fnum <= 5 && fpos <= 1 && ftime <= 1 && fw <= 1 && fstat <= 1
        && fclim <= 1)
      ui[4] = 2;
      ...
      ui[4] = 3;
        ...
        || fcp == 0xB) && fnum <= 5 && ftime <= 1 && fw <= 1 && fstat <= 1
        && fclim <= 1)
      ui[4] = 4;
...
void kvUseInfo::Confidence(const int& c) {
  if (c < 0 || c > 100) {
    CERR(
        "kvUseInfo::Confidence ERROR setting illegal confidence:" << c << std::endl);
    return;
  }
  int i1 = c / 16;
  int i2 = c % 16;
...
void kvUseInfo::HQCid(const int& c) {
  if (c < 0 || c > 255) {
    CERR("kvUseInfo::HQCid ERROR setting illegal id:" << c << std::endl);
    return;
  }
  int i1 = c / 16;
  int i2 = c % 16;
