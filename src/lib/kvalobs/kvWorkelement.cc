...
kvalobs::kvWorkelement::kvWorkelement(const kvWorkelement &we) :
		stationid_(we.stationid_), obstime_(we.obstime_), typeid_(we.typeid_), tbtime_(
				we.tbtime_), priority_(we.priority_), process_start_(
				we.process_start_), qa_start_(we.qa_start_), qa_stop_(
				we.qa_stop_), service_start_(we.service_start_), service_stop_(
				we.service_stop_)
...
bool kvalobs::kvWorkelement::set(const dnmi::db::DRow &r_)
{
	...
	for (; it != names.end(); it++)
	{
		try
		{
			...
			{
				CERR(
						"kvWorkelement::set .. unknown entry:" << *it << std::endl);
			}
		} catch (...)
		{
