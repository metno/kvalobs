...
std::string kvQueries::selectStationParam(const std::list<int> slist,
  ...
  ost << ") AND SP1.qcx=\'" << qcx << "\'" << " AND SP1.fromday<="
      ...
      << "         AND  SP2.qcx =SP1.qcx " << " ) ORDER BY SP1.stationid DESC";
...
std::string kvQueries::selectDataFromType(
    const int sid, const int tid, const boost::posix_time::ptime& otime) {
  ostringstream ost;
...
std::string kvQueries::selectTextDataFromType(
    const int sid, const int tid, const boost::posix_time::ptime& otime) {
  ostringstream ost;
...
std::string kvQueries::selectDataFromAbsType(
    const int sid, const int tid, const boost::posix_time::ptime& otime) {
  ostringstream ost;
...
std::string kvQueries::selectData(const boost::posix_time::ptime& stime,
                                  const boost::posix_time::ptime& etime,
                                  const std::string& ob) {
  ostringstream ost;

  ost << " where obstime>=\'" << to_kvalobs_string(stime) << "\'"
      << " and obstime<=\'" << to_kvalobs_string(etime) << "\'" << " order by "
      << ob;

  return ost.str();
}

std::string kvQueries::selectDataByTabletime(
    const boost::posix_time::ptime& stime,
    const boost::posix_time::ptime& etime, const std::string& ob) {
  ostringstream ost;

  ost << " where tbtime>=\'" << to_kvalobs_string(stime) << "\'"
      << " and tbtime<=\'" << to_kvalobs_string(etime) << "\'" << " order by "
      << ob;
...
std::string kvQueries::selectData(const int sid,
  ...
  ost << " where stationid=" << sid << " and obstime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and obstime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by obstime, typeid DESC";

  return ost.str();
}

std::string kvQueries::selectTextData(const int sid,
                                      const boost::posix_time::ptime& stime,
                                      const boost::posix_time::ptime& etime) {
  ostringstream ost;
...
std::string kvQueries::selectDataByTbtime(
    const int sid, const boost::posix_time::ptime& stime,
    const boost::posix_time::ptime& etime) {
  ostringstream ost;

  ost << " where stationid=" << sid << " and tbtime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and tbtime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by tbtime,typeid";
...
std::string kvQueries::selectDataStat(const boost::posix_time::ptime& stime,
                                      const boost::posix_time::ptime& etime,
  ...
  ost << " where obstime>=\'" << to_kvalobs_string(stime) << "\'"
      ...
      << " and stationid in (" << statList << ") order by stationid,obstime";
...
std::string kvQueries::selectModelData(const int sid,
  ...
  ost << " where stationid=" << sid << " and obstime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and obstime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by obstime";
...
std::string kvQueries::selectObsPgm(long stationid,
  ...
  ost << " WHERE stationid=" << stationid << " AND " << "       (( fromtime<="
      << obst << " AND totime>" << obst << ") OR " << "        ( fromtime<="
      << obst << " AND totime IS NULL ) " << "       ) ORDER BY paramid";
  /*
...
/*
 ...
 - Valid fromtime is found by using a 'correlated subquery'
 */
std::string kvQueries::selectObsPgmByTypeid(
    long tid, const boost::posix_time::ptime& otime) {
  ostringstream ost;
  string obst("\'" + to_kvalobs_string(otime) + "\'");

  ost << " WHERE typeid=" << tid << " AND " << "       (( fromtime<=" << obst
      << " AND totime>" << obst << ") OR " << "        ( fromtime<=" << obst
      << " AND totime IS NULL ) " << "       ) ORDER BY stationid, typeid";
  /*
...
std::string kvQueries::selectData(const std::list<int> slist, const int pid,
  ...
  ost << ") and paramid=" << pid << " and obstime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and obstime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by obstime";
...
std::string kvQueries::selectData(const std::list<int> slist, const int pid,
  ...
  ost << ") and paramid=" << pid << " and typeid=" << tid << " and obstime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and obstime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by obstime";
...
std::string kvQueries::selectData(const int stid, const int pid, const int tid,
  ...
  ost << " WHERE stationid=" << stid << " and paramid=" << pid << " and typeid="
      << tid << " and obstime>=\'" << to_kvalobs_string(stime) << "\'"
      << " and obstime<=\'" << to_kvalobs_string(etime) << "\'"
      << " order by obstime";
...
std::string kvQueries::selectData(const int pid, const int tid,
  ...
  ost << " WHERE paramid=" << pid << " and typeid=" << tid
      << " and controlinfo=\'" << controlString << "\'" << " and obstime>=\'"
      << to_kvalobs_string(stime) << "\'" << " and obstime<=\'"
      << to_kvalobs_string(etime) << "\'" << " order by obstime";
 ...
 */
std::string kvQueries::selectMissingData(
    const float value, const int pid, const boost::posix_time::ptime& Ptime) {
  ostringstream ost;
 ...
 */
std::string kvQueries::selectMissingData(
    const float value, const int pid, const int tid,
    const boost::posix_time::ptime& Ptime) {
  ostringstream ost;

  ost << " WHERE original=" << value << " and paramid=" << pid << " and typeid="
      << tid << " and obstime=\'" << to_kvalobs_string(Ptime) << "\'"
      << " order by obstime";
