...
std::string kvalobs::kvStationParam::toSend() const
{
  ...
  ost << "("
      ...
      << quoted(qcx_)         << ","            
      << quoted(metadata_)    << "," 
      << quoted(descMetadata_)<< ","
      << quoted(fromtime_)    << ")";   
  return ost.str();
}


bool kvalobs::kvStationParam::set(int stationid__,
                                  int paramid__,
				  int level__,
				  int sensor__,
				  int fromday__,
				  ...
				  const miutil::miTime& fromtime__)
{
  ...
  sortBy_= boost::lexical_cast<std::string>(stationid_)+ boost::lexical_cast<std::string>(paramid_);
  return true;
}
