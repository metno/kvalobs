...
namespace kvalobs{
  ...
  public:
    kvKeyVal() {clean();}
    kvKeyVal(const kvKeyVal &keyVal){ set(keyVal);}
    kvKeyVal(const dnmi::db::DRow &r){set(r);}
    kvKeyVal(const std::string &package,
    ...
    char* tableName()            const {return "key_val";}
    ...
    void package(const std::string &v) { package_=v;  }
    void     key(const std::string &v) { key_=v;      }
