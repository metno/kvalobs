       ...
       bool replace)
{
  ...
  if(!elem.exists()){
    err_=InvalidObject;
    errStr_="InvalidObject: The kvDbBase object for table <" + 
      tblName + "> is invalid!";
    return false;
  }
	...
	if(replace){
	  ...
	  while(timeout>=now && busy){
	    ...
	    try{
	      ...
	      err_=Busy;
	      errStr_=string("Busy, Duplicate (replace): [") + ex.what()+string("]");
	      time(&now);
	      busy=true;
	    }
	    catch(dnmi::db::SQLNotConnected & ex) {
...
replace(const kvalobs::kvDbBase &elem, const std::string &tblName)
{
  ...
  if(!elem.exists()){
    err_=InvalidObject;
    errStr_="The kvDbBase object for table <" + 
      tblName+ 
      "> is invalid!";
    return false;
  }
