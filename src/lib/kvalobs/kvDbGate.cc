			...
			} catch (dnmi::db::SQLDuplicate & ex)
			{
				if (replace)
				{
					...
					while (timeout >= now && busy)
					{
						...
						{
							err_ = Busy;
							errStr_ = string("Busy, Duplicate (replace): [")
									+ ex.what() + string("]");
							time(&now);
							busy = true;
						} catch (dnmi::db::SQLNotConnected & ex)
						{
							err_ = NotConnected;
							errStr_ = string(
									"NotConnected, Duplicate (replace): [")
									+ ex.what() + string("]");
							return false;
						} catch (dnmi::db::SQLAborted &ex)
						{
							err_ = Aborted;
							errStr_ = string("Aborted, Duplicate (replace): [")
									+ ex.what() + string("]");
							return false;
						} catch (dnmi::db::SQLException & ex)
						{
							err_ = Error;
							errStr_ = string("Error, Duplicate (replace) : [")
									+ ex.what() + string("]");
							return false;
						} catch (...)
						{
							err_ = UnknownError;
							errStr_ =
									"UnknownError, Duplicate (replace): Unknown exception.";
							return false;
						}
					}
				...
				else
				{
					err_ = Duplicate;
					errStr_ =
							"Duplicate, Duplicate (no replace): Duplicate key!";
					return false;
				}
			} catch (dnmi::db::SQLNotConnected & ex)
			{
