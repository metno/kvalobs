...
public:
	typedef enum
	{
		NoError, Duplicate, ///Cant insert because of duplicate keys
...
protected:
	...
	bool insertImpl(const std::list<kvalobs::kvDbBase*> &elem,
			const std::string &tblName, bool replace = false);

	bool insertImpl(const std::list<kvalobs::kvDbBase*> &elem,
			const std::string &tblName, InsertOption option);
...
public:
	...
	kvDbGate() :
			con(0), busytimeout_(0)
	{
	}
	kvDbGate(dnmi::db::Connection *con_) :
			con(con_), busytimeout_(0)
	{
	}
	~kvDbGate()
	{
	...
	void busytimeout(int btInSecond)
	{
		busytimeout_ = btInSecond;
	}
	...
	void set(dnmi::db::Connection *con_)
	{
		con = con_;
	}
	 ...
	 *        if not.
	 */
	bool insert(const kvalobs::kvDbBase &elem, const std::string &tblName,
			bool replace = false);
	 ...
	 */
	bool insert(const kvalobs::kvDbBase &elem, const char *tblName,
			bool replace = false);
	...
	template<class T>
	bool insertList(const std::list<T>& li, InsertOption option =
			INSERT_OR_UPDATE, const std::string &tblName = "")
	{
		typename std::list<T>::const_iterator it = li.begin();
		std::list<kvalobs::kvDbBase*> myList;
