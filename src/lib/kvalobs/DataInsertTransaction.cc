...
void
DataInsertTransaction::
update( kvalobs::kvDbBase *data )
{
   ostringstream ost;

   ost << "UPDATE " << (tblName.empty()?data->tableName():tblName) << " " << data->toUpdate();
   conection->exec(ost.str());
}
...
bool
DataInsertTransaction::
operator()( dnmi::db::Connection *con )
{
   index = 0;
   fastExit = false;
   lastError.erase();
