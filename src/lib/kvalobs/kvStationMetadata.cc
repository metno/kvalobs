...
namespace kvalobs {
namespace {
template <typename T> class Interpreted {
public:
...
kvStationMetadata::kvStationMetadata()
    : station_(0), param_(INT_NULL), type_(INT_NULL), level_(INT_NULL),
      sensor_(INT_NULL), name_(""),
...
kvStationMetadata::kvStationMetadata(int station, const int *param,
                                     ...
                                     const int *sensor, const std::string &name,
                                     float metadata,
                                     const std::string &description,
                                     const boost::posix_time::ptime &fromtime,
                                     const boost::posix_time::ptime &totime)
    : station_(station), param_(param ? *param : INT_NULL),
      type_(type ? *type : INT_NULL), level_(level ? *level : INT_NULL),
      sensor_(sensor ? *sensor : INT_NULL), name_(name), metadata_(metadata),
      description_(description), fromtime_(fromtime), totime_(totime) {}
...
kvStationMetadata::kvStationMetadata(const kvStationMetadata &d)
    : station_(d.station_), param_(d.param_), type_(d.type_), level_(d.level_),
      sensor_(d.sensor_), name_(d.name_), metadata_(d.metadata_),
      description_(d.description_), fromtime_(d.fromtime_), totime_(d.totime_) {
}

kvStationMetadata::~kvStationMetadata() {}

kvStationMetadata &kvStationMetadata::operator=(const kvStationMetadata &rhs) {
  if (&rhs != this) {
...
const char *kvStationMetadata::tableName() const { return "station_metadata"; }
