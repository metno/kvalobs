...
namespace {
milog::LogLevel getLevelFromVal(const miutil::conf::ValElementList &vals_,
                                const string &what, const string &section_,
                                const milog::LogLevel defLoglevel) {
  string section(section_);
  miutil::conf::ValElementList &vals =
      const_cast<miutil::conf::ValElementList&>(vals_);
  ...
  LOGERROR(
      "Invalid " << what << " value: '" << sVal << "' in section '"<<section<< "'. Valid values fatal, error, warn, info or debug.");

  return defLoglevel;

}

int getNumber(const string &numberStr, bool allowPrefix) {
...
milog::LogLevel getXLevel(const miutil::conf::ConfSection *conf,
                          const std::string &section, const std::string &what,
                          const milog::LogLevel defLoglevel) {
  if (!conf)
    return defLoglevel;
...
milog::LogLevel getXLevelRecursivt(const miutil::conf::ConfSection *conf,
...
milog::LogLevel loglevelFromString(const std::string &level) {
...
milog::LogLevel getTracelevel(const miutil::conf::ConfSection *conf,
                              const std::string &section,
                              const milog::LogLevel defLoglevel) {
  return getXLevel(conf, section, "tracelevel", defLoglevel);
}

milog::LogLevel getTracelevelRecursivt(const miutil::conf::ConfSection *conf,
                                       const std::string &startAtSection,
                                       const milog::LogLevel defLoglevel) {
  return getXLevelRecursivt(conf, startAtSection, "tracelevel", defLoglevel);
}

milog::LogLevel getLoglevel(const miutil::conf::ConfSection *conf,
                            const std::string &section,
                            const milog::LogLevel defLoglevel) {
  return getXLevel(conf, section, "loglevel", defLoglevel);
}

milog::LogLevel getLoglevelRecursivt(const miutil::conf::ConfSection *conf,
                                     const std::string &startAtSection,
                                     const milog::LogLevel defLoglevel) {
  return getXLevelRecursivt(conf, startAtSection, "loglevel", defLoglevel);
}

void getLogfileInfo(const miutil::conf::ConfSection *conf,
  ...
  try {
    ValElementList vals;
    vals = conf->getValueRecursivt(startAtSection, "logfile_info", section,
                                   defValues);
