...
{
  return d.controlinfo().flag( fnum ) == 6
    and not hqc::hqc_touched( d );
}
...
kvDataFactory::kvDataFactory( int stationID, const miutil::miTime & obstime, int typeID, int sensor, int level )
    : stationID_( stationID ), obstime_( obstime ), typeID_( typeID ), sensor_( sensor ), level_( level )
{}

kvDataFactory::kvDataFactory( const kvData & d )
    : stationID_( d.stationID() ), obstime_( d.obstime() ), typeID_( d.typeID() ), sensor_( d.sensor() ), level_( d.level() )
{}


kvData kvDataFactory::getMissing( int paramID, const miutil::miTime & obstime ) const
{
...
kvData kvDataFactory::getData( float val, int paramID, const miutil::miTime & obstime ) const
{
...
bool same_kvData::operator()( const kvData & a, const kvData & b ) const
{
  return a.stationID() == b.stationID()
    ...
    and a.level() == b.level()
    and eq_sensor( a.sensor(), b.sensor() )
...
bool same_obs_and_parameter::operator()( const kvData & a, const kvData & b ) const
{
  return a.stationID() == b.stationID()
    and a.paramID() == b.paramID()
    and a.obstime() == b.obstime()
...
bool exactly_equal_ex_tbtime::operator()( const kvData & a, const kvData & b ) const
{
  return same_obs_and_parameter()( a, b )
      ...
      and a.useinfo() == b.useinfo()
      and a.cfailed() == b.cfailed();
}

bool exactly_equal::operator()( const kvData & a, const kvData & b ) const
{
  return exactly_equal_ex_tbtime()( a, b ) and
    a.tbtime() == b.tbtime();
}
