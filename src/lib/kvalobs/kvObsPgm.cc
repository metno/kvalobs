...
std::string kvalobs::kvObsPgm::toSend() const {
  ostringstream ost;
  ost << "(" << stationid_ << "," << paramid_ << "," << level_ << ","
      << nr_sensor_ << "," << typeid_ << "," << quoted(collector_) << ","
      << quoted(kl00_) << "," << quoted(kl01_) << "," << quoted(kl02_) << ","
      << quoted(kl03_) << "," << quoted(kl04_) << "," << quoted(kl05_) << ","
      << quoted(kl06_) << "," << quoted(kl07_) << "," << quoted(kl08_) << ","
      << quoted(kl09_) << "," << quoted(kl10_) << "," << quoted(kl11_) << ","
      << quoted(kl12_) << "," << quoted(kl13_) << "," << quoted(kl14_) << ","
      << quoted(kl15_) << "," << quoted(kl16_) << "," << quoted(kl17_) << ","
      << quoted(kl18_) << "," << quoted(kl19_) << "," << quoted(kl20_) << ","
      << quoted(kl21_) << "," << quoted(kl22_) << "," << quoted(kl23_) << ","
      << quoted(mon_) << "," << quoted(tue_) << "," << quoted(wed_) << ","
      << quoted(thu_) << "," << quoted(fri_) << "," << quoted(sat_) << ","
      << quoted(sun_) << "," << quoted(fromtime_) << "," << quoted(totime_)
      << ")";
  return ost.str();
}

bool kvalobs::kvObsPgm::set(int stationid, int paramid, int level,
                            int nr_sensor, int typ, bool collector, bool kl00,
                            bool kl01, bool kl02, bool kl03, bool kl04,
                            bool kl05, bool kl06, bool kl07, bool kl08,
                            bool kl09, bool kl10, bool kl11, bool kl12,
                            bool kl13, bool kl14, bool kl15, bool kl16,
                            bool kl17, bool kl18, bool kl19, bool kl20,
                            bool kl21, bool kl22, bool kl23, bool mon, bool tue,
                            bool wed, bool thu, bool fri, bool sat, bool sun,
...
bool kvalobs::kvObsPgm::isOn(const boost::posix_time::ptime& t) const {
  int dayW =
      days_before_weekday(
          t.date(), boost::gregorian::greg_weekday(boost::gregorian::Sunday))
          .days();
...
std::string kvalobs::kvObsPgm::uniqueKey() const {
  ostringstream ost;

  ost << " WHERE stationid=" << stationid_ << " AND " << "       typeid="
      << typeid_ << " AND " << "       paramid=" << paramid_ << " AND "
      << "       level=" << level_ << " AND " << "       fromtime="
      << quoted(fromtime_) << " AND " << "       totime=" << quoted(totime_);
