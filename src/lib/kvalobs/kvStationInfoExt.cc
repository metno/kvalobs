...
kvalobs::kvStationInfoExt::kvStationInfoExt(const kvalobs::kvStationInfoExt &info)
  :stationid_(info.stationid_), obstime_(info.obstime_), typeid_(info.typeid_),
...
addParam(  const Param &param  )
{
   for( list<Param>::iterator it=params_.begin(); it != params_.end(); ++it )
      if( it->paramid == param.paramid && it->sensor == param.sensor && it->level == param.level )
         return;
...
void
kvalobs::
populateCorbaKvStationInfoExtList( const kvalobs::kvStationInfoExtList &stList,
...
kvalobs::operator<<(std::ostream& os, const kvalobs::kvStationInfoExt &inf_)
{
  ...
  os << "kvStationInfoExt: stationid: " << inf.stationID()
     << " obstime: " << inf.obstime() << " typeId: " << inf.typeID() << " #params: " << inf.params_.size()
     << endl;
