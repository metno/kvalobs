  ...
  :CorbaHelper::CorbaApp(argn, argv, opt)
{
    ...
    if(kvPathInCorbaNS.empty()){
      char *buf=getenv("KV_PATH_IN_NS");
      
      if(buf){
	     kvPathInCorbaNS=buf;
      } else {
         LOGFATAL("Path in CORBA nameserver missing. " << endl <<
                   "Set the corba.path value in $KVALOBS/etc/kvalobs.conf" <<  endl
                   << " or set the environment variavble KV_PATH_IN_NS." << endl);
    ...
    if(corbaNS.empty()) {
      LOGFATAL("No CORBA nameserver given!" << endl << 
              "Set the corba.nameserver value in $KVALOBS/etc/kvalobs.conf " << endl <<
              " or set the environment variable KV_CORBA_NS!" << endl);
      printUseMsgAndExit( 1 ); 
    }          
...
KvApp::isOk()const
{
    if(kvPath_.empty() || !CorbaHelper::CorbaApp::isOk())
	return false;
...
namespace{
  ...
  confLoader()
  {
    ...
    conffile=kvPath()+"etc/" + KvApp::getConfFile();
