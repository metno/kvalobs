  ...
  
{
	...
	kvStationInfo stationInfo(st[i].stationId, 
				  miTime(st[i].obstime), 
				  st[i].typeId_);
...
{
  stationInfoList.push_back(kvStationInfo(st.stationId, 
					  miTime(st.obstime), 
					  st.typeId_));
}

bool       
kvalobs::StationInfoCommand::addStationInfo(const kvStationInfo &stationInfo)
{
    try{
	stationInfoList.push_back(stationInfo);
	return true;
...
{
  CIkvStationInfoList it=stationInfoList.begin();
