...

{
   for(CORBA::Long i=0; i<st.length(); i++){
      kvStationInfoExt stationInfo(st[i].stationId,
                                   miTime(st[i].obstime),
                                   st[i].typeId_);
...
kvalobs::StationInfoCommand::
StationInfoCommand(const CKvalObs::StationInfoExtList &st)
{
   for(CORBA::Long i=0; i<st.length(); i++){
      kvStationInfoExt stationInfo( st[i].stationId,
                                    miTime(st[i].obstime),
      ...
      for( CORBA::Long j=0; j<st[i].params.length(); ++j )
         stationInfo.addParam( kvStationInfoExt::Param( st[i].params[j].paramid,
...
{
   stationInfoList.push_back( kvStationInfoExt( st.stationId,
                                                miTime(st.obstime),
                                                st.typeId_)          );
}

bool       
kvalobs::StationInfoCommand::addStationInfo(const kvStationInfo &si )
{
...
bool
kvalobs::StationInfoCommand::
addStationInfo(const kvStationInfoExt &stationInfo)
{
   try{
      stationInfoList.push_back(stationInfo);
      return true;
...
void       
kvalobs::StationInfoCommand::debugInfo(std::ostream &os)const
{
...
getStationInfo()
{
   ...
   for( kvStationInfoExtList::const_iterator it=stationInfoList.begin(); it!=stationInfoList.end(); ++it )
      stationInfoListLegacy.push_back( kvStationInfo( it->stationID(), it->obstime(), it->typeID() ) );
...
getStationInfo() const
{
   ...
   for( kvStationInfoExtList::const_iterator it=stationInfoList.begin(); it!=stationInfoList.end(); ++it )
      const_cast<kvalobs::StationInfoCommand*>(this)->
      stationInfoListLegacy.push_back( kvStationInfo( it->stationID(), it->obstime(), it->typeID() ) );
