...
template<class kvApp>
    const CKvalObs::CDataSource::Result_var DataReinserter<kvApp>::insert( kvData & d ) const
{
...
template<class kvApp>
    const CKvalObs::CDataSource::Result_var DataReinserter<kvApp>::insert( std::list<kvData> &dl) const
{
...
template<class kvApp> const CKvalObs::CDataSource::Result_var
    DataReinserter<kvApp>::insert( const kvalobs::serialize::KvalobsData & data ) const
{
  ...
  for ( std::list<kvData>::iterator it = dlist.begin(); it != dlist.end(); ++ it ) {
    kvUseInfo i = it->useinfo();
    i.HQCid( this->operatorID );
    it->useinfo(i);
    dcopy.insert( * it );
  ...
  const CKvalObs::CDataSource::Result_var ret = app->sendDataToKv( xml.c_str(), decoderID );
