...
const string createString(const kvData &d) {
  ...
  ss << d.stationID() << internSeparator << to_kvalobs_string(d.obstime())
     << internSeparator << d.original() << internSeparator << d.paramID()
     << internSeparator << to_kvalobs_string(d.tbtime()) << internSeparator
     << d.typeID() << internSeparator << sensor << internSeparator << d.level()
     << internSeparator << d.corrected() << internSeparator
     << d.controlinfo().flagstring() << internSeparator
     << d.useinfo().flagstring() << internSeparator << d.cfailed();
  return ss.str();
}
...
const string createString(kvDataList dl) {
  ...
  for (kvDataList::const_iterator it = dl.begin(); it != dl.end(); it++) {
...
kvDataList getKvData(const string & s) {
  ...
  for (int i = 0; i < noOfElements; i++) {
    ...
    try {
      ...
      ret.push_back(
          kvData(pos, obt, org, par, tbt, typ, sen, lvl, cor, cIn, uin, fai));
    } catch (std::exception &e) {
      LOGERROR(
          "Error during parsing of kvData!:" << endl << "Line " << i << endl << current << endl << e.what());
