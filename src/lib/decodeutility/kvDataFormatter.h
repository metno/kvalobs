...
namespace decodeutility {
  ...
  namespace kvdataformatter {
     ...
     */
    const miutil::miString createString(const kvalobs::kvData &d);
     ...
     */
    const miutil::miString createString(kvDataList dl);


    template< class Iterator >
    const miutil::miString createString( Iterator first, Iterator last )
    {
      std::stringstream s;
      while ( first != last ) {
	s  << createString( *first )
	   << mainSeparator;
	first++;
      }
      return (const miutil::miString) miutil::miString(s.str());
    }
