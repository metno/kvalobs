...
KvalobsData::KvalobsData( const std::list<kvData> & data, const std::list<kvTextData> & tdata )
  : overwrite_( false )
{
...
{
  obs_[d.stationID()][d.typeID()][d.obstime()].textData[d.paramID()].content() = d;
}
  ...
  for ( Observations::const_iterator s = obs_.begin(); s != obs_.end(); ++ s ) {
    for ( StationID::const_iterator t = s->begin(); t != s->end(); ++ t ) {
      for ( TypeID::const_iterator o = t->begin(); o != t->end(); ++ o ) {
        for ( ObsTime::const_iterator sensor = o->begin(); sensor != o->end(); ++ sensor ) {
          for ( Sensor::const_iterator level = sensor->begin(); level != sensor->end(); ++ level ) {
            for ( Level::const_iterator param = level->begin(); param != level->end(); ++ param ) {
              ...
              kvData d( s->get(), o->get(), c.original, param->paramID(),
                        tbtime, t->get(), sensor->get(), level->get(),
                        c.corrected, c.controlinfo, c.useinfo, c.cfailed );
              out.push_back( d );
            }
          }
        }
...
void KvalobsData::getData( list<kvTextData> & out, const miutil::miTime & tbtime ) const
{
  ...
  for ( Observations::const_iterator s = obs_.begin(); s != obs_.end(); ++ s ) {
    for ( StationID::const_iterator t = s->begin(); t != s->end(); ++ t ) {
      for ( TypeID::const_iterator o = t->begin(); o != t->end(); ++ o ) {
        for ( Container<TextDataItem>::const_iterator param = o->textData.begin(); param != o->textData.end(); ++ param ) {

          kvTextData d( s->get(), o->get(), param->content().original, param->paramID(),
                        tbtime, t->get() );
          out.push_back( d );
        }
      }
...
void KvalobsData::invalidate( bool doit, int station, int typeID, const miutil::miTime & obstime )
{
  obs_[station][typeID][obstime].invalidate( doit );
}

bool KvalobsData::isInvalidate( int station, int typeID, const miutil::miTime & obstime ) const
{
