...
std::string KvalobsDataSerializer::toString() const
{
  ...
  for ( Observations::const_iterator station = data_.obs().begin(); station != data_.obs().end(); ++ station ) {
    Element * st = set_( root, "station", station->get() );
    for ( StationID::const_iterator type = station->begin(); type != station->end(); ++ type ) {
      Element * tp = set_( st, "typeid", type->get() );
      for ( TypeID::const_iterator obstime = type->begin(); obstime != type->end(); ++ obstime ) {
        Element * ot = set_( tp, "obstime", obstime->get() );
        if ( obstime->invalidate() )
          ot->set_attribute( "invalidate", "1" );

        // kvData:
        for ( ObsTime::const_iterator sensor = obstime->begin(); sensor != obstime->end(); ++ sensor ) {
          Element * snsr = sensor->get() ? set_( ot, "sensor", sensor->get() ) : ot;
          for ( Sensor::const_iterator level = sensor->begin(); level != sensor->end(); ++ level ) {
            Element * lvl = level->get() ? set_( snsr, "level", level->get() ) : snsr;
            for ( Level::const_iterator rest = level->begin(); rest != level->end(); ++ rest ) {
              Element * kvdata = lvl->add_child( "kvdata" );
              const DataContent & content = rest->content();
              kvdata->set_attribute( "paramid", lexical_cast<string>( rest->paramID() ) );
              Element * original = kvdata->add_child( "original" );
              original->add_child_text( lexical_cast<string>( content.original ) );
              if ( std::abs( content.original - content.corrected ) > 0.01 ) {
                Element * corrected = kvdata->add_child( "corrected" );
                corrected->add_child_text( lexical_cast<string>( content.corrected ) );
              }
        ...
        // kvTextData:
        for ( Container<TextDataItem>::const_iterator rest = obstime->textData.begin(); rest != obstime->textData.end(); ++ rest ) {
          Element * kvtextdata = ot->add_child( "kvtextdata" );
          kvtextdata->set_attribute( "paramid", lexical_cast<string>( rest->paramID() ) );
