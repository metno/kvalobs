...
class SortedData : public Container<Next>
{
    Content content_;
  public:
    SortedData( Content c ) : content_( c ) {}

    Content & get() { return content_; }

    const Content & get() const { return content_; }
...
class LeafItem
{
  ...
  Content content_;
  public:
    LeafItem( int param, const Content & content = Content() ) : paramID_( param ), content_( content ) {}
    size_t count() const { return 1; }
    bool operator<( const LeafItem<Content> & d ) const { return paramID_ < d.paramID(); }
...
struct DataContent
{
  ...
  : original( orig ), corrected( corr ), controlinfo( ci ), useinfo( ui ), cfailed( fail )
  {}
  DataContent( const kvData & d )
    : original( d.original() ), corrected( d.corrected() )
...
{
  explicit TextDataContent( const std::string & orig ) : original( orig ) {}
  TextDataContent( const kvTextData & d ) : original( d.original() ) {}
  TextDataContent() {}
  std::string original;
};
