...
namespace {
void throwOnError(const File &f) {
  if (!f.ok()) {
    if ( errno == EACCES)
 ...
 */
bool getStatOrThrow(const string &path, struct stat &sbuf,
                    bool throwIfFileDoNotExist = false) {
...
file_error::file_error(int errno_, const std::string &what__)
    : errno_(errno_) {
  ostringstream err;
...
file_error::file_error(int errno_)
    : errno_(errno_) {
...
bool pathExist(const std::string &path, bool onlyDirAndRegularFiles) {
  struct stat sbuf;
