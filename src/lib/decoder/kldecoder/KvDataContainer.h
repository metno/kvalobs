...
public:
	...
	typedef std::map< int, std::map<int, std::map<boost::posix_time::ptime, TextDataList> > > TextData;
	typedef std::map< int, std::map<int, std::map<boost::posix_time::ptime, DataList> > > Data;
	...
	KvDataContainer():data_(0){}
	///Deletes kvData after it is consumed.
	KvDataContainer( kvalobs::serialize::KvalobsData *kvData);
	~KvDataContainer();

	int getTextData( TextData &textData, const boost::posix_time::ptime &tbtime=boost::posix_time::ptime() )const;
	int getData( Data &data, const boost::posix_time::ptime &tbtime=boost::posix_time::ptime())const;
	int get( TextData &textData, Data &data, const boost::posix_time::ptime &tbtime=boost::posix_time::ptime() )const;

	int getTextData( TextDataByObstime &textData,
			         int stationid, int typeId,
			         const boost::posix_time::ptime &tbtime=boost::posix_time::ptime() )const;
	int getData( DataByObstime &data,
	             int stationid, int typeId,
			     const boost::posix_time::ptime &tbtime=boost::posix_time::ptime())const;

	int get( DataByObstime &data,
