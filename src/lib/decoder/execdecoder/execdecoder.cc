...
class aexecd_error : public exception {
 public:
  ...
  ErrorType error;
  string msg;
  aexecd_error(const std::string &m, ErrorType e)
      : error(e) {
    ostringstream o;
    switch (e) {
      case FormatError:
...
pair<string, string> getKey(const std::string &keyval) {
  string key;
  string val;
...
ExecDecoder::ExecDecoder(dnmi::db::Connection &con, const ParamList &params,
...
void ExecDecoder::writeProgLog(const std::string &logfileToWrite,
  ...
  sprintf(tb, "%04d%02d%02d", int(now.date().year()),
          now.date().month().as_number(), now.date().day().as_number());
...
void ExecDecoder::decodeObstype() {
  ...
  ostringstream ost;
  for (list<string>::iterator it = obsTypeList.begin(); it != obsTypeList.end();
      ++it) {
...
bool ExecDecoder::hasDecoderConfSection() {
  ...
  if (!decoderSection) {
    string myName = name();
    LOGERROR(
        "No section defined for obstype '" << decoderName_ << "' in section 'kvDataInpud." << myName << ".decoders' in the configuration file.");
    decoderName_.erase();
    return false;
  }
...
std::string ExecDecoder::createDecoderConfKey(const std::string &key) const {
  if (key.empty())
...
std::string ExecDecoder::name() const {
  return string("ExecDecoder");
}

std::string ExecDecoder::getBindir() {
  string bindir;
  ConfSection *conf = myConfSection();
  ...
  if (val.empty()) {
    ...
    if (!miutil::file::isdir(bindir)) {
      LOGERROR(
          "The bindir variable is NOT set. Tryning to use '" << bindir << "', but it must be a directory or we have not permission to use it.");
      return "";
    }
    LOGINFO(
        "The bindir variable is NOT set. Using '" << bindir << "' as default directory to look up decoder programs.");
    return bindir;
  }
  ...
  try {
    ...
    LOGERROR(
        "The bindir '" << bindir << " does NOT exist or we do not have permission to access it. (" << ex.what()<<".)");
    return "";
  }
}

std::string ExecDecoder::getDecoderName() {
...
std::string ExecDecoder::getDecoderProg() {
  ...
  try {
    if (prog[0] != '/') {
      ...
      if (bindir.empty()) {
        LOGERROR(
            "The '" << progkey << "' must be an absolute path or relative to bindir, but bindir is undefined or invalid.");
        return "";
      }
    ...
    if (!miutil::file::isRunable(prog)) {
      LOGERROR(
          "The decoder: '" << prog << "' do NOT exist or is NOT executable.");
      return "";
    }
    ...
    LOGERROR(
        "The decoder '" << prog << " does NOT exist or we do not have permission to access it. (" << ex.what() <<".)");
    return "";
  }
}

miutil::conf::ValElementList ExecDecoder::getConfKey(
    const std::string &key, const miutil::conf::ValElementList &defaultVal) {
  return getKeyInMyConfSection(key, defaultVal);
}

miutil::conf::ValElementList ExecDecoder::getDecoderConfKey(
    const std::string &key, const miutil::conf::ValElementList &defaultVal) {
  string decoder = getDecoderName();
...
bool ExecDecoder::createInputFile(const std::string &filename) {
...
bool ExecDecoder::getAexecd(std::string &host, int &port) {
  ...
  if (port < 0 || host.empty()) {
    LOGERROR(
        "Format error: aexecd=(\"hostname\", port). Values: host '" << host << "' port '" <<val[1].valAsString() <<"'.");
    return false;
  }
...
int ExecDecoder::getProgTimeout(int defaultTimeout) {
  return getDecoderConfKey("timeout").valAsInt(defaultTimeout);
}

int ExecDecoder::runProg(const std::string &cmd, const std::string &logfile,
                         const std::string &someId) {
  string host;
  int port;
  ...
  if (!getAexecd(host, port))
    throw aexecd_error("Expecting aexecd=(\"hostname\", port)",
                       aexecd_error::FormatError);

  LOGINFO(
      "Connecting to aexecd at '" << host << ":" << port<<"'. Timeout: " << timeout << ".");
  ...
  if (ret >= 0) {
    ...
    if (!t.is_special()) {
      LOGINFO(
          "Time: " << (double(t.total_microseconds())/1.0e6) << " second. Id: " << someId << ".");
    } else {
  ...
  if (aclient.timeout())
    throw aexecd_error("The command timed out, the process is killed.",
                       aexecd_error::Timeout);
  else
    throw aexecd_error(aclient.getErrMsg(), aexecd_error::Error);

}

DecoderBase::DecodeResult ExecDecoder::doRedirect(const std::string &kvdata,
                                                  std::string &msg) {
  string buf;
  string obstype;
...
std::string ExecDecoder::removeFile(const std::string &filename,
                                    bool doRemove) {
  if (!doRemove)
    return filename;
...
DecoderBase::DecodeResult ExecDecoder::execute(std::string &msg) {
  ...
  cmd << prog << "--" << decoder << " " << inputFile << " --kvdata "
      << kvdataFile << " --loglevel " << ll;
  LOGDEBUG(
      "Running decoder prog '" << cmd.str() << "'\nlogfile '" << logfile << "'\n"<< decoder << " '" << inputFile << "'\nkvdata '" << kvdataFile <<"'.");
  ...
  try {
    ...
    if (exitcode == 0 || exitcode == 1) {
      if (!miutil::file::pathExist(kvdataFile)) {
        LOGINFO(
            "Missing 'kvdata' file from decoder program, this is ok, no data to decode. (" << prog << ").");
        return Ok;
      } else if (miutil::file::fileSize(kvdataFile) == 0) {
        LOGINFO(
            "'kvdata' file from decoder program has length 0. This is ok, no data to decode. (" << prog << ").");
        return Ok;
      } else {
      ...
      LOGERROR(
          "The '" << decoder << "' failed exitcode '" << exitcode << "'. See logfile '" << logfile << "'.");
      msg = "DECODE ERROR.";
      return Error;
    }
  } catch (const miutil::file::file_error &ex) {
    LOGERROR(
        "Problems with the kvdata file from the decoder program." << "\nDecoder: " << prog << "\nkvdata: " << kvdataFile << "\nReason: " << ex.what());
