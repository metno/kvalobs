...
decoderFactory(dnmi::db::Connection &con, const ParamList &params,
  ...
  try {
    dec = new ExecDecoder(con, params, typeList, obsType, obs, decoderId);
  } catch (...) {
    return 0;
  }
...
std::list<std::string> getObsTypesExt(miutil::conf::ConfSection *theKvConf) {
  ...
  std::list<std::string> list;
  LOGDEBUG(
      "ExecDecoder::getObsTypesExt: theKvConf: " << (theKvConf?"NOT null":"NULL"));

  if (!theKvConf)
    return list;

  ConfSection *mySection = theKvConf->getSection(
      "kvDataInputd.ExecDecoder.decoders");
