...
class ExecDecoder : public DecoderBase {
  ...
  getConfKey(const std::string &key,
             const miutil::conf::ValElementList &defaultVal =
                 miutil::conf::ValElementList());
  miutil::conf::ValElementList
  ...
  void writeProgLog(const std::string &logfileToWrite,
  ...
  std::string getObsTypeParts() const {
    return obsTypePart_;
  }
  ...
  int runProg(const std::string &cmd, const std::string &logfile,
              const std::string &someId);
  DecoderBase::DecodeResult doRedirect(const std::string &kvdata,
                                       std::string &msg);

 public:
  ExecDecoder(dnmi::db::Connection &con, const ParamList &params,
