...
kvalobs::decoder::rejectdecoder::RejectDecoder::RejectDecoder(
...
std::string kvalobs::decoder::rejectdecoder::RejectDecoder::name() const {
  return "RejectDecoder";
}

bool kvalobs::decoder::rejectdecoder::RejectDecoder::doDecode(
    const std::string &message, kvalobs::kvRejectdecode &reject) {
  string sep("@$$reject$$@\n");
  string elems[3];
...
kvalobs::decoder::DecoderBase::DecodeResult kvalobs::decoder::rejectdecoder::RejectDecoder::execute(
  ...
  if (doDecode(obs, rejected)) {
    ...
    if (gate.insert(rejected)) {
      ...
      LOGWARN(
          "DBERROR: the message could not be inserted into the db!" << endl << "Reason: " << gate.getErrorStr() << endl);
    }
  }
