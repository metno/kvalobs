...
int synop::readToken(const char *token, int a, int b) {
  if (!b) {
    if (!a)
      return (
          *token == '/' || *token == 'X' || *token == 'x' ?
              undef : (*token - zero));
    else if (a >= strlen(token))
      return undef;
    else
      return (
          *(token + a) == '/' || *(token + a) == 'X' || *(token + a) == 'x' ?
              undef : (*(token + a) - zero));
  } else {
    if (b + a > strlen(token))
      return undef;
...
void synop::setExtraCloudHsToken(const int par, const char *token, int pos,
                                 int len) {
...
void synop::setExtraCloudClToken(const int par, const char *token, int pos,
                                 int len) {
  int val = readToken(token, pos, len);
...
void synop::setExtraCloudNsToken(const int par, const char *token, int pos,
                                 int len) {
  int val = readToken(token, pos, len);
...
int synop::checkCountry() {
  ...
  if (pinfo.callSign.length() >= 3) {
    ...
    if ((cs >= "JWA" && cs <= "JXZ") || (cs >= "LAA" && cs <= "LNZ")
        || (cs >= "3YA" && cs <= "3YZ"))
      return 1;
  }
...
void synop::setHorizontalVisibillity(const char* token) {
  ...
  if (ivv <= 50)
    vv = ivv * 100;  // 0  - 5  km 
  else if (ivv > 50 && ivv <= 55)
    vv = undef;             // not used 
  else if (ivv > 55 && ivv <= 80)
    vv = (ivv - 50) * 1000;  // 5  - 30 km
  else if (ivv > 80 && ivv < 90)
    vv = (ivv - 74) * 5000;  // 30 - 70 km
  else {
...
void synop::setCloudBaseHeight(const char* token) {
  ...
  if (ih == 0)
    ...
    setBuffer(h, -3);  // undef == -3 for klima-DB
