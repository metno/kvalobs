...
class SynopDecodeTest : public testing::Test {
 ...
 protected:
  ...
  virtual void SetUp() {
     ...
     *
     */
    stationList.push_back(
        kvalobs::kvStation(
            4460, 60.1173, 10.829, 170, 0, "Hakadal jernbanestasjon", 1488,
            4460, "", "", "", 8, true,
            boost::posix_time::time_from_string("2007-01-08 00:00:00")));

    stationList.push_back(
        kvalobs::kvStation(
            76900, 66, 2, 6, 0, "MIKE", 0, 76900, "", "LDWR", "", 7, true,
            boost::posix_time::time_from_string("1977-01-01 00:00:00")));

    stationList.push_back(
        kvalobs::kvStation(
            40745, 60.1173, 10.829, 170, 0, "Test1", 40745, 40745, "", "", "",
            8, true,
            boost::posix_time::time_from_string("2007-01-08 00:00:00")));
...
TEST_F( SynopDecodeTest, hVVN_missing ) {
  ...
  //h=/ VV=// N=/
  synopmsg =
      "AAXX 18061 01488 16/// /1701 10139 20115 60062 333 20128 70192 91106 555 0/402 10150=";
  EXPECT_TRUE( decode_h_VV_N( synopmsg, h, VV, N, rejectInfo ) ) << "Rejected: "
                                                                 << rejectInfo;
  EXPECT_TRUE( h==INT_MAX && VV == INT_MAX && N == INT_MAX )
      << "Expect: h=INT_MAX, VV=INT_MAX and N=INT_MAX. Got h=" << h << ", VV="
      << VV << " and N=" << N;

  //h=/ VV=// N=8
  synopmsg =
      "AAXX 18061 01488 16/// 81701 10139 20115 60062 333 20128 70192 91106 555 0/402 10150=";
  EXPECT_TRUE( decode_h_VV_N( synopmsg, h, VV, N, rejectInfo ) ) << "Rejected: "
                                                                 << rejectInfo;
  EXPECT_TRUE( h == INT_MAX && VV == INT_MAX && N == 8 )
      << "Expect: h=INT_MAX, VV=INT_MAX and N=8. Got h=" << h << ", VV=" << VV
      << " and N=" << N;

  //h=/ VV=56 N=/
  synopmsg =
      "AAXX 18061 01488 16/56 /1701 10139 20115 60062 333 20128 70192 91106 555 0/402 10150=";
  EXPECT_TRUE( decode_h_VV_N( synopmsg, h, VV, N, rejectInfo ) ) << "Rejected: "
                                                                 << rejectInfo;
  EXPECT_TRUE( h==INT_MAX && VV == 6000 && N == INT_MAX )
      << "Expect: h=INT_MAX, VV=6000 and N=INT_MAX. Got h=" << h << ", VV="
      << VV << " and N=" << N;

  //h=/ VV=56 N=8
  synopmsg =
      "AAXX 18061 01488 16/56 81701 10139 20115 60062 333 20128 70192 91106 555 0/402 10150=";
  EXPECT_TRUE( decode_h_VV_N( synopmsg, h, VV, N, rejectInfo ) ) << "Rejected: "
                                                                 << rejectInfo;
  EXPECT_TRUE( h==-3 && VV == 6000 && N == 8 )
      << "Expect: h=-3, VV=6000 and N=8. Got h=" << h << ", VV=" << VV
      << " and N=" << N;

  //h=3 VV=56 N=8
  synopmsg =
      "AAXX 18061 01488 16356 81701 10139 20115 60062 333 20128 70192 91106 555 0/402 10150=";
  EXPECT_TRUE( decode_h_VV_N( synopmsg, h, VV, N, rejectInfo ) ) << "Rejected: "
                                                                 << rejectInfo;
  EXPECT_TRUE( h==200 && VV == 6000 && N == 8 )
      << "Expect: h=200, VV=6000 and N=8. Got h=" << h << ", VV=" << VV
      << " and N=" << N;
}
...
TEST_F( SynopDecodeTest, decodeX ) {
  kvalobs::kvRejectdecode rejectInfo;
  float Nh, Cl;
  ParamVal param[] = { { 14, &Nh }, { 23, &Cl }, { 0, 0 } };

  string shipmsg =
      "BBXX LDWR 07221 99662 10018 41997 02718 10036 21036 40067 53095 70111 8xx//"
          " 22200 04064 11016 3//// 4//// 5//// 70082=";

  EXPECT_TRUE( decode2ParamVal( shipmsg, param, rejectInfo) )
      << "Failed to decode ship message.";
...
TEST_F( SynopDecodeTest, scaleDW1_DW2) {
  kvalobs::kvRejectdecode rejectInfo;
  float dw1, dw2;
  ParamVal param[] = { { 65, &dw1 }, { 66, &dw2 }, { 0, 0 } };

  string synopmsg =
      "AAXX 18061 01488 16/// /1701 10139 20115 60062 222XX 30112=";

  EXPECT_TRUE( decode2ParamVal( synopmsg, param, rejectInfo) )
      << "Failed to decode synop message.";
...
TEST_F( SynopDecodeTest, decode333_3Ejjj) {
  ...
  ParamVal param[] = { { 7, &E_ }, { 18, &SD }, { 112, &SA }, { 129, &E }, { 0,
      0 } };

  synopmsg = "AAXX 18061 40745 46/// ///// 333 31002=";

  EXPECT_TRUE( decode2ParamVal( synopmsg, param, rejectInfo) )
      << "Rejected: " << rejectInfo.comment();
  EXPECT_TRUE( E_==FLT_MAX && SD==FLT_MAX ) << "Failed (4E'sss): E'=" << E_
                                            << " SD=" << SD;
...
TEST_F( SynopDecodeTest, decode333_8NsChshs ) {
  ...
  float hshs;
  ParamVal param[] = { { syn::hshs1, &hshs }, { 0, 0 } };

  //Test decoding hshs to meter.
  EXPECT_TRUE( decode2ParamVal( synop, param, rejectInfo, true ) )
      << "Rejected: " << rejectInfo.comment();
  EXPECT_FLOAT_EQ(900, hshs);

  //Test decoding hshs to code.
  EXPECT_TRUE( decode2ParamVal( synop, param, rejectInfo, false ) )
      << "Rejected: " << rejectInfo.comment();
