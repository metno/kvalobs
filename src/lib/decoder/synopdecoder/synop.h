...
class synop {
 ...
 public:
  synop()
      : element(0),
        ...
        type(syn::SYNOP),
        section(syn::S000),
        tokenType(syn::NORMALTOKEN),
        sjj(0),
        extra333(false),
        hshsInMeter(false) {
  }

  synop(bool hshsInMeter_)
      : element(0),
        group(-1),
        clCounter(-1),
        type(syn::SYNOP),
  ...
  bool hasUnrecognised() {
    return !unrecognised.empty();
  }

  /// Output

  std::string Unrecognised() const {
    return unrecognised;
  }  ///< error buffer
  ...
  const syn::info* Info() const {
    return &pinfo;
  }  ///< extra information (see synopElements.h>

  int typeID();

  //SYNOP, SHIP or MOBIL
  syn::synopType getType() const {
    return type;
  }
 ...
 private:
  ...
  void setExtraCloudNsToken(const int par, const char *token, int pos = 0,
                            int len = 0);       ///< Convert to height in meter.
  ...
  void setTemperature(const char*, const int, const syn::signtype s = syn::sn);  ///< common temp func.
                                                                                 ///< returns +-xx.x
  ...
  void setPressure(const char*, const int);            ///< common press func.
                                                       ///< 0xxx = 10xx.x 
                                                       ///< 9xxx = 9xx.x
  void setWind(const int, const char*, int pos = 0, int len = 0);  ///< set wind and
                                                                   ///< transform kt 2 m/s
                                                                   ///< if necessary
