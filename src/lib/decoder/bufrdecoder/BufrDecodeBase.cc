...
bool
BufrDecodeBase::
doDecode( int descriptor, BufrValue &value, bool mustexist ) {
   ...
   if( descVal.unit == "CCITTIA5" ) {
      ...
      if( descVal.value == RVIND )
         ...
         value.sValue = bufrMessage->getStringValue( static_cast<int>( descVal.value ) );
...
bool
BufrDecodeBase::
nextDescriptor( int &descriptor, BufrValue &value )
{
   DescriptorFloatValue descVal;
   double val;
...
bool
BufrDecodeBase::
peekAt( int &descriptor, BufrValue &value,  int index )
{
   DescriptorFloatValue descVal;
   double val;
