...
class BufrDecodeKvResult : public BufrDecodeResultBase {
 ...
 public:
  ...
  void clear();
  void setKvStationList(KvStationList_ptr stationlist) {
    stationList_ = stationlist;
  }
  virtual void setObstime(const miutil::miTime &obstime) {
    obstime_ = obstime;
  }
  virtual miutil::miTime getObstime() const {
    return obstime_;
  }
  virtual void setStationid(int wmono);
  virtual void setLatLong(double latitude, double longitude) {
    latitude_ = latitude;
