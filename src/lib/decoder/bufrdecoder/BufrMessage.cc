...
bool BufrMessage::bufrExpand(const std::string &bufr,
  ...
  bufrex_(&length,
          reinterpret_cast<unsigned long int*>(const_cast<char*>(bufr.data())),
          bufrMessage.ksup, bufrMessage.ksec0, bufrMessage.ksec1,
          bufrMessage.ksec2, bufrMessage.ksec3, bufrMessage.ksec4,
          &bufrMessage.kelem, (char **) bufrMessage.cnames,
          (char **) bufrMessage.cunits, &bufrMessage.kvals, bufrMessage.vals,
          (char **) bufrMessage.cvals, &kerr);
  ...
  busel_(&bufrMessage.kdtlen, bufrMessage.kdtlist, &bufrMessage.kdtexplen,
         bufrMessage.kdtexplist, &kerr);
...
BufrMessage::nextFloatValueDescriptor(DescriptorFloatValue &descriptor) {
...
DescriptorFloatValue*
BufrMessage::peekAtFloatValueDescriptor(DescriptorFloatValue &descriptor,
                                        int index) {
  descriptor.descriptor = -1;
  descriptor.index = -1;
  descriptor.value = RVIND;
...
std::string BufrMessage::getStringValue(int charDescriptorValue) {
  ...
  cerr << " **** Index: " << index << " length: " << charDescriptorValue % 1000
       << " ksup[6]: " << ksup[6] << endl;
  if (index > ksup[6])
    return "";
...
int BufrMessage::descriptorTbl() const {
  if (!valid)
    return -1;
...
operator<<(std::ostream &out, const kvalobs::decoder::bufr::BufrMessage &msg) {
  ...
  if (msg.kdtexplen != msg.ksup[4])
    out << "**** WARNING: kdtexplen(" << msg.kdtexplen << ") != ksup[4] ("
        << msg.ksup[4] << ")" << endl;
  ...
  for (int i = 0; i < kdtexplen; ++i) {
    ...
    out << "   " << setw(3) << i << ": " << setw(1) << setfill('0') << F << " "
        << setw(2) << setfill('0') << X << " " << setw(3) << setfill('0') << Y
        << (N == prev ? '*' : ' ') << " : " << oval.str() << endl;
    prev = N;
  }

  out << "Character Values: " << endl;
  for (int i = 0; i < msg.ksup[6]; ++i) {
    out << "   " << setw(4) << setfill('0') << i << ": " << msg.cvals[i]
        << endl;  //"("<<cnames[i]<<" ["<<cunits[i]<<"])" << endl;
  }
...
operator<<(std::ostream &out, const DescriptorFloatValue &v) {
  ...
  out << setw(1) << setfill('0') << F << " " << setw(2) << setfill('0') << X
