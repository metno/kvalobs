...
public:
   ...
   :data_(elem.data_), textData_(elem.textData_), params(elem.params),
    sid( elem.sid ), tid( elem.tid ),
    dtObs( elem.dtObs ), tbtime( elem.tbtime )
   {}
   ...
   bool addDataObstime( const std::string &param,
                        const std::string &data,
   ...
   int textDataSize()const{ return textData_.size(); }
