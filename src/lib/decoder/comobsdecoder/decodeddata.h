...
class DecodedDataElem {
 ...
 public:
  ...
  bool findParam(const std::string &pname, long &paramid, bool &isText) const;

  std::string findParam(long paramid) const;

  bool addData(const std::string &param, const std::string &data,
               const kvUseInfo &uf = kvalobs::kvUseInfo(),
               const kvalobs::kvControlInfo &cf = kvalobs::kvControlInfo(),
               const int level = 0, const int sensor = 0);

  bool addDataObstime(const std::string &param, const std::string &data,
  ...
  const std::list<kvalobs::kvData> &data() const {
    return data_;
  }
  ...
  int textDataSize() const {
    return textData_.size();
  }

  miutil::miTime getDate() const {
    return dtObs;
  }
  void clean() {
    data_.clear();
