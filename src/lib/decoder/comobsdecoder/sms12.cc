...
const char *PARAM_IN[] = {
    ...
    "RRA", "CCA",
    "TP",  //??
    "PO", "PR", "AA", "PP", "_irix", "_RRRtr", "_Esss",
    "EV_24",  //??
    "TA", "TNT", "TAN_12", "TXT", "TAX_12", "TGN_12", "TW", "UU",
    "SG",  //Sj�gang
    "DD", "FF", "ITZ", "FX", "FG",
    "NN",   //Nddff
    "_hVV", "_wwW1W2", "_RtWdWdWd", "_NhClCmCh", "_NsChshs", "_VT_new1",
    "_VT_new2", "_VT_old1", "_VT_old2", "_knr", "RR", "PB", "UH", "signature",
    "TEXT", 0 };
}

kvalobs::decoder::comobsdecoder::Sms12::Sms12(
...
kvalobs::decoder::comobsdecoder::Sms12::decode(
  ...
  try {
    smsData = new kvalobs::decodeutil::DecodedData(paramList, stationid,
                                                   smscode);
  } catch (...) {
  ...
  for (SmsMelding::CIMeldingList it = obs.begin(); it != obs.end(); it++) {
    ...
    for (int i = 8; PARAM_IN[i]; i++) {
      ...
      if (param == "_irix" || param == "_RRRtr" || param == "RR"
          || param == "_knr" || param == "TEXT") {
        //Skip this. The precipitation is done in doPrecip.
        //The rest is of nao interest in kvalobs.
        continue;
      } else if (param == "PP") {
        if (buf.empty())
          continue;
        ...
        try {
          ...
          IDLOGERROR(logid, "Format error (DD): (" << buf << "). Not a number!");
          continue;
        }
        ...
        try {
          ...
          IDLOGERROR(logid, "FORMAT error FF, FG, or FX: (" << buf << ")!");
          continue;
        }
...
miutil::miTime kvalobs::decoder::comobsdecoder::Sms12::getObsTime(
    std::ostringstream &ost, miutil::miTime &klObs, bool &ccx) {
  string buf;
  int hour;
  miTime klComObs;
  miTime obstime;
  ...
  if (klObs >= fromTime && klObs <= toTime) {
    ost << "observationtime (KLOBS) not in valid time interval: " << klObs
        << "lower limit: " << fromTime << endl << "upper limit: " << toTime
        << endl;
    IDLOGERROR(logid, ost.str());
    return obstime;
  }
  ...
  if (obstime.undef()) {
    ost << "The observation time is not within �3 of the termin! " << endl
        << "Observationime: " << klObs << endl << "Termin: " << hour << endl;
    IDLOGERROR(logid, ost.str());
  }
...
bool kvalobs::decoder::comobsdecoder::Sms12::doPrecip(
    kvalobs::decodeutil::DecodedDataElem &data, int hour) {
  string RR;
  int iTr;
  string irix;
  string sIr;
