...
SmsBase::SmsBase(const ParamList &paramList_,
		 kvalobs::decoder::comobsdecoder::ComObsDecoder &decoder):
  paramList(paramList_),inData(0), inDataparam(0), ComObsDec(decoder)
{
}
...
kvalobs::decoder::comobsdecoder::
SmsBase::
initInData(const miutil::CommaString *indata,
	   const char                **inParams)
{
  inData=indata;
...
kvalobs::decoder::comobsdecoder::
SmsBase::
getInData(const char *param, std::string &val)
{
  val.erase();
...
std::string 
kvalobs::decoder::comobsdecoder::
SmsBase::createDHM(const std::string &dhm)
{
  int  year, mnt, d, h, m;
  char buf[100];
...
std::string 
kvalobs::decoder::comobsdecoder::
SmsBase::createFromDHM(const std::string &dhm)
{
  int  year, mnt, d, h, m;
  char buf[100];
...
miutil::miTime 
kvalobs::decoder::comobsdecoder::
SmsBase::createDTFromDHM(const std::string &dhm, const miutil::miTime &dt)
{
  int  year, mnt, d, h, m;
  char buf[100];
...
std::string 
kvalobs::decoder::comobsdecoder::
SmsBase::createFromMDh(const std::string &MDh)
{
...
miutil::miTime 
kvalobs::decoder::comobsdecoder::
SmsBase::createDTFromDH(const std::string &DH)
{
  int  year, mnt, d, h, m;
  char buf[100];
  ...
  if(retDT.undef()){
    ...
    LOGDEBUG("now:     " << dateNow << endl <<
	     "retDt:   " << retDT   <<endl <<
	     "hoursTo: " << miTime::hourDiff(retDT, dateNow) << endl);
  }
...
miutil::miTime 
kvalobs::decoder::comobsdecoder::
SmsBase::createDTFromYMDhm(const std::string &YDMhm)
{
  int  year, mnt, d, h, m;
  char buf[100];
  ...
  if( d<1 || d>31 || h<0 || h>23  || mnt<1 || mnt>12 || m<0 || m>61)
    return miTime();  //Null date
...
miutil::miTime 
kvalobs::decoder::comobsdecoder::
SmsBase::createDTFromYMDh(const std::string &YYMMDDhh)
{
  int  year, mnt, d, h;
  char buf[100];
...
std::string 
kvalobs::decoder::comobsdecoder::
SmsBase::createFromYMDh(const std::string &YYMMDDhh)
{
  miTime dt;
  char buf[30];
...
void
kvalobs::decoder::comobsdecoder::
SmsBase::stripNewLine(std::string &buf)
{
...
void
kvalobs::decoder::comobsdecoder::
SmsBase::cleanString(std::string &str,
...
char 
kvalobs::decoder::comobsdecoder::
SmsBase::separator(const std::string &buf, 
			 const std::string &validChars)
{
  std::string::size_type pos;
...
kvalobs::decoder::comobsdecoder::
SmsBase::
localTimeToUTC(const miutil::miTime &localTime_)
{
