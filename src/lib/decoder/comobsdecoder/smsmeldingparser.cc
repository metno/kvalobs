...
static void
SmsMeldingParser_startElement(void *user_data, const xmlChar *fullname,
                              const xmlChar **atts);
static void
SmsMeldingParser_endElement(void *user_data, const xmlChar *name);

static void
SmsMeldingParser_characters(void *user_data, const xmlChar *ch, int len);

static void
SmsMeldingParser_errorFunc(void *ctx, const char *msg, ...);

static void
SmsMeldingParser_warningFunc(void *ctx, const char *msg, ...);
static void
SmsMeldingParser_fatalErrorSAXFunc(void *ctx, const char *msg, ...);

#ifdef __cplusplus
}
#endif

static xmlSAXHandler SmsMeldingParser_xmlSaxHandler = { 0,  //    internalSubsetSAXFunc internalSubset;
...
bool SmsMeldingParser::checkNumber(const std::string &sNum, int &num) {
...
void SmsMeldingParser::startDocument() {
  //  cerr << "startDocument!\n";
}

void SmsMeldingParser::endDocument() {
  //cerr << "endDocument!\n";
}

void SmsMeldingParser::startElement(const xmlChar *fullname,
                                    const xmlChar **atts) {
...
void SmsMeldingParser::endElement(const xmlChar *name) {
...
void SmsMeldingParser::characters(const xmlChar *ch, int len) {
  //cerr << "characters: [";
...
void SmsMeldingParser::error(const std::string &msg) {

  errMsg << "xml error: " << msg << endl;
}

void SmsMeldingParser::warning(const std::string &msg) {
  errMsg << "xml warning: " << msg << endl;
}

void SmsMeldingParser::fatalError(const std::string &msg) {
...
SmsMeldingParser::parse(const std::string &melding) {
  ...
  errMsg.str("");
  state = START;
  ret = xmlSAXUserParseMemory(&SmsMeldingParser_xmlSaxHandler, this,
                              melding.c_str(), melding.length());
...
static void SmsMeldingParser_startDoc(void *user_data) {
  ((SmsMeldingParser*) user_data)->startDocument();
}

static void SmsMeldingParser_endDoc(void *user_data) {
  ((SmsMeldingParser*) user_data)->endDocument();
}

static void SmsMeldingParser_startElement(void *user_data,
                                          const xmlChar *fullname,
                                          const xmlChar **atts) {
  ((SmsMeldingParser*) user_data)->startElement(fullname, atts);
}

static void SmsMeldingParser_endElement(void *user_data, const xmlChar *name) {
  ((SmsMeldingParser*) user_data)->endElement(name);
}

static void SmsMeldingParser_characters(void *user_data, const xmlChar *ch,
                                        int len) {
  ((SmsMeldingParser*) user_data)->characters(ch, len);
}

static void SmsMeldingParser_errorFunc(void *ctx, const char *msg, ...) {
  va_list args;
  char errBuf[256];
  char *buf;
  int n;
...
static void SmsMeldingParser_warningFunc(void *ctx, const char *msg, ...) {
  va_list args;
  char errBuf[256];
  char *buf;
  int n;
...
static void SmsMeldingParser_fatalErrorSAXFunc(void *ctx, const char *msg,
                                               ...) {
  va_list args;
  char errBuf[256];
  char *buf;
