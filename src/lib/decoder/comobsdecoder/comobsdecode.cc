...
kvalobs::decoder::comobsdecoder::ComObsDecoder::ComObsDecoder(
...
std::string kvalobs::decoder::comobsdecoder::ComObsDecoder::name() const {
  return "ComObsDecoder";
}

kvalobs::decoder::comobsdecoder::SmsBase*
kvalobs::decoder::comobsdecoder::ComObsDecoder::smsfactory(int smscode) {
  try {
    if (smscode == 2)
...
long kvalobs::decoder::comobsdecoder::ComObsDecoder::getStationid(
    long obsid, bool isWmono) {
  ostringstream ost;
  string key;
  string val;
...
std::string kvalobs::decoder::comobsdecoder::ComObsDecoder::getMetaSaSdEm(
...
kvalobs::decoder::DecoderBase::DecodeResult kvalobs::decoder::comobsdecoder::ComObsDecoder::execute(
    std::string &msg) {
  ...
  if (!smsMelding) {
    kvRejectdecode myrejected = kvRejectdecode(
        obs, boost::posix_time::microsec_clock::universal_time(),
        "comobs/typeid=<UNKNOWN>", mParser.getErrMsg());

    putRejectdecodeInDb(myrejected);

    LOGWARN(
        "Decoder: " << name() <<". Rejected!" << endl << mParser.getErrMsg() << endl);
    return Rejected;
  }
  ...
  if (smsMelding->getClimano() > 0) {
    ...
    kvRejectdecode myrejected = kvRejectdecode(
        obs, boost::posix_time::microsec_clock::universal_time(), ost.str(),
        "No identification!");
  ...
  if (!decoder) {
    ...
    kvRejectdecode myrejected = kvRejectdecode(
        obs, boost::posix_time::microsec_clock::universal_time(), ost1.str(),
        ost.str());
  ...
  data = decoder->decode(stationid, smsMelding->getCode(),
                         smsMelding->getMeldingList(), msg, rejected,
                         hasRejected);
  ...
  for (CITDecodedDataElem it = theData->begin(); it != theData->end(); it++) {
