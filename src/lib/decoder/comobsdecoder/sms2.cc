...
namespace {
    ...
    const char *PARAM_IN[]={"stnr", 
		       ...
		       "SA", 
		       "SD", 
		       "KLSTART", //Kommer fra stasjonen, lokaltid.
...
std::string
kvalobs::decoder::comobsdecoder::
Sms2::createObsTime(const std::string &YYMMDDhhmm, 
			  miutil::miTime &obsTime )
{
  	...
  	if(mnt>12 || mnt<1 || d<1 || d>31 || h<0 || h>23 || m<0 || m>59)
    	return std::string();  //Null date
...
miutil::miTime
kvalobs::decoder::comobsdecoder::
Sms2::createObsTime(const std::string &YYYYMMDDhh,
			        std::string &sYYYYMMDDhh)
{
   ...
   if(YYYYMMDDhh.length()==8){
    	...
    	if(year<(now.year()-1) || year>(now.year()+1) ||
       		mnt<1 || mnt>12 || d<1 || d>31 || h<0 || h>23)
      		return miTime();
...
kvalobs::decoder::comobsdecoder::
Sms2::
onlyZeros(const std::string &val)
{
...
kvalobs::decoder::comobsdecoder::
Sms2::
decodeV1V2V3(const std::string &w_,
	     std::string  &v1,
	     std::string  &v2,
	     std::string  &v3,
  	...
  	for(SmsMelding::CIMeldingList it=obs.begin();
    	...
    	if(getInData("P3", buf)){
      		...
      		if(!buf.empty()){
				...
				if(decodeV1V2V3(buf, v1, v2, v3, vi, sstrm)>0){
	  				...
	  				if(!v1.empty())
	     				data.addData("V4S", vi.substr(0,1));

	   				if(!v2.empty())
	     				data.addData("V5S", vi.substr(1, 1));
	   
	   				if(!v3.empty())
	     				data.addData("V6S", vi.substr(2, 1));
				...
				if(!buf.empty()){
	  				LOGDEBUG1("decode2kv: V�rtegn: P3: " +buf);
				}
      		}
    	}
    	...
    	if(getInData("P2", buf)){
    		//P2 12-18
      		...
      		if(!buf.empty()){
				...
				if(decodeV1V2V3(buf, v1, v2, v3, vi, sstrm)>0){
	  				data.setDate(miTime(dtObs.date(), miClock(18, 0, 0))); 
	  				data.addData("V4", v1);
	   				data.addData("V5", v2);
	   				data.addData("V6", v3);
    	...
    	if(getInData("P1", buf)){
      		...
      		if(!buf.empty()){
				...
				if(decodeV1V2V3(buf, v1, v2, v3, vi, sstrm)>0){
	  				data.setDate(miTime(dtObs.date(), miClock(12, 0, 0)));
	  				data.addData("V4", v1);
	  				data.addData("V5", v2);
	  				data.addData("V6", v3);
...
kvalobs::decoder::comobsdecoder::
Sms2::
doKvPrecipitation(kvalobs::decodeutil::DecodedDataElem &data, 
				  ...
				  std::ostream &ost)
{
	...
	kvUseInfo useInfo;
  	...
  	if(klObs.empty() && klStart.empty()){
    	...
    	sprintf(myBuf, "%04d%02d%02d%02d",
	    		dtObs.year(),dtObs.month(),dtObs.day(),
	    		dtObs.hour());
    	klObs=myBuf;
