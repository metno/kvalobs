...
class SmsMelding {
 ...
 private:
  ...
  void formatLogMsg(std::string &str, const std::string &timestamp);
 ...
 public:
  ...
  int getSynopno() const {
    return synopno;
  }
  int getClimano() const {
    return climano;
  }
  int getCode() const {
    return code;
  ...
  bool hasClimanoOrSynopno() const {
    return climano > 0 || synopno > 0;
  }
  const MeldingList& getMeldingList() const {
    return meldingList;
  }

  bool isValid() const {
    return ((synopno > 0 || climano > 0) && code > -1 && meldingList.size() > 0);
  }

  std::stringstream& getErrStream() {
    return errStream;
  }
  EResultCodes getResCode() const {
    return resCode;
  }
  void setResCode(EResultCodes c) {
    resCode = c;
  }

  static std::string createReturnXML(SmsMelding *msg);
  static std::string createReturnXML(EResultCodes resCode,
                                     const std::string &comment);

  void setRawDoc(const std::string &doc) {
    rawDoc = doc;
  }
  std::string getRawDoc() const {
    return rawDoc;
  }
