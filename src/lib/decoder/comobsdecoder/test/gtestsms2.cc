...
TEST_F( Sms2DecodeTest, MultiObsTime ) {
  ...
  DecodedDataElem elem = getDataSet(data, miTime("2010-12-28 06:00:00"));
  EXPECT_TRUE( elem.dataSize() == 9 ) << " DataSet: size = " << elem.dataSize();
  EXPECT_TRUE( elem.getDate() == miTime("2010-12-28 06:00:00") ) << "Date: "
      << elem.getDate();
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-28 06:00:00"), 34), 2 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 34);
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-28 06:00:00"), 35), 0 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 35);
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-28 06:00:00"), 110), 0.1 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 110);
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-28 06:00:00"), 112), 49 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 112);

  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-27 12:00:00"), 34), 2 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 34);
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-27 12:00:00"), 35), 0 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 35);

  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-27 18:00:00"), 34), 2 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 34);
  EXPECT_FLOAT_EQ( getData( elem, miTime("2010-12-27 18:00:00"), 35), 0 )
      << "Val: " << getData(elem, miTime("2010-12-28 06:00:00"), 35);
}
...
Sms2DecodeTest::runtest(const ParamList &paramList,
  ...
  sprintf(buf, "sms_%05d_%03d_%4d%02d%02dT%02d%02d.txt", melding.getClimano(),
          melding.getCode(), nowTime.year(), nowTime.month(), nowTime.day(),
  ...
  fakeComobsDecoder = new FakeComobsDecoder(*dummyCon, paramList, typesList, "",
                                            "", 1);
  ...
  data = sms.decode(melding.getClimano(), melding.getCode(),
                    melding.getMeldingList(), returnMsg, rejected, hasRejected);
  ...
  if (!data) {
    ...
    if (!elem) {
      ...
      for (kvalobs::decodeutil::TDecodedDataElem::iterator eit = elem->begin();
          eit != elem->end(); ++eit) {
        for (std::list<kvalobs::kvData>::const_iterator dit =
            eit->data().begin(); dit != eit->data().end(); dit++) {
          of << *dit << endl;
          //   cerr << *dit  << endl;
        }
      }
...
Sms2DecodeTest::runtestOnFile(const ParamList &paramList,
  ...
  data = sms.decode(smsmsg->getClimano(), smsmsg->getCode(),
                    smsmsg->getMeldingList(), returnMsg, rejected, hasRejected);
