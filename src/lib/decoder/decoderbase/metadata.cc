...
static void
MetaParser_startElement(void *user_data, const xmlChar *fullname,
                        const xmlChar **atts);
static void
MetaParser_endElement(void *user_data, const xmlChar *name);

static void
MetaParser_characters(void *user_data, const xmlChar *ch, int len);

static void
MetaParser_errorFunc(void *ctx, const char *msg, ...);

static void
MetaParser_warningFunc(void *ctx, const char *msg, ...);
static void
MetaParser_fatalErrorSAXFunc(void *ctx, const char *msg, ...);

static xmlSAXHandler MetaParser_xmlSaxHandler = { 0,  //    internalSubsetSAXFunc internalSubset;
...
class MetaParserImpl {
  ...
  typedef enum {
    ...
    IN_META,
    IN_META_PARAMS,
    IN_META_PARAMS_PARAM,
    IN_META_PARAMS_PARAM_VALUE,
    ERROR,
    END
  } States;
 ...
 public:
  ...
  MetaParserImpl(MetaParser *parser_, Meta *meta_)
      : state(START),
        ...
        meta(meta_),
        parser(parser_),
        errMsg(parser_->errMsg),
        warnMsg(parser_->warnMsg) {
  }

  void startDocument();
  void endDocument();
  void startElement(const xmlChar *fullname, const xmlChar **atts);
...
void miutil::parsers::meta::Values::checkRange(int index) const {
  if (index >= value.size()) {
    ostringstream ost;
...
void miutil::parsers::meta::Values::addValue(const std::string &value_) {
  value.push_back(value_);
}

std::string miutil::parsers::meta::Values::getValue(int index) const {
  checkRange(index);

  return value.at(index);
}

int miutil::parsers::meta::Values::getValueAsInt(int index) const {
  checkRange(index);
...
long miutil::parsers::meta::Values::getValueAsLong(int index) const {
  checkRange(index);
...
float miutil::parsers::meta::Values::getValueAsFloat(int index) const {
  checkRange(index);
...
double miutil::parsers::meta::Values::getValueAsDouble(int index) const {
  checkRange(index);
...
std::ostream&
miutil::parsers::meta::operator<<(std::ostream& out,
...
void miutil::parsers::meta::Meta::addParamValue(const std::string &param,
                                                const std::string &value) {
  params[param].addValue(value);
}

miutil::parsers::meta::Meta::Meta() {
}

miutil::parsers::meta::Meta::~Meta() {
}

miutil::parsers::meta::Values miutil::parsers::meta::Meta::getParamValue(
    const std::string &param, const std::string &defval) const {
  Values::Value values;
...
miutil::parsers::meta::Meta::Params miutil::parsers::meta::Meta::getParams() const {
  Params par;
...
miutil::parsers::meta::Meta::operator=(const miutil::parsers::meta::Meta &rhs) {
  if (&rhs != this) {
    params = rhs.params;
  }
...
miutil::parsers::meta::operator<<(std::ostream& out,
  ...
  for (Meta::CIParamMap it = meta.params.begin(); it != meta.params.end(); it++)
    out << "  " << it->first << ": " << it->second << endl;

  return out;
}

void miutil::parsers::meta::MetaParserImpl::startDocument() {
  //  cerr << "startDocument!\n";
}

void miutil::parsers::meta::MetaParserImpl::endDocument() {
  //cerr << "endDocument!\n";
}

void miutil::parsers::meta::MetaParserImpl::startElement(
  ...
  if (strcmp((const char*) fullname, "meta") == 0) {
    ...
    if (state != IN_META_PARAMS) {
      errMsg << "TAG: <" << fullname
             << ">, but not in state <IN_META_PARAMS>\n";
      errFlag = true;
      return;
    }
    ...
    if (state != IN_META_PARAMS_PARAM) {
      errMsg << "TAG: " << fullname
             << ", but not in state <IN_META_PARAMS_PARAM>\n";
      errFlag = true;
      return;
    }
...
void miutil::parsers::meta::MetaParserImpl::endElement(const xmlChar *name) {
  ...
  if (strcmp((const char*) name, "meta") == 0) {
    ...
    if (state != IN_META_PARAMS_PARAM_VALUE) {
      errMsg
          << "ENDTAG; <value>, but not in state <IN_META_PARAMS_PARAM_VALUE>\n";
      errFlag = true;
      return;
    }
...
void miutil::parsers::meta::MetaParserImpl::characters(const xmlChar *ch,
                                                       int len) {
  //cerr << "characters: [";
...
void miutil::parsers::meta::MetaParserImpl::error(const std::string &msg) {

  errMsg << "xml error: " << msg << endl;
}

void miutil::parsers::meta::MetaParserImpl::warning(const std::string &msg) {
  errMsg << "xml warning: " << msg << endl;
}

void miutil::parsers::meta::MetaParserImpl::fatalError(const std::string &msg) {
  errMsg << "xml fatal error: " << msg << endl;
}

miutil::parsers::meta::MetaParser::MetaParser() {
}
...
miutil::parsers::meta::Meta*
miutil::parsers::meta::MetaParser::parse(const std::string &melding) {
  int ret;
  ...
  errMsg.str("");
  ret = xmlSAXUserParseMemory(&MetaParser_xmlSaxHandler, impl, melding.c_str(),
                              melding.length());
...
static void MetaParser_startDoc(void *user_data) {
  ((miutil::parsers::meta::MetaParserImpl*) user_data)->startDocument();
}

static void MetaParser_endDoc(void *user_data) {
  ((miutil::parsers::meta::MetaParserImpl*) user_data)->endDocument();
}

static void MetaParser_startElement(void *user_data, const xmlChar *fullname,
                                    const xmlChar **atts) {
  ((miutil::parsers::meta::MetaParserImpl*) user_data)->startElement(fullname,
                                                                     atts);
}

static void MetaParser_endElement(void *user_data, const xmlChar *name) {
  ((miutil::parsers::meta::MetaParserImpl*) user_data)->endElement(name);
}

static void MetaParser_characters(void *user_data, const xmlChar *ch, int len) {
  ((miutil::parsers::meta::MetaParserImpl*) user_data)->characters(ch, len);
}

static void MetaParser_errorFunc(void *ctx, const char *msg, ...) {
  va_list args;
  char errBuf[256];
  char *buf;
  int n;
...
static void MetaParser_warningFunc(void *ctx, const char *msg, ...) {
  va_list args;
  char errBuf[256];
  char *buf;
  int n;
...
static void MetaParser_fatalErrorSAXFunc(void *ctx, const char *msg, ...) {
  va_list args;
  char errBuf[256];
  char *buf;
  int n;
