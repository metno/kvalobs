...
AutoObsDecoder::
rejected( const kvalobs::kvRejectdecode &rejectEntry,
...
execute(std::string &msg)
{
    ...
    if( stationid == 0 ){
        ...
        kvalobs::kvRejectdecode reject(ost.str(),
                tbtime,
                name(),
                msg);
    ...
    if (!getline(istr, tmp)) {
        ...
        kvalobs::kvRejectdecode reject(obs,
                tbtime,
                name(),
                "Invalid format."
    ...
    while(getline(istr, tmp)){
        ...
        if( obstime.is_special() ) {
            ...
            logmsg << "stationid: " << stationid << " typeid: " << typeId << ". Invalid obstime '" << data[0] << "'."
                    << " line: " << line;
            return rejected( reject, logmsg.str(), logid,logmsg.str() + "\nObservation:\n" + obs );
        }
