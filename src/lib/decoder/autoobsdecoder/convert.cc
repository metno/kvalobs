...
//text_data table.
DataElem::DataElem(int id, const std::string &val, int sensorno, int height,
                   int mod)
    : id_(id),
      val_(val),
      sensorno_(sensorno),
...
bool DataElem::fVal(float &f) const {
  try {
...
bool DataConvert::SaSdEmEi::dataSa(kvData &data, const SaSdEmEi &sa,
  ...
  data = kvData(saSdEmTemplate.stationID(), saSdEmTemplate.obstime(),
                ...
                saSdEmTemplate.typeID(), saSdEmTemplate.sensor(),
                saSdEmTemplate.level(), v /*corected*/,
                saSdEmTemplate.controlinfo(), saSdEmTemplate.useinfo(),
...
bool DataConvert::SaSdEmEi::dataSd(kvData &data, const SaSdEmEi &sd,
                                   const kvData &saSdEmTemplate) {
  if (sd.sd.empty())
    return false;
...
bool DataConvert::SaSdEmEi::dataEm(kvData &data, const SaSdEmEi &em,
                                   const kvData &saSdEmTemplate) {
  if (em.em.empty())
    return false;
...
bool DataConvert::SaSdEmEi::dataEi(kvData &data, const SaSdEmEi &ei,
                                   const kvData &saSdEmTemplate) {
  if (ei.ei.empty())
    return false;
...
void DataConvert::setSaSdEmEi(const std::string &sa_sd_em_ei) {
  if (sa_sd_em_ei.length() != 4) {
    IDLOGDEBUG(
        logid,
        "DataConvert::setSaSdEm: sa_sd_em_ei.length()!=3: (" << sa_sd_em_ei.length() << ")");
    return;
  }
...
bool DataConvert::allSlash(const std::string &val) {
  return allCh(val, '/');
}

bool DataConvert::allCh(const std::string &val, char ch) {
...
std::vector<DataElem> DataConvert::convert(
    const std::string &param_, const std::string &val,
 ...
 *
 */
bool DataConvert::decodeParam(const std::string &param, std::string &name,
                              int &sensor, int &height, int &mod) {
  string par(param);
  string::size_type i;
  string tmp;
...
bool DataConvert::decodeSpDef(std::vector<DataElem> &data, SplitDef *spDef,
  ...
  for (int i = 0; spDef[i].id; i++) {
      ...
      if (spVal.length() >= 3) {
        if (isdigit(spVal[0]) && isdigit(spVal[1]) && isdigit(spVal[2])) {
          if (spVal[0] == '9' && spVal[1] == '9') {
            ...
            IDLOGDEBUG(
                logid,
                "-- _RRR (" << spVal << ") : " << RRRtr_.RRR << " rt: " << RRRtr_.rt);
          } else if (spVal == "000") {
            RRRtr_.RRR = -1;
          } else {
    ...
    } else if (id == "V1" || id == "V2" || id == "V3" || id == "V4"
        || id == "V5" || id == "V6" || id == "V7") {
...
void DataConvert::decodeVal(std::vector<DataElem> &data,
                            const boost::posix_time::ptime &obsTime,
                            const std::string &param, const std::string &val_,
                            int sensor, int height, int mod) {
  IParamList it;
  std::string name;
  ...
  if (name == "RR_1" || name == "RR_2" || name == "RR_3" || name == "RR_6"
      || name == "RR_9" || name == "RR_12" || name == "RR_15" || name == "RR_18"
      || name == "RR_24" || name == "RR_X") {
    RRRtr_.RR_N = true;
...
boost::posix_time::ptime DataConvert::convertToMiTimeFromHHMM(
    const boost::posix_time::ptime &obst_, const std::string &val_) {
  ostringstream ost;
  string val(val_);
  char buf[100];
...
float DataConvert::RRRtr::RR(int &paramid,
                             const boost::posix_time::ptime &obstime) {
  float rr;
  int RRR_;
  ...
  if (RRR == INT_MAX && tr < 0) {
    if (ir == 3) {
      if (obstime.time_of_day().hours() == 6
          || obstime.time_of_day().hours() == 18) {
        paramid = 109;  //RR_12
        tr = 2;
        return -1.0;
      } else if (obstime.time_of_day().hours() == 0
          || obstime.time_of_day().hours() == 12) {
        paramid = 108;
        tr = 1;
        return -1.0;
      }
...
bool DataConvert::hasSaSdEmEi(SaSdEmEi &saSdEmEi) {
  string sa;

  IDLOGDEBUG(
      logid,
      "hasSaSdEmEi: hasSa=" << (hasSa?"t":"f") << " hasSd=" << (hasSd?"t":"f") << " hasEm=" << (hasEm?"t":"f") << " hasEi=" << (hasEi?"t":"f")<< endl << "saSdEmEi_.hasSa=" << (saSdEm_.hasSa?"t":"f") << " saSdEmEi_.hasSd=" << (saSdEm_.hasSd?"t":"f") << " saSdEmEi_.hasEm=" << (saSdEm_.hasEm?"t":"f") << " saSdEmEi_.hasEi=" << (saSdEm_.hasEi?"t":"f") << endl << "saSdEmEi_.sa=" << saSdEm_.sa << " saSdEmEi_.sd=" << saSdEm_.sd << " saSdEmEi_.em=" << saSdEm_.em << " saSdEmEi_.ei=" << saSdEm_.ei);

  if (!hasSa && !hasSd && !hasEm && !hasEi && !saSdEm_.hasSa && !saSdEm_.hasSd
      && !saSdEm_.hasEm && !saSdEm_.hasEi)
    return false;
  ...
  if (hasSa || saSdEm_.hasSa) {
    if ((saSdEm_.sa.empty() && hasSa && saSdEm_.hasSa) || saSdEm_.sa == "998"
        || saSdEm_.sa == "000")
      sa = "-1";
