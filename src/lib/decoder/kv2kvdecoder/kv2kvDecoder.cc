...
kv2kvDecoder::kv2kvDecoder( dnmi::db::Connection & con, const ParamList & params,
                            const list<kvTypes> & typeList, const miutil::miString & obsType,
                            const miutil::miString & obs, int decoderId )
...
DecoderBase::DecodeResult kv2kvDecoder::handleError_( const kv2kvDecoder::DecoderError & e, miutil::miString & msg )
{
  msg = e.what();
  ostringstream ss;
...
// void kv2kvDecoder::save( const KvalobsData & data )
// {
//   // kvTextData:
//   TDList tdlist;
//   data.getData( tdlist, tbtime );
//   if ( not tdlist.empty() ) {
//     if ( ! putkvTextDataInDb( tdlist, priority_ ) ) {
//       for ( TDList::const_iterator it = tdlist.begin(); it != tdlist.end(); ++ it ) {
//         ostringstream ss;
//         ss  << "update text_data set original=\'" << it->original() << '\''
//             << " where stationid=" << it->stationID()
//             << " and obstime=\'" << it->obstime() << '\''
...
//   // kvData:
//   DList dlist;
//   data.getData( dlist, tbtime );
//   if ( not dlist.empty() ) {
//     if ( ! putKvDataInDb( dlist, priority_ ) ) {
//       for ( DList::const_iterator it = dlist.begin(); it != dlist.end(); ++ it ) {
//         deleteKvDataFromDb( * it );
//       }
//       if ( ! putKvDataInDb( dlist, priority_ ) )
//         throw DecoderError( decoder::DecoderBase::Error, "Could not save data" );
...
struct lt_kvTextData_without_paramID : public binary_function<kvTextData, kvTextData, bool>
{
  bool operator()( const kvTextData & a, const kvTextData & b ) const
  {
    if ( a.stationID() != b.stationID() )
...
void kv2kvDecoder::invalidatePreviousData( KvalobsData & data, const list<KvalobsData::InvalidateSpec> & inv )
{
  list<kvData> tmp;
  ...
  for ( list<KvalobsData::InvalidateSpec>::const_iterator invIt = inv.begin(); invIt != inv.end(); ++ invIt ) {
    DList alreadyInDb;//sentInData;
    if ( ! dbGate.select( alreadyInDb, kvQueries::selectDataFromType( invIt->station, invIt->typeID, invIt->obstime ) ) )
      throw DecoderError( decoder::DecoderBase::Error, "Could not get data from database." );
    for ( DList::const_iterator db = alreadyInDb.begin(); db != alreadyInDb.end(); ++ db ) {
      if ( compare::eq_sensor( 0, db->sensor() ) and db->level() == 0 ) {
        kvDataSet::const_iterator find = sentInData.find( * db );
        if ( find == sentInData.end() ) {
          if ( data.overwrite() ) {
            kvDataFactory f( db->stationID(), db->obstime(), db->typeID(), db->sensor(), db->level() );
...
void kv2kvDecoder::invalidatePreviousTextData( KvalobsData & data, const list<KvalobsData::InvalidateSpec> & inv )
{
  list<kvTextData> tmp;
  data.getData( tmp, tbtime );
  ...
  for ( list<KvalobsData::InvalidateSpec>::const_iterator invIt = inv.begin(); invIt != inv.end(); ++ invIt ) {
    TDList alreadyInDb;
    if ( ! dbGate.select( alreadyInDb, kvQueries::selectTextData( invIt->station, invIt->obstime, invIt->obstime ) ) )
      throw DecoderError( decoder::DecoderBase::Error, "Could not get data from database." );
    for ( TDList::const_iterator db = alreadyInDb.begin(); db != alreadyInDb.end(); ++ db ) {
      if ( db->typeID() == invIt->typeID ) {
        kvDataSet::const_iterator find = sentInData.find( * db );
        if ( find == sentInData.end() ) {
          if ( data.overwrite() ) {
            kvTextData td( db->stationID(), db->obstime(), "", db->paramID(), tbtime, db->typeID() );
            data.insert( td );
          }
          else {
            throw DecoderError( decoder::DecoderBase::Rejected, "Illegal operation: Cannot overwrite text_data" );
          }
        }
      }
    }
...
void kv2kvDecoder::verify( const kvData & d, kvDataPtr dbData ) const
{
  ...
  if ( dbData.get() and abs( dbData->original() - d.original() ) > delta ) {
    ...
    << "Station: " << d.stationID() << endl
    << "Values: DB = " << dbData->original() << ". New = " << d.original();
...
void kv2kvDecoder::adapt( kvData & d, kvDataPtr dbData, bool overwrite ) const
{
  ...
  if ( dbData.get() and not overwrite )
    d.set( d.stationID(), d.obstime(),
           dbData->original(),
           d.paramID(), dbData->tbtime(),
