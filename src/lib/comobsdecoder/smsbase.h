...
namespace kvalobs{
  namespace decoder{
	...
	miutil::miTime nowTime_;  //For testing onlu
	...
	void initInData(const miutil::CommaString *indata,
			const char                **inParams);
	 ...
	 */
	void setNowTime(const miutil::miTime &t){ nowTime_=t;}

	/**
	 * \brief Return the wallclock if nowTime_ is undef.
	 *
	...
	miutil::miClock nowClock()const
	  { return nowTime_.undef()?
	      miutil::miClock::oclock():nowTime_.clock();}
	...
	miutil::miDate  nowDate()const
	  { return nowTime_.undef()?
	      miutil::miDate::today():nowTime_.date();}
	...
	miutil::miTime  nowTime()const
	  {return nowTime_.undef()?
	      miutil::miTime::nowTime():nowTime_;}
	 ...
	 */
	miutil::miTime  
	  createDTFromDHM(const std::string &dhm, 
			 const miutil::miTime &dt=miutil::miTime());
	 ...
	 */
	char separator(const std::string &buf, 
