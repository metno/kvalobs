...
/**
 * ProducerThread is a helper class to run a producer in its own
 * thread. The communication with the thread is through the message queue
 * 'queue'.
 *
...
public:
  ...
  void start(kvalobs::subscribe::Producer *producer);
  void shutdown();
