...
std::string getValue(const std::string &key, miutil::conf::ConfSection *conf, const std::string &defaultValue="") {
...
int getIntValue(const std::string &key, miutil::conf::ConfSection *conf, int defaultValue) {
  miutil::conf::ValElementList ret = conf->getValue(key);
  if (ret.empty()) {
    return defaultValue;
  }
...
int pgConsumerPollSize(miutil::conf::ConfSection *conf, const std::string &progname, int defaultValue) {
  if (!progname.empty()) {
    auto v=getIntValue("pgqueue." + progname + ".consumer_poll_size", conf, -1);
    if (v != -1) {
      return v;
    }
  }
...
PgConfig PgConfig::config(const miutil::conf::ConfSection *conf, const std::string &progname, int defaultConsumerPollSize){
  PgConfig cfg;
  cfg.pollSize= pgConsumerPollSize(const_cast<miutil::conf::ConfSection *>(conf), progname, defaultConsumerPollSize);  
  cfg.connections= pgConnections(const_cast<miutil::conf::ConfSection *>(conf));
