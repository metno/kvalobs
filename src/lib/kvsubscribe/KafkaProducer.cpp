...
public:
  ...
  virtual void dr_cb(RdKafka::Message &message) {
    std::unique_ptr<MessageId> id(
        static_cast<MessageId *>(message.msg_opaque()));
...
KafkaProducer::KafkaProducer(const KafkaConfig &config,
                             ErrorHandler onFailedDelivery,
                             SuccessHandler onSuccessfulDelivery):Producer(config.topic, onFailedDelivery, onSuccessfulDelivery) {
...
void KafkaProducer::init(const KafkaConfig &config,
  ...
  topic__.reset(RdKafka::Topic::create(producer_.get(), config.topic,
                                       tconf.get(), errstr));
  if (!topic__)
    throw std::runtime_error("Failed to create topic: " + errstr);
}

KafkaProducer::~KafkaProducer() { catchup(); }

MessageId KafkaProducer::send(const std::string &data) {
  return send(data.c_str(), data.size());
}

MessageId KafkaProducer::send(const char *data, unsigned length) {
  MessageId *id = new MessageId(messageId_++);

  RdKafka::ErrorCode resp = producer_->produce(
      topic__.get(), RdKafka::Topic::PARTITION_UA,
