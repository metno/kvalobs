...
class ConfSectionPimpel {
 public:
  ...
  ConfSectionPimpel(bool allowMultipleSections_, int lineno_ = 0,
...
miutil::conf::ConfSection::ConfSection() {
...
miutil::conf::ConfSection::ConfSection(bool allowMultipleSections_,
                                       const std::string &filename_,
                                       int lineno_) {
  pimpel[this] = new ConfSectionPimpel(allowMultipleSections_, lineno_,
                                       filename_);
...
bool miutil::conf::ConfSection::ignoreThisSection() const {
  std::map<const ConfSection*, ConfSectionPimpel*>::iterator pit;
...
void miutil::conf::ConfSection::ignoreThisSection(bool f) {
  std::map<const ConfSection*, ConfSectionPimpel*>::iterator pit;
...
int miutil::conf::ConfSection::getLineno() const {
  std::map<const ConfSection*, ConfSectionPimpel*>::iterator pit;
...
std::string miutil::conf::ConfSection::getFilename() const {
  std::map<const ConfSection*, ConfSectionPimpel*>::iterator pit;
...
bool miutil::conf::ConfSection::allowMultipleSections() const {
  std::map<const ConfSection*, ConfSectionPimpel*>::iterator pit;
...
bool miutil::conf::ConfSection::addSection(const std::string &name,
  ...
  if (it != sectionList.end()) {
    if (replaceIfExist) {
      ...
      std::cerr << "addSection: allowMultipleSections: "
...
bool miutil::conf::ConfSection::addValue(const std::string &key,
...
miutil::conf::ConfSection::getSection(const std::string &cs_) {
  string cs(cs_);
  string sectionName;
...
miutil::conf::ConfSectionList miutil::conf::ConfSection::getAllSection(
    const std::string &cf_) {
  string cs(cf_);
  string sectionName;
...
miutil::conf::ValElementList miutil::conf::ConfSection::getValue(
...
miutil::conf::ValElementList miutil::conf::ConfSection::getValueRecursivt(
    const std::string &startInSection, const std::string &key,
    std::string &foundKeyInSection, const ValElementList &defValue) const {
...
std::list<string> miutil::conf::ConfSection::getKeys() const {
  std::list<std::string> list;

  for (CIValueList it = valueList.begin(); it != valueList.end(); it++)
    list.push_back(it->first);

  return list;
}

std::list<string> miutil::conf::ConfSection::getSubSections() const {
  std::list<std::string> list;

  for (CISectionList it = sectionList.begin(); it != sectionList.end(); it++)
    list.push_back(it->first);

  return list;
}

void miutil::conf::ConfSection::deleteAllIgnoredSections() {
...
void miutil::conf::ConfSection::printImple(std::ostream &ost, int nSpace,
  ...
  for (itVal = valueList.begin(); itVal != valueList.end(); itVal++) {
...
void miutil::conf::ConfSection::print(std::ostream &ost, bool pritty) const {
