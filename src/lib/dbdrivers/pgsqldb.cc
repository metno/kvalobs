...
bool dnmi::db::drivers::PGConnection::isConnected() {
  if (con == 0)
    return false;
...
bool dnmi::db::drivers::PGConnection::tryReconnect() {
  if (!con)
    return false;
...
void dnmi::db::drivers::PGConnection::beginTransaction() {
  exec("BEGIN");
}

void dnmi::db::drivers::PGConnection::endTransaction() {
  exec("END");
}

void dnmi::db::drivers::PGConnection::rollBack() {
  exec("ROLLBACK");
}

void dnmi::db::drivers::PGConnection::exec(const std::string &query) {
  PGresult *p = 0;
...
std::string dnmi::db::drivers::PGConnection::lastError() const {
  char *msg;
...
dnmi::db::drivers::PGResult::PGResult(PGresult *r)
    : Result(PQnfields(r)) {
  res = r;
...
bool dnmi::db::drivers::PGResult::hasResult() const {
  return res != 0;
}

int dnmi::db::drivers::PGResult::fields() const {
  return nFields;
}

std::string dnmi::db::drivers::PGResult::fieldName(int index) const {
  if (index >= nFields)
...
int dnmi::db::drivers::PGResult::fieldSize(const std::string &fieldName) const {
  return fieldSize(fieldIndex(fieldName));
}

int dnmi::db::drivers::PGResult::size() const {
  return PQntuples(res);
}

bool dnmi::db::drivers::PGResult::hasNext() const {
...
void dnmi::db::drivers::PGResult::nextImpl() {
  if (tupleIndex >= nTuples)
...
void dnmi::db::drivers::PGPimpel::createSavepoint(const std::string &name) {
  con->exec("SAVEPOINT " + name);
}

void dnmi::db::drivers::PGPimpel::rollbackToSavepoint(const std::string &name) {
  con->exec("ROLLBACK TO SAVEPOINT " + name);
}

void dnmi::db::drivers::PGPimpel::releaseSavepoint(const std::string &name) {
...
dnmi::db::drivers::PGPimpel::PGPimpel(PGConnection *con_)
    : dnmi::db::priv::Pimpel(),
      con(con_) {
}
