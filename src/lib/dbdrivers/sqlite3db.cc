...
bool dnmi::db::drivers::SQLiteConnection::isConnected() {
  if (!pimpel) {
...
bool dnmi::db::drivers::SQLiteConnection::tryReconnect() {
  if (!pimpel) {
    errMsg = "SQLite: Driver internal error (no pimpel)!";
    return false;
  }
...
void dnmi::db::drivers::SQLiteConnection::beginTransaction() {
  if (!pimpel) {
    errMsg = "SQLite: Driver internal error (no pimpel)!";
...
void dnmi::db::drivers::SQLiteConnection::endTransaction() {
  exec("COMMIT");
}

void dnmi::db::drivers::SQLiteConnection::rollBack() {
  exec("ROLLBACK");
}

void dnmi::db::drivers::SQLiteConnection::exec(const std::string &query) {
  int sqliteRes;
  char *msg = 0;
  bool busy = false;
...
std::string dnmi::db::drivers::SQLiteConnection::lastError() const {
  return errMsg;
}
...
bool dnmi::db::drivers::SQLiteResult::hasResult() const {
  return sqlData->data.size() > 0;
}

int dnmi::db::drivers::SQLiteResult::fields() const {
  return sqlData->fieldNames.size();
}

std::string dnmi::db::drivers::SQLiteResult::fieldName(int index) const {
...
int dnmi::db::drivers::SQLiteResult::fieldSize(int index) const {
  if (index >= sqlData->fieldNames.size())
    throw SQLException("index out of range!");
...
int dnmi::db::drivers::SQLiteResult::size() const {
  return sqlData->data.size();
}

bool dnmi::db::drivers::SQLiteResult::hasNext() const {
...
void dnmi::db::drivers::SQLiteResult::nextImpl() {
  if (nextData == sqlData->data.end())
...
dnmi::db::drivers::SQLitePimpel::SQLitePimpel(SQLiteConnection *con_)
    : dnmi::db::priv::Pimpel(),
      con(con_) {
}

std::string dnmi::db::drivers::SQLitePimpel::getConnect() const {
  return connect;
}
...
std::string dnmi::db::drivers::SQLitePimpel::getErrorCode() const {
  return errorCode;
}

void dnmi::db::drivers::SQLitePimpel::setErrorCode(int errCode) {
  if (errCode < 0) {
...
std::string dnmi::db::drivers::SQLitePimpel::getErrMsg() const {
  return errMsg;
}

void dnmi::db::drivers::SQLitePimpel::setErrMsg(const char *errmsg) {
  if (!errmsg)
    errMsg = "SQLite: UNKNOWN error!";
  else
    errMsg = errmsg;
...
void dnmi::db::drivers::SQLitePimpel::createSavepoint(const std::string &name) {
