/*
 ...
 $Id: sqlitedb.cc,v 1.5.6.4 2007/09/27 09:02:26 paule Exp $                                                       
...
dnmi::db::drivers::SQLiteConnection::SQLiteConnection(
    ...
    : Connection(driverId),
      con(0) {
  char *msg = 0;
  con = sqlite_open(connect.c_str(), 0, &msg);

  if (con)
    sqlite_busy_timeout(con, 5000);  //sets busy timeout to 5 second.
...
dnmi::db::drivers::SQLiteConnection::~SQLiteConnection() {
  if (con) {
    std::cerr << "Disconnect from a SQLite database!\n";
    sqlite_close(con);
    ;
  }
}
...
void dnmi::db::drivers::SQLiteConnection::exec(const std::string &query) {
  ...
  sqliteRes = sqlite_exec(con, query.c_str(), 0, 0, &msg);
...
dnmi::db::drivers::SQLiteConnection::execQuery(const std::string &query) {
  ...
  if (!con)
    throw SQLNotConnected(
        "SQLite: NO CONNECTION, not connected to any database!");
  ...
  sqliteRes = sqlite_exec(con, query.c_str(), dataCallback, data, &msg);
...
std::string dnmi::db::drivers::SQLiteConnection::esc(
  ...
  char *buf = sqlite_mprintf("%q", stringToEscape.c_str());
