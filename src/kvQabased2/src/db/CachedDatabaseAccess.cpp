...
CachedDatabaseAccess::CachedDatabaseAccess(DatabaseAccess * baseImplementation, const kvalobs::kvStationInfo & obs) :
...
{
	std::map<std::string, int>::const_iterator flagPosition = qcxFlagPositions_.find(qcx);
...
kvalobs::kvAlgorithms CachedDatabaseAccess::getAlgorithm(const std::string & algorithmName) const
{
	boost::shared_ptr<kvalobs::kvAlgorithms> & algorithm = algorithms_[algorithmName];
	if ( ! algorithm )
		algorithm = boost::shared_ptr<kvalobs::kvAlgorithms>(new kvalobs::kvAlgorithms(FilteredDatabaseAccess::getAlgorithm(algorithmName)));
	return * algorithm;
}

std::string CachedDatabaseAccess::getStationParam(const kvalobs::kvStationInfo & si, const std::string & parameter, const std::string & qcx) const
{
	if ( si != lastStationParamQuery_.si or parameter != lastStationParamQuery_.parameter or qcx != lastStationParamQuery_.qcx )
	{
		lastStationParamQuery_.result = FilteredDatabaseAccess::getStationParam(si, parameter, qcx);
...
void CachedDatabaseAccess::getData(DataList * out, const kvalobs::kvStationInfo & si, const qabase::DataRequirement::Parameter & parameter, int minuteOffset) const
{
...
void CachedDatabaseAccess::getTextData(TextDataList * out, const kvalobs::kvStationInfo & si, const qabase::DataRequirement::Parameter & parameter, int minuteOffset) const
{
