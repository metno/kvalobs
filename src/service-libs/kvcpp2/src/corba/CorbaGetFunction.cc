...
*/
#include "../../include/corba/CorbaGetFunction.h"
#include "../../include/corba/CorbaKvApp.h"
...
namespace kvservice
{
  namespace corba
  {
	...
	: decodeInfo( decodeInfo ), it( it )
      ...
      {
	it.cleanup();
	return service->getRejectdecode( decodeInfo, 
	...
	return ok;
      ...
      getKvModelDataFunc::getKvModelDataFunc( list<kvModelData> &dataList, const WhichDataHelper &wd )
	: dataList( dataList )
	, wd( wd )
      { 
      }
	...
	return ok;
      ...
      getKvReferenceStationsFunc::getKvReferenceStationsFunc( int stationid, int paramid, list<kvReferenceStation> &refList )
	  : stationid( stationid )
	  , paramid( paramid )
	  , refList( refList )
      {
	...
	if ( ok ) {
	  ...
	  for ( CORBA::ULong i = 0; i < stp->length(); i++ ) {
	    kvStationParam param( (*stp)[i].stationid,
				  (*stp)[i].paramid,
				  (*stp)[i].level,
	...
	: terminate( terminate )
      ...
      DataIterator_ptr 
      getKvDataFunc_abstract::getDataIter( const WhichDataHelper &wd )
      {
	DataIterator_ptr it;
