...
KvDataHandler::KvDataHandler(dnmi::db::Connection & connection1, dnmi::db::Connection & connection2, KvGetDataReceiver & dataReceiver) :
...
public:
	...
	QueryResult(std::shared_ptr<dnmi::db::Result> result) : result_(result) {}
...
{
	std::future<QueryResult<kvalobs::kvData>> dataFetch = std::async(
		std::launch::async,
		[this, &wd](){
	...
	std::future<QueryResult<kvalobs::kvTextData>> textDataFetch = std::async(
		std::launch::deferred,
		[this, &wd](){
	...
	while ( true )
	{
		...
		int dataStationId = d.empty() ? std::numeric_limits<int>::max() : d.front().stationID();
		int textDataStationId = td.empty() ? std::numeric_limits<int>::max() : td.front().stationID();
...
std::string KvDataHandler::query_(const WhichDataHelper & wd, const std::string & table)
{
