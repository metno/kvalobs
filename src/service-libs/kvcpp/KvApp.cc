...
std::string KvApp::getConsumerGroupId(const std::string &consumerGroupIdKey) {
  ...
  if( val.empty() ){
    idkey="";
    val = kvalobs::subscribe::createConsumerGroupId(progname);
  }

  if(idkey.empty()) {
    LOGINFO("Autogenerated consumer group id: '" << val << "'")
  } else {
    LOGINFO("Using consumer group id: '" << val << "' from config key: '" << idkey <<"'");
  }
  return val;
}
