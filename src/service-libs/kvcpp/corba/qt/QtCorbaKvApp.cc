...
namespace qt {
namespace {
class GotDataEvent : public QEvent {
  CommandBase * cmdBase;
 public:
...
QtCorbaKvApp::QtCorbaKvApp(int &argc, char **argv, bool guiApp,
                           conf::ConfSection *conf, const char *options[][2])
...
QtCorbaKvApp::SubscriberID QtCorbaKvApp::subscribeDataNotify(
    const KvDataSubscribeInfoHelper &info, const QObject *receiver,
    const char *member) {
  SubscriberID ret = subscribeDataNotify(info, signalQueue);
  if ((not ret.empty()) and receiver and member) {
    if (!connect(this, SIGNAL(kvDataNotify(kvservice::KvWhatListPtr)), receiver,
                 member)) {
      unsubscribe(ret);
      return "";
    }
...
QtCorbaKvApp::SubscriberID QtCorbaKvApp::subscribeData(
    const KvDataSubscribeInfoHelper &info, const QObject *receiver,
    const char *member) {
  SubscriberID ret = subscribeData(info, signalQueue);
  if ((not ret.empty()) and receiver and member) {
    if (!connect(this, SIGNAL(kvData(kvservice::KvObsDataListPtr)), receiver,
                 member)) {
      unsubscribe(ret);
      return "";
    }
...
QtCorbaKvApp::SubscriberID QtCorbaKvApp::subscribeKvHint(
