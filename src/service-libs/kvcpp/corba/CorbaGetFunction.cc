...
bool CorbaGetFunction::operator()(const char *name) {
  ...
  try {
    ...
    LOGERROR(
        "Can't lookup CORBA nameserver" << endl << "Reason: " << ex.what() << endl);
    return false;
  } catch (...) {
    LOGERROR("Can't lookup CORBA nameserver" << "Reason:  unknown!");
    return false;
  }
...
getKvRejectDecodeFunc::getKvRejectDecodeFunc(const RejectDecodeInfo &decodeInfo,
...
getKvWorkstatisticFunc::getKvWorkstatisticFunc(
    const CKvalObs::CService::WorkstatistikTimeType timeType_,
    const boost::posix_time::ptime &from_, const boost::posix_time::ptime &to_,
...
bool getKvWorkstatisticFunc::process(
    CKvalObs::CService::kvServiceExt_ptr service_) {
  ...
  return service->getWorkstatistik(timeType, to_kvalobs_string(from).c_str(),
                                   to_kvalobs_string(to).c_str(),
                                   it.getCorbaObjPtr().out());
}
;
...
bool getKvParamsFunc::process(kvServiceExt_ptr service) {
  ...
  if (ok) {
    ...
    for (CORBA::ULong i = 0; i < params->length(); i++) {
      paramList.push_back(
          kvParam((*params)[i].paramID, std::string((*params)[i].name),
                  std::string((*params)[i].description),
    ...
    for (CORBA::ULong i = 0; i < stations->length(); i++)
      stationList.push_back(
          kvStation(
              ...
              (*stations)[i].height,
              (*stations)[i].maxspeed,
              std::string((*stations)[i].name),
              (*stations)[i].wmonr,
              (*stations)[i].nationalnr,
              std::string((*stations)[i].ICAOid),
              std::string((*stations)[i].call_sign),
              std::string((*stations)[i].stationstr),
              (*stations)[i].environmentid,
              (*stations)[i].static_,
              boost::posix_time::time_from_string_nothrow(
                  std::string((*stations)[i].fromtime))));
  }
  if (stations)
    delete stations;
      ...
      for (CORBA::ULong i = 0; i < data->length(); i++) {
        for (CORBA::ULong k = 0; k < data[i].dataList.length(); k++) {
          kvalobs::kvModelData myData(
              ...
              boost::posix_time::time_from_string_nothrow(
                  (const char *) data[i].dataList[k].obstime),
              data[i].dataList[k].paramID, data[i].dataList[k].level,
              data[i].dataList[k].modelID, data[i].dataList[k].original);
  ...
  if (ok) {
    ...
    for (CORBA::ULong i = 0; i < types->length(); i++) {
      kvTypes type((*types)[i].typeID_, std::string((*types)[i].format),
                   (*types)[i].earlyobs, (*types)[i].lateobs,
                   std::string((*types)[i].read),
                   std::string((*types)[i].obspgm),
  ...
  if (ok) {
    ...
    for (CORBA::ULong i = 0; i < stp->length(); i++) {
      kvStationParam param(
          ...
          (*stp)[i].today,
          (*stp)[i].hour,
          std::string((*stp)[i].qcx),
          std::string((*stp)[i].metadata),
          std::string((*stp)[i].desc_metadata),
          boost::posix_time::time_from_string_nothrow(
              std::string((*stp)[i].fromtime)));
...
getKvStationMetaDataFunc::getKvStationMetaDataFunc(
    list<kvStationMetadata> &stParam, int stationid,
...
bool getKvStationMetaDataFunc::process(kvServiceExt_ptr serviceext) {
  ...
  bool ok = serviceext->getStationMetaData(metadata, stationid,
                                           myObstime.c_str(),
                                           metadataName_.c_str());
...
//			sensorPtr = sensor_== kvDbBase::INT_NULL?0:&sensor_;
      ...
      kvStationMetadata meta(
          (*metadata)[i].stationid,
          paramPtr,
          typePtr,
          levelPtr,
          sensorPtr,
          ...
          boost::posix_time::time_from_string_nothrow(
              std::string((*metadata)[i].fromtime)),
          (strlen((*metadata)[i].totime) == 0 ?
              boost::posix_time::ptime() :
              boost::posix_time::time_from_string_nothrow(
                  std::string((*metadata)[i].totime))));
  ...
  if (ok) {
    ...
    for (CORBA::ULong i = 0; i < obspgm->length(); i++) {
      kvObsPgm obsp(
          ...
          // UNDEF:
          boost::posix_time::time_from_string_nothrow(
              (const char *) (*obspgm)[i].fromtime),
          boost::posix_time::time_from_string_nothrow(
              (const char *) (*obspgm)[i].totime));
...
DataIterator_ptr getKvDataFunc_abstract::getDataIter(
    const WhichDataHelper &wd) {
  DataIterator_ptr it;
  ...
  try {
    ...
    LOGINFO("Can't lookup CORBA nameserver" "Reason: " << ex.what() << endl);
    return DataIterator::_nil();
  } catch (...) {
    LOGINFO("Can't lookup CORBA nameserver" << "Reason: Unkown!");
    return DataIterator::_nil();
  }
  return it;
}
...
getKvDataFunc::getKvDataFunc(KvGetDataReceiver &dataReceiver,
    ...
    : getKvDataFunc_abstract(terminate),
