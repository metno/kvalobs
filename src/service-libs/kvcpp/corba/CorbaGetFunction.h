...
namespace kvservice
{
  ...
  namespace corba
  {
	...
	kvservice::RejectDecodeIterator &it;
      public:
	getKvRejectDecodeFunc( const CKvalObs::CService::RejectDecodeInfo &decodeInfo, 
			       kvservice::RejectDecodeIterator &it );
      protected:
	...
	std::list<kvalobs::kvReferenceStation> &refList;
      public:
	getKvReferenceStationsFunc( int stationid, int paramid, 
				    std::list<kvalobs::kvReferenceStation> &refList );
      protected:
	virtual bool process( CKvalObs::CService::kvService_ptr service );
      };
	...
	getKvDataFunc_abstract( termfunc terminate );
      protected:
	virtual CKvalObs::CService::DataIterator_ptr getDataIter( const WhichDataHelper &wd );
	termfunc terminate;
      };
	...
	getKvDataFunc( KvGetDataReceiver &dataReceiver, 
		       const WhichDataHelper &wd,
		       termfunc terminate );
      protected:
	virtual bool process( CKvalObs::CService::kvService_ptr service );
