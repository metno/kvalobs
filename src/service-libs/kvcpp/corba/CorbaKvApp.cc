...
namespace kvservice {
namespace corba {
CorbaKvApp::CorbaKvApp(int &argc, char **argv, miutil::conf::ConfSection *conf,
                       const char *options[][2])
    : CorbaHelper::CorbaApp(argc, argv, options),
      corbaThread( NULL),
      refService(kvServiceExt::_nil()),
      shutdown_(false) {
  ...
  if (nameserver.empty()) {
    LOGFATAL(
        "The key <corba.nameserver> in the configurationfile has no value!");
    exit(1);
  }
  ...
  LOGINFO(
      "Using kvalobs in path <" << nameserver << "::" << nameserverpath << ">!");
...
conf::ConfSection* CorbaKvApp::readConf(const string &fname) {
  ...
  if (!fis) {
    ...
    if (!conf) {
      LOGERROR(
          "Error while reading configuration file: <" << fname << ">!" << endl << parser.getError());
    } else {
...
miutil::conf::ConfSection* CorbaKvApp::readConf(
    std::initializer_list<std::string> fileNameAlternatives) {
...
kvServiceExt_ptr CorbaKvApp::lookUpService(bool forceNS, bool & usedNS) {
  CORBA::Object_var obj;
  CKvalObs::CService::kvServiceExt_ptr ptr;
  ...
  while (true) {
    if (forceNS) {
      ...
      if (CORBA::is_nil(obj))
        throw LookUpException(
            "EXCEPTION: Can't obtain a reference for 'kvService'\n           from the CORBA nameserver!");

      ptr = CKvalObs::CService::kvServiceExt::_narrow(obj);

      if (CORBA::is_nil(ptr))
        throw LookUpException(
            "EXCEPTION: Can't narrow reference for 'kvService'!");

      refService = CKvalObs::CService::kvServiceExt::_duplicate(ptr);
    ...
    if (CORBA::is_nil(refService))
      ...
      return CKvalObs::CService::kvServiceExt::_duplicate(refService);
  }
}
...
bool CorbaKvApp::getKvWorkstatistik(
  ...
  getKvWorkstatisticFunc func(timeType, from, to, it);
  return func("getKvWorkstatistik");
}

bool CorbaKvApp::connectToKvInput(bool reConnect) {

  if (reConnect || CORBA::is_nil(dataInput)) {

    LOGDEBUG("Looking up CORBA Data object");

    dataInput = CKvalObs::CDataSource::Data::_narrow(
        this->getObjFromNS(nameserverpath + "kvinput"));
...
const Result_var CorbaKvApp::sendDataToKv(const char *data,
                                          const char *obsType) {
...
bool CorbaKvApp::getKvData(KvGetDataReceiver &dataReceiver,
                           const WhichDataHelper &wd) {
  getKvDataFunc func(dataReceiver, wd, terminateFunc);
  return func("getKvData");
}

bool CorbaKvApp::getKvData(KvObsDataList &dataList, const WhichDataHelper &wd) {
  getKvDataFunc_deprecated func(dataList, wd, terminateFunc);
  return func("getKvData(deprecatedversion)");
}

bool CorbaKvApp::addDataNotifySubscriber(DataNotifySubscriber *ptr,
...
bool CorbaKvApp::addDataSubscriber(DataSubscriber *ptr,
                                   const std::string &subid) {
  Lock lock(mutex);
...
bool CorbaKvApp::addHintSubscriber(HintSubscriber *ptr,
                                   const std::string &subid) {
  Lock lock(mutex);
...
string CorbaKvApp::subscribeDataNotify(const KvDataSubscribeInfoHelper &info,
  ...
  try {
    ...
    LOGINFO(
        "WARNING: KvQtCorbaApp::subscribeDataNotify: hmmm, very strange, a unkown exception!\n");
  }
...
string CorbaKvApp::subscribeData(const KvDataSubscribeInfoHelper &info,
                                 dnmi::thread::CommandQue &que) {
  DataSubscriber *sub;
  ...
  try {
    for (int i = 0; i < 2; i++) {
      ...
      try {
        ret = service->subscribeData(*info.getDataSubscribeInfo(), refSub);
        if (strlen(static_cast<char*>(ret)) == 0) {
...
void CorbaKvApp::unsubscribe(const string &id) {
...
void CorbaKvApp::unsubscribeAll() {
  ...
  try {
    for (int i = 0; i < 2; i++) {
      ...
      try {
        ...
        LOGINFO(
            "WARNING:(KvQtCorbaApp::unsubscribe) Exception CORBA::TRANSIENT!\n");
      } catch (CORBA::COMM_FAILURE &ex) {
        LOGINFO(
            "WARNING:(KvQtCorbaApp::unsubscribe) Exception CORBA::COMM_FAILURE!\n");
      } catch (...) {
