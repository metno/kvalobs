...
kvservice::priv::DataSubscriber::DataSubscriber(dnmi::thread::CommandQue &que_)
    : que(que_) {
}

kvservice::priv::DataSubscriber::~DataSubscriber() {
}

#include <sys/time.h>

void kvservice::priv::DataSubscriber::callback(
    const CKvalObs::CService::ObsDataList& data) {
  LOGDEBUG("Callback");
