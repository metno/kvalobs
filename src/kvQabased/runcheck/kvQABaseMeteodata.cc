...
kvQABaseMeteodata::kvQABaseMeteodata(kvQABaseDBConnection & dbcon,
	...
	dbcon_(dbcon), stationinfo_(stationinfo), saveCache_(dbcon)
{
}
 ...
 */
bool kvQABaseMeteodata::loadObsData(const int sid, const int tstart,
		const int tstop)
{
	...
	// add new data
	for (std::list<kvalobs::kvData>::const_iterator it = dlist.begin(); it
			!= dlist.end(); ++it)
	{
 ...
 */
bool kvQABaseMeteodata::loadTextData(const int sid, const int tstart,
		const int tstop)
{
 ...
 */
bool kvQABaseMeteodata::loadModelData(const int sid, const int tstart,
		const int tstop)
{
		...
		const kvObsPgmList & oplist) // obs_pgm
{
	...
	if (obs_vars.pars.empty() && refobs_vars.pars.empty()
...
std::string kvQABaseMeteodata::perlScript(const kvQABaseScriptManager& sman) const
{
	...
	ret << "my @obstime=(" << obstime.year() << "," << obstime.month() << ","
			<< obstime.day() << "," << obstime.hour() << "," << obstime.min()
			<< "," << obstime.sec() << ");\n\n";
...
void kvQABaseMeteodata::fillObsVariables(kvQABase::script_var & vars)
{
	...
	for (vector<int>::iterator pp = vars.allpos.begin(); pp != vars.allpos.end(); pp++)
	{
		...
		if (obsdata[*pp].empty())
		{
			...
			throw std::runtime_error(
					"Unable to find observations for station/timestep");
		}
		...
		for (TDataFromTime::iterator ttp = textdata[*pp].begin(); ttp
				!= textdata[*pp].end(); ttp++)
		{
			...
			for (int j = 0; j < n; j++)
			{
				...
				for (i = 0; i < np; i++)
				{
					/* check for:
					 ...
					 ( -typeID == typeID )
					 */
					if (ttp->second[i].paramID() == vid && ((tid == -32767
							&& abs(ttp->second[i].typeID()) == abs(
									stationinfo_.typeID())) || abs(
							ttp->second[i].typeID()) == abs(tid)))
					{
	...
	for (TimeOffsets::reverse_iterator rt = alltimes.rbegin(); rt
			!= alltimes.rend(); rt++)
		vars.alltimes.push_back(*rt); // add timestep to list
}

void kvQABaseMeteodata::fillModelVariables(kvQABase::script_var& vars)
{
	...
	for (; pp != vars.allpos.end(); pp++)
	{
		...
		if (modeldata[*pp].size() == 0)
		{
			IDLOGERROR( "html", "no MODELDATA found for station:" << *pp
					...
					<< endl );
			throw std::runtime_error(
					"Unable to find model data for station/timestep");
		}
...
void kvQABaseMeteodata::setFlag(ScriptReturnType::const_iterator param,
...
void kvQABaseMeteodata::setMissing(ScriptReturnType::const_iterator param,
		...
		ScriptReturnType::const_iterator pp, const std::string & type)
{
	...
	if (tidx >= obs_vars.alltimes.size())
		throw out_of_range("Timeindex from script out of bounds:"
				+ boost::lexical_cast<string>(tidx));
	if (sidx >= obs_vars.allpos.size())
		throw out_of_range("Stationindex from script out of bounds:"
				+ boost::lexical_cast<string>(tidx));
	...
	if (obsdata.count(station) == 0)
		throw runtime_error("Obsdata not found for station:"
				+ boost::lexical_cast<string>(station));
	...
	for (ipar = 0; ipar < np; ++ipar)
	{
		...
		if (paramid == d.paramID() && sensor == d.sensor() && level
				== d.level() && typeID == d.typeID())
			break;
	}
	if (ipar == np)
	{
...
{
	for (ObsKeys::const_iterator itr = updated_obs.begin(); itr
			!= updated_obs.end(); itr++)
	{
		// ouch!
		kvalobs::kvData
				newdata =
						obsdata[itr->second.stationID][itr->second.time][itr->second.paridx];
