...
void addObsDataToScript(scriptrunner::Script & script, const DataStore::ParameterSortedDataList & obsdata)
{
	...
	for ( DataStore::ParameterSortedDataList::const_iterator param = obsdata.begin(); param != obsdata.end(); ++ param )
	{
		...
		for ( DataStore::DataList::const_iterator it = param->second.begin(); it != param->second.end(); ++ it )
		{
		...
		missingCount += std::count_if(missing.begin(), missing.end(), have_missing_value);

		input.add(param->first.baseName(), value.begin(), value.end());
		input.add(param->first.baseName() + "_missing", missing.begin(), missing.end());
		input.add(param->first.baseName() + "_controlinfo", controlinfo.begin(), controlinfo.end());
	}
...
void addRefObsDataToScript(scriptrunner::Script & script, const DataStore::ParameterSortedRefDataList & refobsdata)
{
	if ( not refobsdata.empty() )
	{
		...
		{
			std::vector<float> missing;
			for ( DataStore::RefDataList::const_iterator it = param->second.begin(); it != param->second.end(); ++ it )
			...
			input.adds(param->first.baseName(), textDataStrings.begin(), textDataStrings.end());
			input.add(param->first.baseName() + "_missing", missing.begin(), missing.end());
		}
...
void addModelDataToScript(scriptrunner::Script & script, const DataStore::ParameterSortedModelDataList & modeldata)
{
	if ( not modeldata.empty() )
	{
		...
		{
			std::vector<float> missing;
			for ( DataStore::ModelDataList::const_iterator it = param->second.begin(); it != param->second.end(); ++ it )
			...
			input.add(param->first.baseName(), modelDataValues.begin(), modelDataValues.end());
			input.add(param->first.baseName() + "_missing", missing.begin(), missing.end());
		}
...
void addMetaDataToScript(scriptrunner::Script & script, const DataStore::ParameterSortedMetaDataList & metadata)
{
	if ( not metadata.empty() )
	{
		scriptrunner::ScriptInput input("meta");

		for ( DataStore::ParameterSortedMetaDataList::const_iterator meta = metadata.begin(); meta != metadata.end(); ++ meta )
			input.add(meta->first.baseName(), meta->second.begin(), meta->second.end());
