...
void fillMissing(ParameterSortedList & dataList)
{
	...
	for ( typename ParameterSortedList::iterator parameter = dataList.begin(); parameter != dataList.end(); ++ parameter )
	{
...
//	to.addMin(concreteObsRequirement.lastTime());
	...
	ParameterTranslation translation = getTranslation(concreteObsRequirement, abstractObsRequirement);

	for ( DataRequirement::ParameterList::const_iterator parameter = concreteObsRequirement.parameter().begin(); parameter != concreteObsRequirement.parameter().end(); ++ parameter )
	{
		ParameterTranslation::const_iterator find = translation.find(* parameter);
		if ( find == translation.end() )
			throw UnableToGetData("Unable to find translation for parameter: " + parameter->baseName());

		db::DatabaseAccess::DataList dbdata;
		db.getData(& dbdata, observation, * parameter, concreteObsRequirement.firstTime());
...
{
	ParameterTranslation translation = getTranslation(concreteRefObsRequirement, abstractRefObsRequirement);

	for ( DataRequirement::ParameterList::const_iterator parameter = concreteRefObsRequirement.parameter().begin(); parameter != concreteRefObsRequirement.parameter().end(); ++ parameter )
	{
		ParameterTranslation::const_iterator find = translation.find(* parameter);
		if ( find == translation.end() )
			throw UnableToGetData("Unable to find translation for parameter: " + parameter->baseName());
		...
		for (db::DatabaseAccess::TextDataList::const_iterator it = textData.begin(); it != textData.end(); ++ it )
...
//	to.addMin(concreteModelRequirement.lastTime());
	...
	ParameterTranslation translation = getTranslation(concreteModelRequirement, abstractModelRequirement);

	for ( DataRequirement::ParameterList::const_iterator parameter = concreteModelRequirement.parameter().begin(); parameter != concreteModelRequirement.parameter().end(); ++ parameter )
	{
		ParameterTranslation::const_iterator find = translation.find(* parameter);
		if ( find == translation.end() )
			throw UnableToGetData("Unable to find translation for parameter: " + parameter->baseName());
		...
		db.getModelData(& modelData, observation, * parameter, concreteModelRequirement.firstTime());
...
{
	ParameterTranslation translation = getTranslation(concreteMetaRequirement, abstractMetaRequirement);

	for ( qabase::DataRequirement::ParameterList::const_iterator parameter = concreteMetaRequirement.parameter().begin(); parameter != concreteMetaRequirement.parameter().end(); ++ parameter )
	{
		ParameterTranslation::const_iterator find = translation.find(* parameter);
		if ( find == translation.end() )
			throw UnableToGetData("Unable to find translation for metadata parameter: " + parameter->str());
		...
		qcx_(qcx)
{
	...
	if ( abstractObsRequirement or concreteObsRequirement )
	{
		...
		populateObs_(db, observation, * abstractObsRequirement, * concreteObsRequirement);
	}

	const DataRequirement * abstractRefObsRequirement = abstractSignature.refobs();
	const DataRequirement * concreteRefObsRequirement = concreteSignature.refobs();
	if ( abstractRefObsRequirement or concreteRefObsRequirement )
	{
		if ( ! abstractRefObsRequirement or ! concreteRefObsRequirement )
			throw UnableToGetData("check- and algorithm signature does not match");
		populateRefObs_(db, observation, * abstractRefObsRequirement, * concreteRefObsRequirement);
	}
	...
	if ( abstractModelRequirement or concreteModelRequirement )
	{
		...
		populateModel_(db, observation, * abstractModelRequirement, * concreteModelRequirement);
	}
...
namespace
{
bool setFlag(kvalobs::kvData & data, int flagIndex, int newValue, bool regardlessOfOldValue = false)
{
...
bool updateCfailed(kvalobs::kvData & data, const std::string & qcx, int newValue)
{
	if ( newValue > 1 )
	{
