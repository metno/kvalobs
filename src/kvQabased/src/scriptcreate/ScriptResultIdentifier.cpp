...
ScriptResultIdentifier::ScriptResultIdentifier(const std::string & identifier) {
  ...
  if (correctionType == "flag")
    ...
    throw UndefinedCorrectionType(
        "Undefined correction type: " + correctionType);

}

ScriptResultIdentifier::ScriptResultIdentifier(const std::string & parameter,
                                               int timeIndex, int stationIndex,
...
std::ostream & operator <<(std::ostream & s,
  ...
  s << sri.parameter() << '_' << sri.timeIndex() << '_' << sri.stationIndex()
    << '_';
