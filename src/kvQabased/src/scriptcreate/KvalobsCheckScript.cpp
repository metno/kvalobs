		...
		scriptLog_(scriptLog)
{
	try
	{
		...
		store_.reset(new DataStore(database, obs, check.qcx(), abstractSignature, concreteSignature));
		script_.reset(new scriptrunner::Script(algorithm.script(), getInterpreter(algorithm.language())));
...
void KvalobsCheckScript::run(db::DatabaseAccess::DataList * modificationsOut)
{
	...
	{
		(* scriptLog_) << "\nRESULT:\n\n";
		for (scriptrunner::RunResult::RunReturn::const_iterator it = ret.begin(); it != ret.end(); ++it)
	...
	if ( scriptLog_ and not modificationsOut->empty() )
	{
		...
		for ( db::DatabaseAccess::DataList::const_iterator it = modificationsOut->begin(); it != modificationsOut->end(); ++ it )
			(* scriptLog_) << '\n' << * it;
		(* scriptLog_) << std::endl;
	}
}
