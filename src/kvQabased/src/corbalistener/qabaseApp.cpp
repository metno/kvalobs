...
QaBaseApp::QaBaseApp(int argn, char **argv, const std::string &driver_,
		const std::string &connect_, const char *opt[][2]) :
	KvApp(argn, argv, opt), dbConnect(connect_), dbDriver(driver_), refManager(
			CKvalObs::CManager::CheckedInput::_nil()), shutdown_(false),
			orbIsDown(false)
{
 ...
 */
CKvalObs::CManager::CheckedInput_ptr QaBaseApp::lookUpManager(bool forceNS,
		bool &usedNS)
{
		...
		if (forceNS)
		{
			...
			if (CORBA::is_nil(obj))
				throw LookUpException(
						"EXCEPTION: Can't obtain a reference for 'kvManagerCheckedInput'\n           from the CORBA nameserver!");

			ptr = CKvalObs::CManager::CheckedInput::_narrow(obj);

			if (CORBA::is_nil(ptr))
				throw LookUpException(
						"EXCEPTION: Can't narrow reference for 'kvManagerCheckedInput'!");
...
void QaBaseApp::releaseDbConnection(dnmi::db::Connection *con)
{
