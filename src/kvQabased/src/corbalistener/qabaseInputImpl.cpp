/*
 ...
 $Id: qabaseInputImpl.cc,v 1.1.2.3 2007/09/27 09:02:21 paule Exp $
...
QaBaseInputImpl::QaBaseInputImpl(QaBaseApp &app_) :
	app(app_)
{
}
...
CORBA::Boolean QaBaseInputImpl::newDataCB(const CKvalObs::StationInfo& data,
		...
		const micutil::KeyValList& args, CORBA::Boolean &bussy)
{
	...
	try
	{
		app.getInQue().postAndBrodcast(new QaWorkCommand(data,
				CKvalObs::CManager::CheckedInput::_duplicate(callback), args));
	} catch (...)
	{
		return false;
	}
