...
public:
  ...
  void beginTransaction(dnmi::db::Connection::IsolationLevel isolation =
                            dnmi::db::Connection::READ_COMMITTED);
  void commit();
  void rollback();
...
KvalobsDatabaseAccess::TransactionEnforcingDatabaseConnection::
    TransactionEnforcingDatabaseConnection(dnmi::db::Connection *connection,
                                           bool takeOwnershipOfConnection)
    : connection_(connection), transactionInProgress_(false),
...
dnmi::db::Result *
KvalobsDatabaseAccess::TransactionEnforcingDatabaseConnection::execQuery(
...
void KvalobsDatabaseAccess::TransactionEnforcingDatabaseConnection::
...
bool KvalobsDatabaseAccess::TransactionEnforcingDatabaseConnection::
    transactionInProgress() const {
...
KvalobsDatabaseAccess::~KvalobsDatabaseAccess() { delete connection_; }
...
kvalobs::kvAlgorithms
KvalobsDatabaseAccess::getAlgorithm(const std::string &algorithmName) const {
  std::ostringstream query;
...
std::string KvalobsDatabaseAccess::getStationParam(
    const kvalobs::kvStationInfo &si, const std::string &parameter, int sensor,
    int level, const std::string &qcx) const {
  ...
  query << "SELECT metadata FROM station_param WHERE "
        ...
        << "') AND "
           "fromday<="
        << dayNumber << " AND " << dayNumber
        << "<=today AND "
           "qcx='"
        << qcx
        << "' AND "
           "level="
        << level << " AND sensor='" << sensor
        << "' AND "
           "fromtime<='"
  ...
  if (result->hasNext()) {
    ...
    throw std::runtime_error("Unable to find station_param for " + parameter +
                             " for qcx=" + qcx);
  }
}

void KvalobsDatabaseAccess::getStationParamAll(
    qabase::StationParamList &outResult, const kvalobs::kvStationInfo &si,
    const std::string &parameter, const std::string &qcx) const {
  const boost::gregorian::date &date = si.obstime().date();
  const boost::gregorian::date firstDayOfYear(date.year(), 1, 1);
  int dayNumber = (date - firstDayOfYear).days() + 1;
  outResult.clear();
  ...
  while (result->hasNext()) {
    ...
    try {
      ...
      ost << "Failed to convert 'sensor=" << row[0] << "' or 'level=" << row[1]
          << "' to integer (out_of_range)";
      throw std::runtime_error(ost.str());
    } catch (const std::invalid_argument &ex) {
      std::ostringstream ost;
      ost << "Failed to convert 'sensor=" << row[0] << "' or 'level=" << row[1]
          << "' to integer (invalid_argument)";
      throw std::runtime_error(ost.str());
    }
    stp.metadata = row[2];
...
qabase::Observation
KvalobsDatabaseAccess::getObservation(const kvalobs::kvStationInfo &si) const {
  std::ostringstream query;
...
void KvalobsDatabaseAccess::getModelData(
  ...
  if (minutesBackInTime != 0) {
    boost::posix_time::ptime first =
        si.obstime() + boost::posix_time::minutes(minutesBackInTime);
    if (first == si.obstime())
...
void KvalobsDatabaseAccess::getData(
  ...
  query << "SELECT "
           ...
           "d.sensor, d.level, d.corrected, d.controlinfo, d.useinfo, "
           "d.cfailed, o.observationid "
           "FROM "
  ...
  boost::posix_time::ptime t =
      obs.obstime() + boost::posix_time::minutes(minuteOffset);
  if (t == obs.obstime())
...
std::string obsdataQuery(const qabase::Observation &obs) {
  ...
  query
      << "SELECT "
         "o.stationid, o.obstime, d.original, d.paramid, o.tbtime, o.typeid, "
         "d.sensor, d.level, d.corrected, d.controlinfo, d.useinfo, d.cfailed "
         "FROM "
         "observations o, obsdata d "
         "WHERE "
         "o.observationid = d.observationid AND "
...
void KvalobsDatabaseAccess::complete_(const qabase::Observation &obs,
                                      DataList *out) const {
  ResultPtr result(connection_->execQuery(obsdataQuery(obs)));
  while (result->hasNext()) {
    kvalobs::kvData d = kvDataFromRow(result->next());
    auto func = std::bind(kvalobs::compare::same_obs_and_parameter(), d,
                          std::placeholders::_1);
...
KvalobsDatabaseAccess::KvalobsDataPtr
KvalobsDatabaseAccess::complete(const qabase::Observation &obs,
                                const DataList &d,
  ...
  for (DataList::const_iterator it = data.begin(); it != data.end(); ++it) {
    ...
    if (result->size() != 1)
      throw std::runtime_error(
          "Update statement did not affect exactly one row");
  }
#endif
}

std::list<qabase::Observation *>
KvalobsDatabaseAccess::selectFailedDataForControl(int limit) {
  // Check for any data that has not completed the check in the last 10 minutes.
  // Possibly crached or stopped qaBase process. We run the checks again.

  // SELECT o.observationid, stationid, typeid, obstime, o.tbtime FROM workque
  // q, observations o WHERE q.observationid=o.observationid AND
  // qa_start<now()-'10 minutes'::interval AND qa_stop is null ORDER BY
  // priority, tbtime LIMIT 1;";

  std::ostringstream query;

  if (qaId > -1) {
    query << "SELECT o.observationid, stationid, typeid, obstime, o.tbtime "
             "FROM workque q, observations o "
             "WHERE q.observationid=o.observationid AND qa_start<now()-'10 "
             "minutes'::interval AND qa_stop is null"
             " AND (qa_id="
          << qaId
          << " OR qa_id is null) "
             "ORDER BY qa_id, priority, tbtime LIMIT "
          << limit << ";";
  } else
    query << "SELECT o.observationid, stationid, typeid, obstime, o.tbtime "
             "FROM workque q, observations o "
             "WHERE q.observationid=o.observationid AND qa_start<now()-'10 "
             "minutes'::interval AND qa_stop is null "
             "ORDER BY priority, tbtime LIMIT "
          << limit << ";";
...
std::list<qabase::Observation *>
KvalobsDatabaseAccess::selectLatestDataBasedOnObstimeForControl(int limit) {
  // Ensure that we process the latest data the last 3 hours first.
  // Truncate the search criteria to hour, with minutes, seconds, microsecond
  // set to zero.

  // SELECT o.observationid, stationid, typeid, obstime, o.tbtime FROM workque
  // q, observations o WHERE q.observationid=o.observationid AND qa_start IS
  // NULL AND process_start is not null AND obstime >= date_trunc('hour',
  // now()-'3 hours'::interval) ORDER BY priority, obstime LIMIT 1;
  ...
  if (qaId > -1) {
    query << "SELECT o.observationid, stationid, typeid, obstime, o.tbtime "
             "FROM workque q, observations o "
             "WHERE q.observationid=o.observationid AND qa_start IS NULL AND "
             "process_start is not null"
             " AND obstime >= date_trunc('hour', now()-'3 hours'::interval) "
             "AND (qa_id="
          << qaId
          << " OR qa_id is null) "
             "ORDER BY qa_id, priority, obstime LIMIT "
          << limit << ";";
  } else {
    query << "SELECT o.observationid, stationid, typeid, obstime, o.tbtime "
             "FROM workque q, observations o "
             ...
             "process_start is not null"
             " AND obstime >= date_trunc('hour', now()-'3 hours'::interval) "
             "ORDER BY priority, obstime LIMIT "
          << limit << ";";
  }
...
std::list<qabase::Observation *>
KvalobsDatabaseAccess::selectOlderDataControl(int limit) {
  // Process older data sorted by tbtime. Process the latest observations first.
  // SELECT o.observationid, stationid, typeid, obstime, o.tbtime FROM workque
  // q, observations o WHERE q.observationid=o.observationid AND process_start
  // is not null AND qa_start IS NULL ORDER BY priority, tbtime LIMIT 1;
  std::ostringstream query;

  if (qaId > -1) {
    query << "SELECT o.observationid, stationid, typeid, obstime, o.tbtime "
             "FROM workque q, observations o "
             "WHERE q.observationid=o.observationid AND process_start is not "
             "null AND qa_start IS NULL AND (qa_id="
          << qaId
          << " OR qa_id is null ) "
             "ORDER BY qa_id, priority, tbtime LIMIT "
          << limit << ";";
...
KvalobsDatabaseAccess::newObservation(std::unique_ptr<dnmi::db::Result> &r) {
  ...
  while (r->hasNext()) {
    ...
    boost::posix_time::ptime tbtime =
        boost::posix_time::time_from_string(row[4]);
    ret.push_back(
        new qabase::Observation(observationid, station, type, obstime, tbtime));
  }
  return ret;
}

std::list<qabase::Observation *>
KvalobsDatabaseAccess::selectDataForControl(int limit) {
  ...
  if (needOwnTransaction) {
    ...
    LOGWARN("trying to select data for control with previous transaction - "
  ...
  try {
    ...
    if (obs.size() == 1) {
      LOGINFO("Selected for control "
              << whichSelect << ": " << (*obs.begin())->stationID() << "/"
              << (*obs.begin())->typeID() << "/"
...
isolation level not guaranteed");
    ...
    std::unique_ptr<dnmi::db::Result> result(connection_->execQuery("select
o.stationid from workque q, observations o where o.observationid=q.observationid
and qa_start is not null and qa_stop is null;")); while (result->hasNext()) {
      auto row = result->next();
    ...
    query << "select o.observationid, stationid, typeid, obstime, o.tbtime from
workque q, observations o " "where q.observationid=o.observationid and
process_start is not null and "
    ...
    int type = boost::lexical_cast<int>(row[2]);
    boost::posix_time::ptime obstime =
boost::posix_time::time_from_string(row[3]); boost::posix_time::ptime tbtime =
boost::posix_time::time_from_string(row[4]);
    ...
    LOGINFO("Selected for controll: "<< station <<"/" << type << "/" <<
pt::to_kvalobs_string(obstime)<<"/" << pt::to_kvalobs_string(tbtime)<<"("<<
observationid << ")"); return new qabase::Observation(observationid, station,
type, obstime, tbtime);
}
*/
void KvalobsDatabaseAccess::markProcessDone(const qabase::Observation &obs) {
  ...
  query
      << "UPDATE workque SET qa_stop=statement_timestamp() WHERE observationid="
      << obs.id();
...
void KvalobsDatabaseAccess::storeFetched(long long obsid,
                                         const kvalobs::kvData &d) const {
  ...
  if (!insertResult.second) {
    ...
    if (obsid != oldId) {
      std::ostringstream s;
      s << "obsid mismatch: got " << obsid << " previous was " << oldId
        << " data: " << d;
      throw std::runtime_error(s.str());
    }
  }
...
void KvalobsDatabaseAccess::storeFetched(long long obsid,
                                         const kvalobs::kvTextData &d) const {
  ...
  if (!insertResult.second) {
    ...
    if (obsid != oldId) {
      std::ostringstream s;
      s << "textdata obsid mismatch: got " << obsid << " previous was " << oldId
        << " data: " << d;
      throw std::runtime_error(s.str());
    }
  }
