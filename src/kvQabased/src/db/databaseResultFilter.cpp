...
struct caseInsensitiveEq
{
	...
	caseInsensitiveEq(const std::string & wanted) : wantedLower(boost::algorithm::to_lower_copy(wanted)) {}
...
float parseStationParam(const std::string & metadataToParse, const std::string & wanted)
{
	std::vector<std::string> params;
	boost::split(params, metadataToParse, boost::algorithm::is_any_of(";\n"));

	std::vector<std::string>::const_iterator find = std::find_if(params.begin(), params.end(), caseInsensitiveEq(wanted));
...
void filterTypeId_(DList & data, int preferredTypeId)
{
	...
	{
		typename std::map<int, DList>::iterator preferred = typeSortedData.find(preferredTypeId);
...
{
	std::set<T> values;
	for ( db::DatabaseAccess::DataList::const_iterator it = data.begin(); it != data.end(); ++ it )
