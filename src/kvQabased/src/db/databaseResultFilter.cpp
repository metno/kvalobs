...
struct caseInsensitiveEq {
  ...
  bool operator()(const std::string &s) const {
    return s == wantedLower ||
           boost::algorithm::to_lower_copy(s) == wantedLower;
  }
};
...
float parseStationParam(const std::string &metadataToParse,
  ...
  if ((params.size() % 2) != 0) {
    throw std::runtime_error(
        "metadata, the number of parameters and values must be equal");
  }

  std::vector<std::string>::const_iterator find =
      std::find_if(params.begin(), params.end(), caseInsensitiveEq(wanted));
...
namespace {
template <class DList> void filterTypeId_(DList &data, int preferredTypeId) {
  ...
  if (typeSortedData.size() > 1) {
    typename std::map<int, DList>::iterator preferred =
        typeSortedData.find(preferredTypeId);
