...
void parse(const boost::filesystem::path & configFile, variables_map & vm,
           const options_description & configFileOptions) {
using namespace boost::filesystem;

if (not exists(configFile))
  throw std::runtime_error(
      to_native_file(configFile) + ": no such file or directory");
...
database.add_options()(
    ...
    "Database user")(
    "select-for-control-count,s",value<int>(&selectForControlCount_)->default_value(6),
