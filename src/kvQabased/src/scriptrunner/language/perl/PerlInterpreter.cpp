...
std::string PerlInterpreter::fullScript(const Script & script) const {
  ...
  for (std::vector<Script::Input>::const_iterator it =
      script.getInput().begin(); it != script.getInput().end(); ++it) {
    ss << "# " << it->name() << '\n';
...
std::ostream & PerlInterpreter::numericDeclarations(
    std::ostream & s, const Script::Input & input) const {
  for (Script::Input::NumericParameters::const_iterator it = input.numeric()
      .begin(); it != input.numeric().end(); ++it)
    s << "my $" << it->first << " = " << it->second << ";\n";
  return s;
}

std::ostream & PerlInterpreter::stringDeclarations(
    std::ostream & s, const Script::Input & input) const {
  for (Script::Input::StringParameters::const_iterator it = input.strings()
      .begin(); it != input.strings().end(); ++it)
    s << "my $" << it->first << " = " << quote(it->second) << ";\n";
  return s;
}

std::ostream & PerlInterpreter::numericListDeclarations(
    std::ostream & s, const Script::Input & input) const {
  for (Script::Input::NumericListParameters::const_iterator it = input
      .numericList().begin(); it != input.numericList().end(); ++it) {
    s << "my @" << it->first << " = (";
...
std::ostream & PerlInterpreter::stringListDeclarations(
    std::ostream & s, const Script::Input & input) const {
  for (Script::Input::StringListParameters::const_iterator it =
      input.stringList().begin(); it != input.stringList().end(); ++it) {
    s << "my @" << it->first << " = (";
...
std::string registration = internal::registerInterpreter("perl",
                                                         getPerlInterpreter);
}
