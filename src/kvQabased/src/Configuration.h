...
public:
	 ...
	 *         Otherwise true.
	 */
	bool runNormally() const { return runNormally_; }
	 ...
	 */
	bool haveObservationToCheck() const { return observationToCheck_; }
	 ...
	 */
	const kvalobs::kvStationInfo * observationToCheck() const { return observationToCheck_; }

	/**
	 * Has the user specified a single qcx check to run?
	 * @return tru if user have specified a singe qcx
	 */
	bool onlySpecificQcx() const { return not qcxFilter_.empty(); }
	 ...
	 */
	const std::string & baseLogDir() const { return baseLogDir_; }
	 ...
	 * @return same stream as given in argument.
	 */
	std::ostream & help(std::ostream & s, const boost::program_options::options_description & options) const;
