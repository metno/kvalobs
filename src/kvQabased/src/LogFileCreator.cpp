...
boost::filesystem::path getLogFileName(
    const kvalobs::kvStationInfo & observationToCheck) {
...
boost::filesystem::path getLogFile(
    const kvalobs::kvStationInfo & observationToCheck,
  ...
  if (not exists(base) and !create_directories(base))
    throw LogFileCreationError(
        "Unable to create base logging folder " + base.string());
  ...
  if (is_directory(base)) {
    ...
    if (not exists(logFile) and !create_directories(logFile))
      throw LogFileCreationError(
          "Unable to create logging folder " + logFile.string());
  ...
  if (exists(logFile)) {
    ...
    try {
      ...
      if (pos == std::string::npos)
        pos = newName.size() - 1;
      unsigned logVersion = boost::lexical_cast<unsigned>(
          newName.substr(pos + 1));
      if (logVersion >= 9)  // max number of log files
          {
        remove(logFile);
        return;
      }
      newName = newName.substr(0, pos + 1)
          + boost::lexical_cast<std::string>(logVersion + 1);
...
LogFileCreator::LogStreamPtr LogFileCreator::getLogStream(
  ...
  if (not baseDirectory_.empty()) {
    try {
      boost::filesystem::path logFile = getLogFile(observationToCheck,
                                                   baseDirectory_);
      renameOldLogs(logFile);
      LogStreamPtr logStream(new boost::filesystem::ofstream(logFile));
      if (not logStream->good())
        throw LogFileCreationError(
            "Error when opening log file: " + to_native_file(logFile));
      return logStream;
