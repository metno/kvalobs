...
TEST_F(DelayedSaveDatabaseAccessTest, onlySaveOnCommit)
{
	...
	for ( db::DatabaseAccess::DataList::const_iterator it = toSave.begin(); it != toSave.end(); ++ it )
	{
		FakeDatabaseAccess::SavedData::const_iterator find = fakeDb.savedData.find(* it);
		EXPECT_FALSE( find == fakeDb.savedData.end() ) << "Unable to find data: " << * it;
		if ( find != fakeDb.savedData.end() )
			EXPECT_TRUE(kvalobs::compare::exactly_equal()(* find, * it )) << * find << " is different from " << * it;
	}
}
...
TEST_F(DelayedSaveDatabaseAccessTest, rereadSavedData)
{
	...
	EXPECT_TRUE(kvalobs::compare::exactly_equal()(secondRead.front(), data.front()));
}
