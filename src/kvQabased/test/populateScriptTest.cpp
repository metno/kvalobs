...
{
	qabase::CheckSignature abstractRequirement("obs;X,Y;;|meta;M;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_24,RR_12;10;0,-1440|meta;RR_24_MAX;10;0", observation.stationID());
...
{
	qabase::CheckSignature abstractRequirement("obs;X;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_12;;", observation.stationID());
	qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
...
TEST_F(populateScriptTest, usesOriginalValueForObs)
{
	...
	EXPECT_CALL(mockDatabase, getData(_, observation, qabase::DataRequirement::Parameter("RR_24"), 0))
				.Times(AtLeast(1))
				.WillRepeatedly(SetArgumentPointee<0>(data));

	qabase::CheckSignature abstractRequirement("obs;R;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_24;;", observation.stationID());
	qabase::DataStore store(mockDatabase, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
	...
	const scriptrunner::ScriptInput::ValueList & rr24Missing = numList["R_missing"];
...
{
	qabase::CheckSignature abstractRequirement("obs;R;;|refobs;Rstart,Robs;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_X;;|refobs;KLSTART,KLOBS;;", observation.stationID());
	qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
	...
	const scriptrunner::ScriptInput::ValueList & rstartMissing = numList["Rstart_missing"];
	ASSERT_EQ(1u, rstartMissing.size());
	EXPECT_EQ(3, rstartMissing.front());

	const scriptrunner::ScriptInput::ValueList & robsMissing = numList["Robs_missing"];
...
TEST_F(populateScriptTest, oneRefObsNonexisting)
{
	...
	qabase::CheckSignature abstractRequirement("obs;R;;|refobs;Rstart,Robs;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_X;;|refobs;KLSTART,KLOBS;;0,-60", observation.stationID());
	qabase::DataStore store(mockDatabase, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
...
{
	qabase::CheckSignature abstractRequirement("obs;X;;|meta;M,N;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_24;;|meta;RR_24_MIN,RR_24_MAX;;", observation.stationID());
	qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
...
{
	qabase::CheckSignature abstractRequirement("obs;X;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_12;10;0,-1440", observation.stationID());
	qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
...
{
	qabase::CheckSignature abstractRequirement("obs;X;;|model;mX;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_12&0&&;;|model;RR_12&0&&;;", observation.stationID());
	qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement);
	qabase::addDataToScript(script, store);
...
{
	qabase::CheckSignature abstractRequirement("obs;X,Y;;|meta;M;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_24;10;0,-1440|meta;RR_24_MAX;10;0", observation.stationID());

	EXPECT_THROW(qabase::DataStore store(database, observation, "QC1-1-1", abstractRequirement, concreteRequirement),
...
{
	qabase::CheckSignature abstractRequirement("obs;X,Y;;|meta;M;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("meta;RR_24_MAX;10;0", observation.stationID());
...
{
	qabase::CheckSignature abstractRequirement("meta;M;;", observation.stationID());
	qabase::CheckSignature concreteRequirement("obs;RR_24;10;0,-1440|meta;RR_24_MAX;10;0", observation.stationID());
