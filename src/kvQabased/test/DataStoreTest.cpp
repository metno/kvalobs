...
TEST(DataStore_ParameterSortedDataListTest, fillMissing_nothingToFill) {
  kvalobs::kvDataFactory factory(
      10, boost::posix_time::time_from_string("2010-06-09 06:00:00"), 302);
...
TEST(DataStore_ParameterSortedDataListTest, fillMissing_missingLastEntry) {
  ...
  expectedResult["B"].push_back(
      factory.getMissing(
          0, boost::posix_time::time_from_string("2010-06-07 06:00:00")));

  qabase::fillMissing(dl);

  ASSERT_EQ(3u, dl["B"].size());
  EXPECT_EQ(boost::posix_time::time_from_string("2010-06-07 06:00:00"),
            dl["B"][2].obstime());
...
TEST(DataStore_ParameterSortedDataListTest, fillMissing_manyMissingLastEntries) {
  ...
  ASSERT_EQ(3u, dl["A"].size());
  EXPECT_EQ(boost::posix_time::time_from_string("2010-06-08 06:00:00"),
            dl["A"][1].obstime());
...
TEST(DataStore_ParameterSortedDataListTest, fillMissing_missingInAllSets) {
  ...
  ASSERT_EQ(3u, dl["A"].size());
  EXPECT_EQ(boost::posix_time::time_from_string("2010-06-09 06:00:00"),
            dl["A"][0].obstime());
