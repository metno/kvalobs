...
namespace {
kvalobs::kvChecks makeCheck(const std::string & signature,
                            const std::string & name = "test_check_1") {
  return kvalobs::kvChecks(
      10, "QC1-2-101", "QC1-2", 1, name, signature, "* * * * *",
      boost::posix_time::time_from_string("2010-01-01 00:00:00"));
}

kvalobs::kvAlgorithms makeAlgorithm(const std::string & signature,
...
std::string simpleFlagScript = "sub check() {\n"
    " my @retvector;\n"
...
std::string simpleRejectScript = "sub check() {\n"
    " my @retvector;\n"
...
TEST_F(KvalobsCheckScriptTest, runCheck) {
  ...
  EXPECT_CALL(database, getAlgorithm(__func__)).Times(AtLeast(1)).WillRepeatedly(
      Return(makeAlgorithm("obs;X;;", simpleScript, __func__)));

  KvalobsCheckScript script(database, observation,
                            makeCheck("obs;RR_24;;", __func__));
...
TEST_F(KvalobsCheckScriptTest, missingFlagUpdatesCorrected) {
  ...
  EXPECT_CALL(database, getAlgorithm(__func__)).Times(AtLeast(1)).WillRepeatedly(
      Return(makeAlgorithm("obs;X;;", simpleRejectScript, __func__)));
...
TEST_F(KvalobsCheckScriptTest, setsGreatestFlag) {
  ...
  EXPECT_CALL(database, getAlgorithm(__func__)).Times(AtLeast(1)).WillRepeatedly(
      Return(makeAlgorithm("obs;X;;", simpleFlagScript, __func__)));

  kvalobs::kvDataFactory factory(
      10, boost::posix_time::time_from_string("2010-05-25 06:00:00"), 302);
  ...
  EXPECT_CALL(database, getData(_, observation, qabase::DataRequirement::Parameter("RR_24"), 0))
      .Times(AtLeast(1)).WillRepeatedly(SetArgumentPointee<0>(dataList));
  ...
  ASSERT_EQ(1u, modifications.size());
  EXPECT_EQ(kvalobs::kvControlInfo("0050000000000000"),
            modifications.front().controlinfo());
}
...
TEST_F(KvalobsCheckScriptTest, setsFlagInCorrectPosition) {
  ...
  KvalobsCheckScript script(
      database,
      observation,
      kvalobs::kvChecks(
          observation.stationID(), "QC1-3-101", "QC1-3", 1, __func__,
          "obs;RR_24;;", "* * * * *",
          boost::posix_time::time_from_string("2010-01-01 00:00:00")));
  db::DatabaseAccess::DataList modifications;
  script.run(&modifications);
  ...
  EXPECT_EQ(kvalobs::kvControlInfo("0004000000000000"),
            modifications.front().controlinfo());
}
