...
App::App(int argn, char **argv)
  ...
  for (int k = 1; k < argn; k++) {
    ...
    if (key == "--tracelevel") {
      ...
      LOGWARN(
          "Unknown option: " << key << (!val.empty()?string("="+val):"") << endl);
    }
  }
  ...
  if (logdir_[0] != '/') {
    LOGFATAL(
        "The 'logdir' directory must " << "be an absolute path. ex /my/logdir.");
    usage();
  }

  if (stat(logdir_.c_str(), &sbuf) < 0) {
    LOGFATAL(
        "Cant stat the file/path <" << logdir_ << ">\n" << "-- " << strerror(errno) << endl);
    exit(1);
  }
  ...
  if (access(logdir_.c_str(), R_OK | W_OK) < 0) {
    LOGFATAL(
        "autoobs2kv must have read/write access to the 'logdir' directory\n");
    exit(1);
  }
  ...
  if (datadir_[0] != '/') {
    LOGFATAL(
        "The 'datadir' directory must " << "be an absolute path. ex /my/datadir.");
    usage();
  }

  if (stat(datadir_.c_str(), &sbuf) < 0) {
    LOGFATAL(
        "Cant stat the file/path <" << datadir_ << ">\n" << "-- " << strerror(errno) << endl);
    exit(1);
  }
  ...
  if (access(datadir_.c_str(), R_OK | W_OK) < 0) {
    LOGFATAL(
        "autoobs2kv must have read/write access to the 'datadir' directory\n");
    exit(1);
  }
  ...
  if (piddir_[0] != '/') {
    LOGFATAL(
        "The 'piddir' directory must " << "be an absolute path. ex /my/piddir.");
    usage();
  }

  if (stat(piddir_.c_str(), &sbuf) < 0) {
    LOGFATAL(
        "Cant stat the file/path <" << piddir_ << ">\n" << "-- " << strerror(errno) << endl);
    exit(1);
  }
  ...
  if (access(piddir_.c_str(), R_OK | W_OK) < 0) {
    LOGFATAL(
        "autoobs2kv must have read/write access to the 'piddir' directory\n");
    exit(1);
  }
  ...
  if (myConf) {
    ...
    if (it != valelem.end()) {
      ...
      LOGWARN(
          "No \"path\" is given in the corba section of autoobs2kv.conf" << endl << " using default path <kvalobs>." << endl << "Notis this is the path that is the monitored production" << endl << "server!");
      kvServer_ = "kvalobs";
    }
  }
  ...
  if (!getDirList(mydir, datadir_)) {
    LOGERROR("Cant get the directory list for <" << datadir_ << "!" <<endl);
    exit(1);
  }

  //Remove all files directorys in the datadir that is not
  //in highpriServers and backup
  for (IFileList it = mydir.begin(); it != mydir.end(); it++) {
    if (it->file() == "backup")
      continue;
  ...
  for (TKvDataReceiverList::const_iterator it = highpriServers_.begin();
      it != highpriServers_.end(); it++)
    highpri += " " + it->confName;

  for (TKvDataReceiverList::const_iterator it = lowpriServers_.begin();
      it != lowpriServers_.end(); it++)
    lowpri += " " + it->confName;

  LOGINFO(
      "Pushing data to kvDataInputd in the following paths in the\n" << "CORBA nameserver:\n" "  highpri servers: " << highpri << endl << "   lowpri servers: " << lowpri << endl << "datadir:              <" << datadir_ << ">" << endl << "logdir:               <" << logdir_ << ">" << endl << "piddir:               <" << piddir_ << ">" << endl << "kvserver:             <" << kvServer_ << ">" << endl << "aopath:               <" << aopath_ << ">" << endl << "newfileInterfaceName: <" << newfileName << ">" <<endl << "CORBA nameserver:     <" << getNameservice() << ">" << endl);
...
bool App::readFile(const std::string &file, std::string &content) const {
...
bool App::readFile(const std::string &path, const std::string &name,
...
bool App::writeFile(const std::string &path, const std::string &name,
  ...
  for (unique = 0; unique < UNIQUE; unique++) {
    sprintf(buf, "-%04d%02d%02d%02d-%d.dat", t.date().year(), t.date().month(),
            t.date().day(), t.time_of_day().hours(), unique);
    ost.str("");
    ost << path << "/" << name << buf;
  ...
  if (fd) {
    ...
    if (rename(fileInProgres.c_str(), file.c_str()) == -1) {
      LOGERROR("Cant rename file: " << fileInProgres << " -> " << file << "\n");
    } else {
...
bool App::getFileList(FileList &fileList, const std::string &path_) const {
  ...
  try {
    ...
    while (dir.hasNext()) {
      ...
      if (stat(filepath.c_str(), &sbuf) < 0) {
        LOGERROR(
            "Cant get modification time for the file <" << filepath << ">!");
        continue;
      }
    ...
    LOGERROR(
        "EXCEPTION: While scanning the directory <" << path << ">. Reason: " << ex.what());
    return false;
  }
...
bool App::getDirList(FileList &dirList, const std::string &path_) const {
  dnmi::file::Dir dir;
  string file;
  string path(path_);
  string filepath;
  ...
  try {
    ...
    LOGERROR(
        "EXCEPTION: While scanning the directory <" << path << "> for directorys. Reason: " << ex.what());
    return false;
  }
...
CORBA::Object_ptr App::getRefInNS(const std::string &name_,
...
bool App::putRefInNS(CORBA::Object_ptr objref, const std::string &name) {
  return putObjInNS(objref, name);
}

Result*
App::sendDataToKvalobs(const std::string &message, const std::string &obsType,
                       KvDataSrc &server) {
  Data_ptr ptrData;
  bool forceNS = false;
  bool usedNS;
...
CKvalObs::CDataSource::Data_ptr App::lookUpKvData(bool forceNS, bool &usedNS,
                                                  KvDataSrc &server)
  ...
  while (true) {
    if (forceNS) {
      ...
      obj = getRefInNS("kvinput", server.dataReceiver().name,
                       server.dataReceiver().ns);
...
void App::initLogger(const std::string &ll, const std::string &tl) {
  char *name;
  string filename;
...
bool App::dataReady(int timeoutInSecs, bool &timeout) {
  unsigned long s, ns;
...
void App::notifyServers() {
  std::list<CollectData*>::iterator it;

  for (it = collectDataThreads.begin(); it != collectDataThreads.end(); it++) {
    (*it)->notifyData();
  }
}
...
void App::addCollectDataThread(CollectData *data) {
  collectDataThreads.push_back(data);
}

void App::joinAllCollectDataThread() {
  std::list<CollectData*>::iterator it;
 ...
 */
bool App::removeDir(const std::string &path_) const {
  dnmi::file::Dir dir;
  string path(path_);
  string filepath;
  string file;
  ...
  try {
    ...
    LOGERROR(
        "EXCEPTION: While removing file in directory <" << path << ">. Reason: " << ex.what());
  }
...
bool setLoglevel(const std::string &ll, const std::string &tl) {
  LogLevel loglevel;
