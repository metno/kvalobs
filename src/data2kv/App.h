...
class App : public CorbaHelper::CorbaApp {
 ...
 public:
  ...
  CORBA::Object_ptr
  getRefInNS(const std::string &name_, const std::string &path_,
             const CorbaHelper::ServiceHost &nameserver);

  bool putRefInNS(CORBA::Object_ptr objref, const std::string &name_);

  CKvalObs::CDataSource::Data_ptr
  lookUpKvData(bool forceNS, bool &usedNS, KvDataSrc &server);

  CKvalObs::CDataSource::Result
  *sendDataToKvalobs(const std::string &message, const std::string &obsType,
                     KvDataSrc &server);

  std::string kvServer() const {
    return kvServer_;
  }
  TKvDataReceiverList highpriServers() const {
    return highpriServers_;
  }
  TKvDataReceiverList lowpriServers() const {
    return lowpriServers_;
  }
  bool inShutdown();
  void doShutdown();
  ...
  bool getFileList(FileList &fileList, const std::string &path) const;

  bool getDirList(FileList &dirList, const std::string &path) const;

  bool removeDir(const std::string &path) const;

  bool readFile(const std::string &filepath, std::string &content) const;

  bool readFile(const std::string &path, const std::string &name,
                std::string &content) const;
  bool writeFile(const std::string &path, const std::string &name,
                 const std::string &content) const;

  std::list<std::string> copyDataToDirs() const {
    return copyToDirList;
  }
  std::string aopath() const {
    return aopath_;
  }
  std::string newfileInterfaceName() const {
    return newfileName;
  }
