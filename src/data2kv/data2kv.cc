...
main(int argn, char **argv)
{
	...
	for(its=servers.begin(); its!=servers.end(); its++){
		...
		try{
			collectDataThread=new CollectData(app, false, app.datadir()+its->dirName, *its);
	...
	for(its=servers.begin(); its!=servers.end(); its++){
		...
		try{
			collectDataThread=new CollectData(app, true, app.datadir()+its->dirName, *its);
			app.addCollectDataThread(collectDataThread);
		}
		catch(...){
			LOGFATAL("NOMEM: Cant initialize the application.");
	...
	try{
		...
		if(!app.putRefInNS(ref, "/"+app.aopath()+"/"+app.newfileInterfaceName())){
		...
		if(dnmi::file::isRunningPidFile(pidfile, error)){
		    if(error){
		      LOGFATAL("An error occured while reading the pidfile:" << endl
			       ...
			       << endl << "data2kv is not running. " << 
			       "If it is running and there is problems. Kill data2kv and"
			       << endl << "restart it." << endl << endl);
		      return 1;
		    }else{
		      LOGFATAL("Is data2kv allready running?" << endl
