...
CollectData::
CollectData( App &app_,
				 bool  lowpri_,
				 const std::string &datadir_,
				 const KvDataReceiver &dataReceiver_ )
	:app(app_), lowpri(lowpri_), datadir(datadir_),
    server( dataReceiver_ ), dataReady_(false),dataReadyCond(&dataReadyMutex)
{
	start_undetached();
}
...
bool 
CollectData::
dataReady(unsigned long timeoutInSecs, bool &timeout)
{
	timeout=false;
	unsigned long s, ns;
...
bool
CollectData::
sendMessageToKvalobs( const std::string &msg,
		                ...
		                bool &tryToResend)
{
	...
	if(!resTmp){
		kvServerIsUp=false;
		tryToResend=true;
		LOGERROR( "Cant connect to " << server.dataReceiver().confName << ". Is it running?" 
		          << endl);
	...
	if(res->res==CKvalObs::CDataSource::OK){
      ...
      LOGERROR(server.dataReceiver().confName << " DECODEERROR (rejected): " << res->message);
      tryToResend=false;
	}else{
      LOGERROR("kvalobs Unknown response from " << server.dataReceiver().confName 
      		   << ". Check if the code is in sync"
	            << " with 'datasource.idl'!");
      tryToResend=false;
	}
...
bool
CollectData::tryToSendSavedObservations()
{
		 ...
		 it++)
	{
		...
		if(!sendMessageToKvalobs(content, type, kvServerIsUp, tryToResend)){
			...
			if(!tryToResend){
				LOGERROR("SAVEDOBS, " << server.dataReceiver().confName 
						   << " 'says' I should delete the observation."); 
				unlink(it->name().c_str());
			}else{
				LOGERROR("Cant send saved observation to " << server.dataReceiver().confName 
						   << "." << endl  <<"Will try to send later!");
			}
			...
			LOGINFO(server.dataReceiver().confName
					  <<" got the observation. Deleteing local copy!");
			unlink(it->name().c_str());
		}
	}
