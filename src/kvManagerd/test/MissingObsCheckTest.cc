...
int main(int argc, char** argv) {
  ...
  system(
      string("sqlite3 " + constr + "< ../../kvalobs_database/script.create")
          .c_str());
  status = system(
      string("sqlite3 " + constr + "< MissingObsCheckTest_import.sql").c_str());

  if (WEXITSTATUS(status) != 0) {
    cerr << "Cant execute: 'sqlite3 " << constr
         << " < MissingObsCheckTest_import.sql'\n";
    return 1;
  }
  ...
  MissingObsCheck missingCheck(*con, que, &genCache,
                               boost::bind(&MyShutdown::shutdown, &app));
