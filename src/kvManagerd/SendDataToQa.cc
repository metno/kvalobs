...
SendDataToQa::SendDataToQa(ManagerApp &app_,
                           dnmi::thread::CommandQue &inputQue_)
    : app(app_),
      inputQue(inputQue_) {
}

void SendDataToQa::operator()() {
...
bool SendDataToQa::sendData(const kvalobs::kvStationInfo &stationInfo) {
  bool qaOk = true;
  bool bussy = true;
...
void SendDataToQa::processStartedElements(int wait_on_qabase) {
  ostringstream ost;
  int qaDownTime;
  ...
  for (list<kvWorkelement>::iterator it = workList.begin();
      it != workList.end() && !app.shutdown(); it++) {
...
dnmi::db::Connection*
SendDataToQa::getNewDbConnection() {
