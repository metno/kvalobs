/*
 ...
 $Id: NewDataJob.cc,v 1.2.2.2 2007/09/27 09:02:34 paule Exp $                                                       
...
void NewDataJob::doJob(dnmi::db::Connection &con) {
   ...
   */
  boost::posix_time::ptime startime =
  ...
  try {
    result = dbGate.select(
        values, kvQueries::selectKeyValues("kvQabased", "checkpoint"));
  ...
  for (itd = datalist.begin(); itd != datalist.end(); itd++) {
    ...
    if (itd->stationID() != stationid || itd->obstime() != obstime
        || itd->typeID() != typeID) {

      // post the previous stationinfo
      if (stationid != -1)
        postToQue(stationinfo);
      ...
      LOGDEBUG(
          jobName() << " : " << num_mis_stations << ", Adding new station:" << stationid << " for obstime:" << obstime);
  ...
  LOGDEBUG(
      jobName() << " FINISHED. Pushed " << num_mis_stations << " stations, total of " << num_mis_params << " dataentries for processing to qabase");
}
