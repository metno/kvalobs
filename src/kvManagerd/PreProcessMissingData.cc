...
bool PreProcessMissingData::loadParamsAndTypes() {
  ...
  try {
    ...
    if (result) {
      ...
      LOGERROR(
          "Failed to read the '" << table <<"' table from the database.\n" << "Reason: " << dbGate.getErrorStr());
    }
    ...
    LOGERROR(
        "Exception: Failed to read the '" << table <<"' table from the database.\n" << "Reason: " << ex.what());
  } catch (...) {
    LOGERROR(
        "Exception: Failed to read the '" << table <<"' table from the database.\n" << "Reason: Unknown.");
  }
  ...
  if (nOk == 2) {
    ...
    nextDbCheck = boost::posix_time::ptime(
        now.date(),
        boost::posix_time::time_duration(now.time_of_day().hours(), 0, 0));
  } else {
  ...
  LOGDEBUG(
      "PreProcessMissingData: Next loading of 'types' and 'params' table: "
          + to_kvalobs_string(nextDbCheck));

  return nOk == 2;
}

bool PreProcessMissingData::paramIsMinute(int typeid_, int paramid) {
  bool isMinParam = false;

  if (!loadParamsAndTypes()) {
    LOGERROR(
        "PreProcessMissingData: The 'param' and 'types' table is not available.\n" << "Can't decide if paramid (" << paramid << ") typeid (" << typeid_ << ") is minute data.");
    return false;
  }
...
void PreProcessMissingData::flagDataNotInObsPgm(
    const std::list<kvalobs::kvObsPgm> &obsPgm,
    std::list<kvalobs::kvData> &data, std::list<kvalobs::kvData> &wildObs) {
  bool hasThis;

  for (std::list<kvalobs::kvData>::iterator it = data.begin(); it != data.end();
      ++it) {
    ...
    for (std::list<kvalobs::kvObsPgm>::const_iterator itop = obsPgm.begin();
      ...
      if (it->paramID() == paramid && it->sensor() < nr_sensor
          && it->level() == level && itop->isOn(it->obstime())) {
        hasThis = true;
        break;
      }
    }
...
bool PreProcessMissingData::isMissingMessage(
    const std::list<kvalobs::kvData> &datalist) const {
  if (datalist.empty())
    return true;
...
void PreProcessMissingData::removeFromList(
    const kvalobs::kvData &data, std::list<kvalobs::kvData> &dataList) const {
  for (std::list<kvalobs::kvData>::iterator it = dataList.begin();
      it != dataList.end(); ++it) {
    if (data.stationID() == it->stationID() && data.typeID() == it->typeID()
        && data.paramID() == it->paramID() && data.level() == it->level()
...
void PreProcessMissingData::doJob(long stationId, long typeId,
  ...
  if (typeId < 0) {
    LOGDEBUG1("Generated data is not checked for missing. typeId=" << typeId);
    return;
  } else {
    LOGINFO(
        "doJob STARTING typeId:" << typeId << " obstime:" << obstime << std::endl);
  }
  ...
  try {
    result = dbGate.select(
        datalist, kvQueries::selectDataFromType(stationId, typeId, obstime));
  } catch (dnmi::db::SQLException &ex) {
  ...
  try {
    result = dbGate.select(obspgmlist,
                           kvQueries::selectObsPgm(stationId, typeId, obstime));
  } catch (dnmi::db::SQLException &ex) {
    LOGERROR("Exception: " << ex.what() << std::endl);
  } catch (...) {
    LOGERROR("Unknown exception: con->exec(ctbl) .....\n");
  ...
  for (itop = obspgmlist.begin(); itop != obspgmlist.end(); itop++) {
    ...
    for (sensor = 0; sensor < nr_sensor; sensor++) {
      ...
      for (itd = datalist.begin(); itd != datalist.end(); itd++) {
        if (itd->paramID() == paramid && itd->level() == level
            && itd->sensor() == sensor && itd->typeID() == typeId)
          break;
      }
      ...
      if (itd == datalist.end()) {  //paramid not found
        ...
        kvData data(stationId, obstime, original, paramid, tbtime, typeId,
                    sensor, level, corrected, controlinfo, useinfo, failed);
  ...
  if (paramObsPgmCount > 0) {
    if (missingParamCount > 0) {
      LOGINFO(
          "Missing " << missingParamCount << " parameters. Should be " << paramObsPgmCount << " parameters for the station!");
    } else {
