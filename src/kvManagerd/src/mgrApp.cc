/*
  ...
  $Id: mgrApp.cc,v 1.20.6.1 2007/09/27 09:02:36 paule Exp $                                                       
...
ManagerApp::ManagerApp(int argn, char **argv, 
...
void 
ManagerApp::setCheckedInput(CKvalObs::CManager::CheckedInput_ptr ptr)
{
  refCheckedInput=ptr;
}
...
bool 
ManagerApp::sendDataToQA(const kvalobs::kvStationInfo &info_)
{
  ...
  try{
    for(int i=0; i<2; i++){
      ...
      try{
	qa->newDataCB(stInfo, refCheckedInput, keyValList);
	return true;
      }
...
ManagerApp::lookUpKvService(bool forceNS, bool &usedNS)
{
...
ManagerApp::lookUpQabase(bool forceNS, bool &usedNS)
{
  CORBA::Object_var obj;
...
kvalobs::kvStation 
ManagerApp::lookUpStation(const miutil::miString &kvQuerie)
{
...
void                 
ManagerApp::releaseDbConnection(dnmi::db::Connection *con)
{
...
void 
ManagerApp::status(const std::string &m)
{
  boost::mutex::scoped_lock l(mutex);
  status_=m;
}
