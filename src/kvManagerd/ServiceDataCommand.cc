...
ServiceDataCommand::ServiceDataCommand(const CKvalObs::StationInfoList &stInfo)
    : CheckedDataCommandBase(stInfo) {
}

ServiceDataCommand::~ServiceDataCommand() {
}

bool ServiceDataCommand::executeImpl() {
  const int LIMIT = 10;
  ostringstream ost;
  int errorCount = 0;
  ...
  LOGINFO(
      "Cleaned up workque: errorCount=" << errorCount << " #workelements: " << workList.size());
}
