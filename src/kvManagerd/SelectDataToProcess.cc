...
SelectDataToProcess::SelectDataToProcess(
    ManagerApp &app_, dnmi::thread::CommandQue &inputQue_,
    dnmi::thread::CommandQue &outputQue_,
    dnmi::thread::CommandQue &workInProcessQue_)
    : app(app_),
      inputQue(inputQue_),
      outputQue(outputQue_),
      workInProcessQue(workInProcessQue_),
      dbCon(0) {
}

SelectDataToProcess::SelectDataToProcess(const SelectDataToProcess &p)
    : app(p.app),
      inputQue(p.inputQue),
      outputQue(p.outputQue),
      workInProcessQue(p.workInProcessQue),
      dbCon(p.dbCon),
      iRange(1) {
}
...
void SelectDataToProcess::processElements(
    std::list<kvalobs::kvWorkelement> &workList, dnmi::db::Connection *con) {
  ...
  while (it != workList.rend() && !app.shutdown()) {
    ...
    ost << "UPDATE workque SET process_start='"
        << to_kvalobs_string(it->process_start()) << "' WHERE stationid="
        << it->stationID() << "  AND obstime='"
        << to_kvalobs_string(it->obstime()) << "' AND typeid=" << it->typeID();
    ...
    if (!gate.exec(ost.str())) {
      LOGERROR(
          "processElements:: Cant update table workque." << "-- Stationid: " << it->stationID() << endl << "--   obstime: " << it->obstime() << endl << "--    typeid: " << it->typeID() << endl << "--  query: " << ost.str() << endl << "-- reason: " << gate.getErrorStr());
      try {
        con->rollBack();
      } catch (...) {
      }
    ...
    try {
      ...
      LOGDEBUG(
          "processElements: started procesing of: " << endl << "-- Stationid: " << it->stationID() << endl << "-- obstime:   " << it->obstime() << endl << "-- typeid:    " << it->typeID() << endl << "-- priority:  " << it->priority());
    } catch (...) {
...
dnmi::db::Connection*
SelectDataToProcess::newConnection() {
...
bool SelectDataToProcess::processLowPriData(
    const boost::posix_time::ptime &baseTime) {
  ...
  if (iRange < DAY_LIMIT) {
    ...
    LOGDEBUG(
        "processLowPriData: iRange=" << iRange << endl << "   baseTime: " << baseTime << endl << " lowerBound: " << lowerBound << endl << " upperBound: " << upperBound);

    ost << "WHERE obstime>='" << to_kvalobs_string(lowerBound) << "' AND "
        << "obstime<'" << to_kvalobs_string(upperBound) << "' AND "
        << "process_start IS NULL " << "ORDER BY priority, stationid, typeid, "
        << "         obstime DESC " << "LIMIT " << LOW_PRI_LIMIT;

    if (!gate.select(workList, ost.str(), "workque")) {
      LOGERROR(
          "Cant get 'workelement' from 'workque'!" << "-- " << gate.getErrorStr());
      return false;
    }
    ...
    LOGDEBUG(
        "processLowPriData: iRange<" << DAY_LIMIT << endl << "   baseTime: " << baseTime << endl << " upperBound: " << upperBound);
...
void SelectDataToProcess::operator()() {
  ...
  while (!app.shutdown()) {
    ...
    ost << "WHERE obstime>='" << to_kvalobs_string(lowerBound) << "' AND "
        ...
        << "ORDER BY priority, obstime, tbtime, typeid DESC " << "LIMIT "
        << HIGH_PRI_LIMIT;
