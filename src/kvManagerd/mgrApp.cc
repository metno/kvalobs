...
ManagerApp::ManagerApp(int argn, char **argv, const std::string &driver_,
                       const std::string &connect_, const char *opt[][2])
    : KvApp(argn, argv, opt),
      dbDriver(driver_),
      dbConnect(connect_),
...
bool ManagerApp::isOk() const {
  //For now!
  return KvApp::isOk();
}

void ManagerApp::setServiceCheckedInput(
    CKvalObs::CManager::CheckedInput_ptr ptr) {
  refServiceCheckedInput = ptr;
}

void ManagerApp::setCheckedInput(CKvalObs::CManager::CheckedInput_ptr ptr) {
  refCheckedInput = ptr;
}

bool ManagerApp::isGenerated(long stationid, long typeid_,
                             dnmi::db::Connection *con) {
  return genCache_.isGenerated(stationid, typeid_, con);
}

bool ManagerApp::sendDataToQA(const kvalobs::kvStationInfo &info, bool &bussy) {
  ...
  if (CORBA::is_nil(refCheckedInput)) {
    LOGFATAL(
        "There is no instance of the " << "interface CKvalObs::CManager::CheckedInput. This is a BUG!!!");
    return false;
  }
  ...
  try {
    for (int i = 0; i < 2; i++) {
      ...
      try {
        ...
        LOGWARN(
            "(sendDataToQA) Exception CORBA::TRANSIENT!" << endl << "The reason can be a network problem!");
    ...
    LOGERROR(
        "Can't contact CORBA nameservice. Is the CORBA nameservice running." << endl << "(" <<ex.what() << ")" << endl);
    return false;
  } catch (...) {
    LOGERROR(
        "Cant send data to kvQaBase, hmmm, very strange, a unkown exception!\n");
    return false;
  }
...
bool ManagerApp::sendDataToKvService(const kvalobs::kvStationInfoList &info_,
                                     bool &bussy) {
  kvalobs::kvStationInfoList info =
      const_cast<kvalobs::kvStationInfoList&>(info_);
  //  kvalobs::CIkvParamInfoList it;
  ...
  for (CORBA::Long k = 0; itInfoList != info.end(); k++, itInfoList++) {
    ...
    stInfoList[k].obstime = CORBA::string_dup(
        to_kvalobs_string(itInfoList->obstime()).c_str());
  ...
  try {
    for (int i = 0; i < 2; i++) {
      ...
      try {
        if (!service->dataReadyExt("__##kvManagerd@@very_secret_hash:-)##__",
                                   stInfoList, refServiceCheckedInput,
                                   serviceBussy)) {
          if (serviceBussy)
            bussy = true;
        }
        ...
        LOGWARN(
            "(sendDataToKvService) Exception CORBA::TRANSIENT!" << endl << "The reason can be a network problem!");
      } catch (CORBA::COMM_FAILURE &ex) {
    ...
    LOGERROR(
        "Cant send data to kvService, hmmm, very strange, a unkown exception!\n");
    return false;
  }
 ...
 */
CKvalObs::CService::DataReadyInputExt_ptr ManagerApp::lookUpKvService(
    bool forceNS, bool &usedNS) {
    ...
    if (forceNS) {
      ...
      if (CORBA::is_nil(obj))
        throw LookUpException(
            "EXCEPTION: Can't obtain a reference for 'kvServiceDataReady'\n           from the CORBA nameserver!");

      ptr = CKvalObs::CService::DataReadyInputExt::_narrow(obj);

      if (CORBA::is_nil(ptr))
        throw LookUpException(
            "EXCEPTION: Can't narrow reference for 'kvServiceDataReady'!");
...
CKvalObs::CQaBase::QaBaseInput_ptr ManagerApp::lookUpQabase(bool forceNS,
                                                            bool &usedNS) {
  CORBA::Object_var obj;
  ...
  while (true) {
    if (forceNS) {
      ...
      if (CORBA::is_nil(obj))
        throw LookUpException(
            "EXCEPTION: Can't obtain a reference for 'kvQabaseInput'\n           from the CORBA nameserver!");

      ptr = CKvalObs::CQaBase::QaBaseInput::_narrow(obj);

      if (CORBA::is_nil(ptr))
        throw LookUpException(
            "EXCEPTION: Can't narrow reference for 'kvQabaseInput'!");
...
kvalobs::kvStation ManagerApp::lookUpStation(const std::string &kvQuerie) {
  return kvalobs::kvStation();
}

dnmi::db::Connection*
ManagerApp::getNewDbConnection() {
  ...
  if (!con) {
    LOGERROR(
        "Can't create a database connection  (" << dbDriverId << ")" << endl << "Connect string: <" << dbConnect << ">!");
    return 0;
  }
...
void ManagerApp::releaseDbConnection(dnmi::db::Connection *con) {
  //Due to a 'race condition' error i dbMgr.connect(  ... )
  //that can cause a severe error we must use a protect
  //connect and releaseConnection with a mutex.
  boost::mutex::scoped_lock l(mutex);
...
void ManagerApp::ok(bool f) {
  boost::mutex::scoped_lock l(mutex);
  ok_ = f;
}

bool ManagerApp::ok() const {

  boost::mutex::scoped_lock l(const_cast<ManagerApp*>(this)->mutex);
  return ok_;
}

std::string ManagerApp::status() const {
  boost::mutex::scoped_lock l(const_cast<ManagerApp*>(this)->mutex);
  return status_;
}

void ManagerApp::status(const std::string &m) {
  boost::mutex::scoped_lock l(mutex);
  status_ = m;
}
