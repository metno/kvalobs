...
class ManagerApp : public KvApp {
   ...
   */
  CKvalObs::CQaBase::QaBaseInput_ptr lookUpQabase(bool forceNS, bool &usedNS);
 ...
 public:
  ManagerApp(int argn, char **argv, const std::string &driver,
  ...
  bool sendDataToQA(const kvalobs::kvStationInfo &info, bool &bussy);
  bool sendDataToKvService(const kvalobs::kvStationInfoList &info_,
                           bool &bussy);

  bool checkForMissingObs() const {
    return checkForMissingObs_;
  }
  void checkForMissingObs(bool f) {
    checkForMissingObs_ = f;
  }
