/*
 ...
 $Id: PreProcessWorker.cc,v 1.2.2.3 2007/09/27 09:02:35 paule Exp $                                                       
...
PreProcessWorker::PreProcessWorker(const PreProcessWorker &p)
    : app(p.app),
      jobList(p.jobList),
      oneTimeJobList(p.oneTimeJobList),
      inputQue(p.inputQue),
...
void PreProcessWorker::addJob(PreProcessJob *job) {
  boost::mutex::scoped_lock lock(mutex);

  jobList->push_back(PreProcessJobPtr(job));
}

void PreProcessWorker::addOneTimeJob(OneTimeJob *job) {
  boost::mutex::scoped_lock lock(mutex);

  oneTimeJobList->push_back(OneTimeJobPtr(job));
}

void PreProcessWorker::removeJob(PreProcessJob *job) {
  boost::mutex::scoped_lock lock(mutex);
...
void PreProcessWorker::postDataToOutputQue(StationInfoCommand *infoCmd,
  ...
  try {
    for (; it != infoList.end(); it++) {
      if (!gate.select(
          data,
          kvQueries::selectDataFromType(it->stationID(), it->typeID(),
                                        it->obstime())))
        continue;

      kvStationInfo info(it->stationID(), it->obstime(), it->typeID());
...
void PreProcessWorker::operator()() {
  ...
  while (!app.shutdown()) {
    ...
    for (; it != jobList->end(); it++) {
      ...
      LOGDEBUG(
          "StationInfo: stationID: " << itInfoList->stationID() << " typeID: " << itInfoList->typeID() << " obstime: " << itInfoList->obstime());

      for (; itInfoList != infoList.end(); itInfoList++) {
        LOGDEBUG("Preprocessing: jobName: " << (*it)->jobName());
        (*it)->doJob(itInfoList->stationID(), itInfoList->typeID(),
                     itInfoList->obstime(), *con);
      }
    }
