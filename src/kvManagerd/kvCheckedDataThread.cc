...
KvCheckedDataThread::KvCheckedDataThread(ManagerApp &app_,
                                         dnmi::thread::CommandQue &inputQue_)
    : app(app_),
      inputQue(inputQue_) {
}

void KvCheckedDataThread::operator()() {
 ...
 */
bool KvCheckedDataThread::processWaitingObs(CheckedDataHelper &helper) {
  const int LIMIT = 10;
  ...
  do {
    ...
    ost << "WHERE qa_stop IS NOT NULL AND "
        ...
        << "ORDER BY stationid, typeid, obstime DESC " << "LIMIT " << LIMIT;
    ...
    if (!gate.select(workList, ost.str(), "workque")) {
      LOGERROR(
          "DBERROR: Cant retrive data from workque!" << "Querry: " << ost.str() << endl << "Reason: " << gate.getErrorStr());
    ...
    for (; it != workList.end(); it++) {
      ...
      for (sit = sendtList.begin(); sit != sendtList.end(); sit++) {
        if (it->stationID() == sit->stationID()
            && it->obstime() == sit->obstime() && it->typeID() == sit->typeID())
          break;
      }

      if (sit == sendtList.end()) {
        stList.push_front(
            kvalobs::kvStationInfo(it->stationID(), it->obstime(),
                                   it->typeID()));
        sendtList.push_back(
            kvalobs::kvStationInfo(it->stationID(), it->obstime(),
                                   it->typeID()));
 ...
 * NOT in workList.
 */
void KvCheckedDataThread::removeFromSendtList(
    kvalobs::kvStationInfoList &sendtList,
  ...
  while (it != sendtList.end()) {
    for (cIt = workList.begin(); cIt != workList.end(); cIt++) {
      if (it->stationID() == cIt->stationID() && it->obstime() == cIt->obstime()
...
void KvCheckedDataThread::cleanupWorkque() {
  const int LIMIT = 100;
  ostringstream ost;
  int errorCount;
  list<kvalobs::kvWorkelement> workList;
  ...
  do {
    ...
    for (it = workList.begin(); it != workList.end(); it++) {
      if (!gate.remove(*it, "workque")) {
        errorCount++;
        LOGERROR(
            "DBERROR: cant remove from workque: " << endl << "-- Stationid: " << it->stationID() << " obstime: " << it->obstime() << " typeid: " << it->typeID() << endl << "Reason: " << gate.getErrorStr());
      }

      if (!gate.insert(*it, "workstatistik")) {
        LOGERROR(
            "DBERROR: cant insert into workstatistik: " << endl << "-- Stationid: " << it->stationID() << " obstime: " << it->obstime() << " typeid: " << it->typeID() << endl << "Reason: " << gate.getErrorStr());
      }
    }
...
dnmi::db::Connection*
KvCheckedDataThread::getNewConnection() {
