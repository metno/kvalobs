...
CheckForMissingObsMessages::CheckForMissingObsMessages(
    ManagerApp &app_, dnmi::thread::CommandQue &outputQue_)
    : app(app_),
      outputQue(outputQue_) {
}

CheckForMissingObsMessages::CheckForMissingObsMessages(
    const CheckForMissingObsMessages &c)
    : app(c.app),
      outputQue(c.outputQue) {
}

CheckForMissingObsMessages::~CheckForMissingObsMessages() {
}

boost::posix_time::ptime CheckForMissingObsMessages::lastMissingRuntime(
  ...
  if (lastMissingRunTime_.is_not_a_date_time()) {
    ...
    if (!gate.select(
        keys, kvQueries::selectKeyValues("kvManagerd", "LastMissingRun"))) {
      LOGWARN(
          "Cant access the key 'kvManagerd.LastMissingRun' from the 'key_val' table!" << endl << "Reason: " << gate.getErrorStr());
    } else if (keys.empty()) {
      kvKeyVal keyVal("kvManagerd", "LastMissingRun",
                      to_kvalobs_string(lastMissingRunTime_));

      if (!gate.insert(keyVal, true)) {
        LOGWARN(
            "Cant insert the key 'kvManagerd.LastMissingRun' into the 'key_val' table!" << endl << "Reason: " << gate.getErrorStr());
      } else {
        LOGINFO(
            "Created new key 'kvManagerd.LastMissingRun' in the 'key_val' table!");
      }
    } else {
      lastMissingRunTime_ = boost::posix_time::time_from_string_nothrow(
          keys.begin()->val());
      ...
      LOGINFO(
          "key_val table: Key 'kvManagerd.LastMissingRun': " <<lastMissingRunTime_);

      if (lastMissingRunTime_ < tmp) {
        LOGINFO(
            "The saved 'kvManagerd.LastMissingRun'(" << lastMissingRunTime_ <<") is to old."<< endl << "Setting it to: " << tmp);
...
void CheckForMissingObsMessages::lastMissingRuntime(
    dnmi::db::Connection *con,
    const boost::posix_time::ptime &newLastMissingRuntime) {
  kvDbGate gate(con);

  kvKeyVal keyVal("kvManagerd", "LastMissingRun",
                  to_kvalobs_string(newLastMissingRuntime));
...
void CheckForMissingObsMessages::operator()() {
 ...
 */
void CheckForMissingObsMessages::findMissingData(
    const boost::posix_time::ptime& runtime, dnmi::db::Connection *con) {
  string logfile;
  ...
  LOGINFO(
      "Missing search: started: " << startAt << " Stopped: " << stopAt << " elpased time: " << stopAt - startAt);
