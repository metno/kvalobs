...
void MissingObsCheck::postCommandToQue(kvalobs::StationInfoCommand *cmd) {
  ...
  for (; it != stationInfoList.end(); it++) {
    if (!gate.insert(
        kvWorkelement(it->stationID(), it->obstime(), it->typeID(), tbtime, 10,
                      undefTime, undefTime, undefTime, undefTime, undefTime),
        true)) {
      LOGERROR(
...
bool MissingObsCheck::checkObstime(const boost::posix_time::ptime &runTime,
  ...
  pt::ptime checkTime = pt::ptime(
      runTime.date(), pt::time_duration(runTime.time_of_day().hours(), 0, 0));

  checkTime -= pt::hours(h);
  checkTime += pt::minutes(m);

  obstime = pt::ptime(checkTime.date(),
                      pt::time_duration(checkTime.time_of_day().hours(), 0, 0));

  return checkTime < runTime;
}

MissingObsCheck::MissingObsCheck(dnmi::db::Connection &con_,
                                 dnmi::thread::CommandQue &outputQue_,
                                 GenCache *genCache_, ShutdownHook shutdown_)
    : con(&con_),
...
MissingObsCheck::~MissingObsCheck() {
}

void MissingObsCheck::findMissingData(
    const boost::posix_time::ptime& runtime,
  ...
  boost::posix_time::ptime obstime;
  boost::posix_time::ptime nowObstime(
      runtime.date(),
  ...
  searchUntilObstime = pt::ptime(
      lastSearchForMissing.date(),
      pt::time_duration(lastSearchForMissing.time_of_day().hours(), 0, 0));
  ...
  LOGINFO(
      "Start search: " << runtime << endl << "Last search:  " << lastSearchForMissing << endl << "obstime:      " << nowObstime << endl << "Search until obstime: " << searchUntilObstime << endl << "nTimes:       " << nTimes);
  ...
  for (itTypes = typeslist.begin(); itTypes != typeslist.end(); itTypes++) {
    ...
    LOGINFO(
        "Searching for typeid: " << itTypes->typeID() << " lateobs: " << itTypes->lateobs() << endl << "obstime: " << obstime);

    for (int i = 0; i < nTimes; i++) {
      obspgmlist.clear();
      result = dbGate.select(
          obspgmlist,
          kvQueries::selectObsPgmByTypeid(itTypes->typeID(), obstime));
      ...
      LOGINFO(
          "Searching for typeid: " << itTypes->typeID() << " obstime: " << obstime << " obspgmlist size: " << obspgmlist.size());
      ...
      for (itop = obspgmlist.begin(); itop != obspgmlist.end(); itop++) {
        ...
        result = dbGate.select(
            datalist, kvQueries::selectDataFromType(sid, tid, obstime));
        ...
        LOGINFO(
            "Missing observation: stationid: " << sid << " typeid: " << tid << endl << "For obstime: " << obstime << endl);
