        ...
        const long lateobsInMinute)
{
    ...
    pt::ptime checkTime=pt::ptime(runTime.date(),
                                  pt::time_duration(runTime.time_of_day().hours(), 0, 0));

    checkTime -= pt::hours(h);
    checkTime += pt::minutes(m);

    obstime = pt::ptime(checkTime.date(), pt::time_duration(checkTime.time_of_day().hours(), 0, 0));
        ...
        const boost::posix_time::ptime& lastSearchForMissing)
{
    ...
    searchUntilObstime = pt::ptime( lastSearchForMissing.date(),
