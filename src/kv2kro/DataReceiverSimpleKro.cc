...
MyGetDataReceiver::MyGetDataReceiver(const std::string& fileFromTime,
...
bool MyGetDataReceiver::next(KvObsDataList &datalist) {
  ...
  for (it = datalist.begin();    //sett it til starten av listen
    ...
    if (it->dataList().size() > 0) {
      cout << "stationID: " << it->stationid() << " obstime: "
           << it->dataList().front().obstime() << " parameters: "
           << it->dataList().size() << endl;
    }
...
/*****************************************/
void kv2kro::createPidFile(const std::string &progname) {
  FILE *fd;
