...
/*
  ...
  if (opt.typeids.empty() && opt.stations.empty()) {
    cout << "typelist and stationlist cant both be empty!" << endl << endl;
    use(1);
  }

  if (opt.fromtime.undef()) {
    cout << "Fromtime must be given!" << endl << endl;
    use(1);
  }
...
*/
  ...
  string q="where stationid=12550 and obstime ='2017-10-06 06:00' order by paramid,level;";

  if( ! gate.select(data, q) ) {
    cerr << "Failed to get data\n";
    cerr << "Error: " << gate.getErrorStr()<< ".\n\n";
  } else {
    kvalobs::serialize::KvalobsData kvdata(data);
    kvalobs::serialize::KvalobsDataSerializer ser(kvdata);
...
void use(int exitcode) {
  cout
      << "kvpush, a helper program to trigger pushing of"
      << endl
      << "data to subscribers of kvalobs data.!"
      << endl
      << endl
      << "Use"
      << endl
      << "   kvpush [-h] [-q] [-i typeidlist] -s stationlist -f fromdate [-t todate]"
      ...
      << "Date format: " << endl
      << "   'YYYY-MM-DDThh:mm:ss', mm:ss in the time part can be omitted."
      << endl << endl;
