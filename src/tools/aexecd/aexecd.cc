...
main(int argn, char *argv[])
{
   ...
   pidFile = dnmi::file::createPidFileName( kvalobs::kvPath(kvalobs::rundir), "aexecd" );
   ...
   if( dnmi::file::isRunningPidFile( pidFile, pidFileError ) ) {
       if( pidFileError ){
           LOGFATAL( "An error occured while reading the pidfile:" << endl
                   ...
                   << "aexecd is not running. If it is running and" << endl
                   << "there is problems. Kill aexecd and" << endl
                   << "restart it." << endl << endl );
           return 1;
       }else{
           LOGFATAL( "Is aexecd allready running?" << endl
   ...
   if(argn==2){
      ...
      if( val.empty() ) port=0;
      else port=val[0].valAsInt( 0 );
   }

   if(port==0)
      Usage(1);

   LOGINFO("aexecd started. Listening on port: " << port << ". Pidfile '" << pidFile << "'.");
   ...
   while(sigTerm==0) {
         ...
         while((deadpid=waitpid(-1, &exitStatus, WNOHANG))>0) {
            ...
            if(itPid!=childMap.end()) {
                if( retStatus == 0 ) {
                    ...
                    LOGERROR(itPid->second << ", terminated with exitstatus " << retStatus << ".");
                }
                childMap.erase(itPid);
            }else {
               LOGERROR("ERROR, the child with pid " << deadpid << " is not registred in the childMap.");
            }
         }
      }
      ...
      if(!pSocket) {
         if(!interupted) {
            ...
            if(server.errorType()==miutil::socket_status_codes::NO_MEM) {
               LOGFATAL("Server, out of memmory. Exiting and in the hope that someone/something restart me.");
               exit(4);
            }
         }
      ...
      if(pid< 0) {
         ...
         if(!childLog.empty())
            fd=open(childLog.c_str(), O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP);
         else
...
*/
bool
GetCommandAndLogfile(const std::string &buf, 
                     std::string &command,
                     std::string &logfile,
                     int &timeout )
{
