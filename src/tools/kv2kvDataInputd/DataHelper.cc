...
Param::Param(const kvalobs::kvData &data)
    : paramid(data.paramID()),
      sensor(data.sensor()),
      level(data.level()) {
}

Param::Param(const kvalobs::kvTextData &textData)
    : paramid(textData.paramID()),
      sensor( INT_MAX),
      level( INT_MAX) {
}

bool Param::isTextData() const {
...
bool Param::operator<(const Param &rhs) const {
  ...
  if (paramid < rhs.paramid || (paramid == rhs.paramid && sensor < rhs.sensor)
...
bool Param::operator==(const Param &rhs) const {
  if (paramid == rhs.paramid && sensor == rhs.sensor && level == rhs.level) {
    return true;
  } else {
    return false;
  }
...
operator<<(std::ostream &out, const Param &param) {
  out << "[" << param.paramid << "," << param.sensor << "," << param.level
      << "]";
  return out;
}
...
bool ObsData::add(const kvalobs::kvData &kvData) {
  if (stationid == INT_MAX) {
    ...
    type = kvData.typeID();
  } else if (stationid != kvData.stationID() || type != kvData.typeID()) {
    return false;
  }
...
bool ObsData::add(const kvalobs::kvTextData &kvData) {
  if (stationid == INT_MAX) {
    stationid = kvData.stationID();
    type = kvData.typeID();
  } else if (stationid != kvData.stationID() || type != kvData.typeID()) {
...
std::string ObsData::createHeader(ParamDefsPtr paramdefs) {
  ostringstream ost;
  bool first = true;
...
std::string ObsData::getData(const Param &param,
...
void ObsData::klData(std::string &data, std::string &decoder,
...
bool ObsData::operator<(const ObsData &rhs) const {
...
void DataHelper::addData(KvObsDataListPtr obs) {
  for (KvObsDataList::iterator it = obs->begin(); it != obs->end(); ++it) {
...
void DataHelper::initNext() {
  //Index the obsData to make it easy to get the encoded data

  dataList.clear();

  for (ObsDataList::iterator it = obsData.begin(); it != obsData.end(); ++it) {
...
bool DataHelper::nextData(string &data, string &decoder,
                          ParamDefsPtr paramdefs) {
