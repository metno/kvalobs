...
CorbaServerConf::
CorbaServerConf( const CorbaServerConf &cns )
   : name( cns.name), ns( cns.ns )
{
}

CorbaServerConf&
CorbaServerConf::
operator=( const CorbaServerConf &rhs )
{
   if( this != &rhs ) {
      name = rhs.name;
      ns = rhs.ns;
...
bool
CorbaServerConf::
decodeConfspec( const std::string &confString, const std::string &defaultNameserver )
{
