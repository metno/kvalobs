...
class KvPushApp : public KvApp {
   ...
   */
  void replace(std::string &src, const std::string &what,
  ...
  bool selectFrom(const Options::List &stationidList,
                  ...
                  bool doQa, dnmi::db::Connection *con);

  bool selectData(const std::string &query, const miutil::miTime &fromtime,
                  const miutil::miTime &totime, bool doQa,
                  dnmi::db::Connection *con);

  bool updateWorkque(dnmi::db::Result *res, bool doQa,
                     dnmi::db::Connection *con);
