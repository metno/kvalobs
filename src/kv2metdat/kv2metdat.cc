...
int main(int argc, char** argv) {
  ...
  while (passf) {
    getline(passf, line);
    boost::split(words, line, boost::algorithm::is_space(),
    ...
    if (words[0] == "dbpass") {
      if (!words[1].empty())
        constr = "user=kvalobs dbname=kvalobs host=localhost password="
            + words[1];
      else
  ...
  list<kvStation> pos;
  gate.select(
      pos, " where (lat!=0 or lon!=0) and static=true and stationid < 100000");
