...
void
DataNotifyCommand::
buildWhatList( kvskel::kvDataNotifySubscriber::WhatList &wl,
...
execute(KvDataSubscriberInfo &sinf)
{
	...
	DataToSubscribersPtr dp=data();
 	...
 	if(!sinf.thisStation(dp->stationid) ||
     	...
     	LOGDEBUG("DataNotifyCommand::excute: NOT interested in this dataset!" << endl
     				<< sinf);
	...
	try{
   	ret=0;
  		...
  		LOGINFO("SUCCESS: CALL Stationid: " << dp->stationid << " typeid: " << 
