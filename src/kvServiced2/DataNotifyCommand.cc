/*
 ...
 $Id: DataNotifyCommand.cc,v 1.1.2.2 2007/09/27 09:02:21 paule Exp $                                                       
...
void DataNotifyCommand::buildWhatList(
    kvskel::kvDataNotifySubscriber::WhatList &wl, DataToSubscribersPtr dp) {
...
int DataNotifyCommand::execute(KvDataSubscriberInfo &sinf) {
  using namespace CKvalObs::CService;
  kvDataNotifySubscriber::WhatList wl;
  int ret;
  DataToSubscribersPtr dp = data();

  if (!sinf.thisStation(dp->stationid) || !sinf.checkStatusAndQc(dp)) {
    LOGDEBUG(
        "DataNotifyCommand::excute: NOT interested in this dataset!" << endl << sinf);
  ...
  try {
    ...
    subscriber->callback(wl);
    LOGINFO(
        "SUCCESS: CALL Stationid: " << dp->stationid << " typeid: " << dp->typeid_ << " obstime: " << dp->obstime);
  } catch (CORBA::TRANSIENT &ex) {
    LOGERROR(
        "EXCEPTION: (timeout?) Can't send <DataNotify> event to subscriber!" << endl << "Subscriberid: " << sinf.subscriberid() << ">!");
    ret = 1;
  } catch (...) {
    LOGERROR(
        "EXCEPTION: Can't send <DataNotify> event to subscriber!" << endl << "Subscriberid: " << sinf.subscriberid() << ">!");
    ret = -1;
  }
