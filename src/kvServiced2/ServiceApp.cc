...
ServiceApp::ServiceApp(int argn, char **argv, const std::string &driver_,
...
bool ServiceApp::isOk() const {
  //For now!
  return KvApp::isOk();
}

kvalobs::kvStation ServiceApp::lookUpStation(
    const miutil::std::string &kvQuerie) {
  return kvalobs::kvStation();
}

dnmi::db::Connection*
ServiceApp::getNewDbConnection() {
  Connection *con;
...
void ServiceApp::releaseDbConnection(dnmi::db::Connection *con) {
...
void ServiceApp::notifyAllKvHintSubscribers(bool kvup) {
  if (kvup) {
...
bool ServiceApp::sendToManager(kvalobs::kvStationInfoList &retList,
...
bool ServiceApp::isMaxClientReached() {
...
void ServiceApp::addReaperObj(ReaperBase *rb) {
  boost::mutex::scoped_lock l(reaperMutex);
  //rb->_add_ref();
  rb->addRef();
...
void ServiceApp::removeReaperObj(ReaperBase *rb) {
  boost::mutex::scoped_lock l(reaperMutex);
  ...
  LOGDEBUG(
      "removeReaperObj: No ReaperObj foun in the reaperList! (This must be an error???)");
}

void ServiceApp::cleanUpReaperObj() {
  ...
  while (it != reaperObjList.end()) {
    ...
    ost << "active: " << ((*it)->isActive() ? "true" : "false") << " running: "
        << (running ? "true" : "false") << " lastAccess: "
        << miutil::miTime(lastAccess) << " timedif: " << now - lastAccess;
...
void ServiceApp::loadAllPersistentSubscribers() {
  ...
  if (!gate.select(psList)) {
    LOGERROR(
        "DBERROR: Cant read in the PsSubscribers." << endl << gate.getErrorStr());
    return;
  }
    ...
    if (it->subscribertype() == 0) {  //Data subscriber.
      ...
      try {
        ...
        if (!subscribers.createPsDataSubscriber(sub)) {
          LOGERROR(
              "ERROR: Cant start an ps subscriber thread for <"<<it->name() << "> (" << it->subscriberid() << ").");
          delete sub;
        } else {
          LOGINFO(
              "Start an ps subscriber thread for <"<<it->name() << "> (" << it->subscriberid() << ").");
        }
      } catch (...) {
        LOGERROR(
            "EXCEPTION: Cant start an ps subscriber thread for <"<<it->name() << "> (" << it->subscriberid() << ").");
        delete sub;
      }
      ...
      try {
        ...
        sub = new KvPsDataNotifySubscriber(subRef, *this, true,
                                           it->subscriberid());
      ...
      LOGWARN(
          "Unknown sybscribertype <" << it->subscribertype() << " name <" << it->name() << ">.");
    }
  }
...
void ServiceApp::removeAllUndefinedSubscribers() {
}

bool ServiceApp::savePsSubscriberSIOR(dnmi::db::Connection *dbCon,
  ...
  if (!gate.select(psSubscribers, ost.str())) {
    LOGERROR(
        "Failed to retrive the persistent subscriber <" << name << " from the database!" << gate.getErrorStr());
  ...
  if (psSubscribers.empty()) {
    LOGWARN(
        "The persistent subscriber <" << name << "> dont exist in the database!");
    return false;
  }
  ...
  if (!gate.update(*psSubscribers.begin())) {
    LOGWARN(
        "Failed to update the SIOR to the persistent subscriber <" << name << " in the database!" << gate.getErrorStr());
    return false;
  } else {
    LOGINFO(
        "The SIOR for the persistent subscriber <" << name << "> is updated in the database!");
  }
