...
public:
  	KvSubscriberBase(bool persistent=false, const std::string &subid="")
  		:subscriberid_(subid), persistent_(persistent), 
  		 t1NoConnection(0),t2NoConnection(0), subscribed_(true){
		}
  	KvSubscriberBase(const KvSubscriberBase &p)
    	:subscriberid_(p.subscriberid_), persistent_(p.persistent_), 
     	 t1NoConnection(p.t1NoConnection), t2NoConnection(p.t2NoConnection){
    	}
   ...
   ///Is this a persitent subscriber?
  	 ...
  	 * otherwise.
  	 */
  	bool connected()const{ return t1NoConnection==0; }
  	...
  	void subscribed(bool f){ subscribed_=f;}
