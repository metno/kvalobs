...
public:
	kvPsSubscriber(const std::string &name=""):name_(name) {}
  	kvPsSubscriber(const dnmi::db::DRow &r){ set(r);}
  	kvPsSubscriber(int            stationID,
   					int            typeID,   
  	...
  	char* tableName() const {return const_cast<char*>(std::string(std::string("ps_subscriber_")+name_).c_str());}
   ...
   miutil::miTime tbtime()const { return tbtime_; }
	miutil::miTime touched()const { return touched_; }
};
