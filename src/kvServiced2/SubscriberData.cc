...
std::string
SubscriberData::
createSubscriberid(TSubType st)
{
  	int                COUNT_MAX=1000;
  	int                count=0;
  	...
  	switch(st){
  	case DataSub:       servicename="data";       break;
  	case DataNotifySub: servicename="datanotify"; break;
  	case HintSub:       servicename="kvHint";     break;
  	}
...
bool
SubscriberData::
writeSubscriberFile(const std::string &subscriberid, 
		    ...
		    const std::string &corbaref)
{
  ...
  if(!si->qcAll()){
    for(KvDataSubscriberInfo::CITQc it=si->qcbegin();
	it!=si->qcend();
	it++){
      switch(*it){
...
bool 
SubscriberData::
readSubscriberFile(const std::string &fname, ServiceApp &app)
{
	...
	LOGINFO("Reading subscriber file: '"<<fname<<"'.");
    	...
    	if(key.find("Last call")!=string::npos){
      	...
      	for(int i=0; i<cval.size(); i++){
				try{
	  				...
	  				LOGERROR("EXCEPTION: readSubscriberFile: while setting QcId"
		   					<< " index=" << i << " cval.size="<< cval.size());
				}
      	}
      	...
      	for(int i=0; i<cval.size(); i++){
      		if(cval[i]=="All"){
				...
				try{
	  				...
	  				LOGERROR("EXCEPTION: readSubscriberFile: while setting stationid "
		   					<< "index=" << i << " cval.size="<< cval.size());
				}
      	}
    	}else{
  	...
  	switch(subType){
  		case DataSub:{
  			...
  			try{
  				sub=new KvDataSubscriber(statusid, qc, stations,
  												  app, ptr, false, subid);
  			}
  			catch(...){
  				LOGERROR("NO MEM!")
  		...
  		case DataNotifySub:{
  			...
  			try{
  				sub=new KvDataNotifySubscriber(statusid, qc, stations,
  												  		 app, ptr, false, subid);
  			}
  			catch(...){
  				LOGERROR("NO MEM!")
...
bool 
SubscriberData::
updateSubscriberFile( const std::string &subscriberid,
...
bool
SubscriberData::
removeSubscriberFile(const std::string &subscriberid)
{
  	string path(subPath+subscriberid+".sub");
...
SubscriberData::
SubscriberData(const std::string &fname)
{
   subPath=fname;
...
std::string 
SubscriberData::
createDataSubscriber(KvDataSubscriber *subscriber)
{
...
bool
SubscriberData::
createPsDataSubscriber(KvPsDataSubscriber *subscriber)
{
	CorbaHelper::CorbaApp *capp=CorbaHelper::CorbaApp::getCorbaApp();
...
bool
SubscriberData::
createPsDataNotifySubscriber(KvPsDataNotifySubscriber *subscriber)
{
	CorbaHelper::CorbaApp *capp=CorbaHelper::CorbaApp::getCorbaApp();
...
bool
SubscriberData::
registerPsDataSubscriber(CKvalObs::CService::kvDataSubscriberExt_ptr s,
                         const std::string &subid)
{
	mutex::scoped_lock lock(mutex);
  
	for(ITPsDataSub it=psDataSubs.begin();
	     it!=psDataSubs.end();
	     it++){
	   if(it->first==subid){
...
std::string 
SubscriberData::
createDataNotifySubscriber(KvDataNotifySubscriber *subscriber)
{
  	CorbaHelper::CorbaApp *capp=CorbaHelper::CorbaApp::getCorbaApp();
...
std::string 
SubscriberData::
createHintSubscriber(CKvalObs::CService::kvHintSubscriber_ptr s,
							const std::string &subid)
{
  	CorbaHelper::CorbaApp *capp=CorbaHelper::CorbaApp::getCorbaApp();
...
bool 
SubscriberData::
unregisterPsSubscriber(const std::string &id)
{
...
hasSubscribers()
{
  ...
  if(dataSubs.empty() && dataNotifySubs.empty() && 
     psDataSubs.empty() && psDataNotifySubs.empty())
    return false;
...
void
SubscriberData::
readAllSubscribersFromFile(ServiceApp &app)
{
	dnmi::file::Dir dir;
   string          file;
...
sendKvHintUp()
{
  ...
  while(it!=hintSubs.end()){
    try{
      ...
      LOGINFO("HINT SUBSCRIBER NOT LISTNING: Deletes the subscriber <" 
	      << it->first << ">!");
      ITHintSub tit=it;
      it++;
      hintSubs.erase(tit);
...
void 
SubscriberData::
forAllSubscribers(DataToSubscribersPtr data2sub)
{
  	mutex::scoped_lock lock(mutex);
  	...
  	for(ITDataSub it=dataSubs.begin(); it!=dataSubs.end(); it++){
    	...
    	try{
      	...
      	LOGDEBUG("CALL: subscriber <" << it->first << "> stationid=" << 
	       			data2sub->stationid << " typeid: " << data2sub->typeid_ <<
  	...
  	for(ITPsDataSub it=psDataSubs.begin(); it!=psDataSubs.end(); it++){
    	...
    	try{
      	...
      	LOGDEBUG("CALL: ps subscriber <" << it->first << "> stationid=" << 
	       			data2sub->stationid << " typeid: " << data2sub->typeid_ <<
	       			" obstime: " << data2sub->obstime);
  	...
  	for(ITDataNotifySub it=dataNotifySubs.begin(); 
       it!=dataNotifySubs.end(); 
       it++){
  	...
  	for(ITPsDataNotifySub it=psDataNotifySubs.begin(); 
       ...
       it++){
    	...
    	try{
      	...
      	LOGDEBUG("CALL: ps notify subscriber <" << it->first << "> stationid=" << 
	      			data2sub->stationid << " typeid: " << data2sub->typeid_ <<
	       			" obstime: " << data2sub->obstime);
...
removeTerminatedSubscribers()
{
  	...
  	for(ITDataSub it=dataSubs.begin(); it!=dataSubs.end(); ){
    	if(it->second.join(ret)){
      	...
      	if(ret>0){
				if(ret==1){
	  				LOGINFO("UNSUBSCRIBE: DataSubscriber <" << tit->first 
		  	             << "> removed!");
				}else{
	  				LOGERROR("NOT RESPONDING: DataSubscriber <" << tit->first 
		                  << "> removed!");
				}
  	...
  	for(ITDataNotifySub it=dataNotifySubs.begin(); it!=dataNotifySubs.end(); ){
    	if(it->second.join(ret)){
      	...
      	if(ret>0){
				if(ret==1){
	  				LOGINFO("UNSUBSCRIBE: DataNotifySubscriber <" << tit->first 
				  			 << "> removed!");
				}else{
	  				LOGERROR("NOT RESPONDING: DataNotifySubscriber <" << tit->first 
		   					<< "> removed!");
				}
