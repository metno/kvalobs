/*
  ...
  $Id: DataCommandExt.cc,v 1.1.2.2 2007/09/27 09:02:21 paule Exp $                                                       
...
execute(KvDataSubscriberInfo &sinf)
{
  	...
  	if(!sinf.thisStation(data()->stationid) ||
     	!sinf.checkStatusAndQc(data())){
     	LOGDEBUG("DataCommand::excute: NOT interested in this dataset!" << endl
     				<< sinf);
  	...
  	try{
   	...
   	if(CORBA::is_nil(subscriber))
    	...
    	if(!subscriber->callback(d->data, subscriberid().c_str())){
    		ret=2;
    		LOGWARN("FAILED: Subscriber dont accept more data.");
    	}else{
    		LOGINFO("SUCCESS: CALL Stationid: " << d->stationid << " typeid: " << 
	    	 		  d->typeid_ << " obstime: " << d->obstime);
    	}
   }
