...
void DataNotifyCommandExt::buildWhatList(
    kvskel::kvDataNotifySubscriberExt::WhatList &wl, DataToSubscribersPtr dp) {
...
int DataNotifyCommandExt::execute(KvDataSubscriberInfo &sinf) {
  ...
  if (!sinf.thisStation(dp->stationid) || !sinf.checkStatusAndQc(dp)) {
    LOGDEBUG(
        "DataNotifyCommandExt::excute: NOT interested in this dataset!" << endl << sinf);
  ...
  try {
    ...
    if (!subscriber->callback(wl, subscriberid().c_str())) {
      ...
      LOGINFO(
          "SUCCESS DataNotify: CALL Stationid: " << dp->stationid << " typeid: " << dp->typeid_ << " obstime: " << dp->obstime);
    }
