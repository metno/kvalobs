...
setLogger(const std::string &name)
{
...
runPersistent()
{
	...
	std::string name=kvalobs::kvPsSubscribers::nameFromSubscriberid(subscriberid());;

	setLogger(name);
	
	time(&retry);
	
	LOGINFO("Persistent thread for subscriber: " << subscriberid() << " started!" );
...
void 
SubscriberThread<cmdType>::
tryToSendSavedData(bool &moredata)
{
	using namespace std;
...
void 
SubscriberThread<cmdType>::
saveDataInfo(cmdType *cmd, bool &moredata)
{
	using namespace std;
	using namespace kvalobs;
