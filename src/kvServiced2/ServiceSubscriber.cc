...
void ServiceSubscriber::updateWorkelementServiceStart(
    const kvalobs::kvStationInfo &st, dnmi::db::Connection *con) {
  kvDbGate gate(con);
  ostringstream ost;

  ost << "UPDATE workque SET service_start='" << miTime::nowTime()
      << "' WHERE stationid=" << st.stationID() << "  AND obstime='"
      << st.obstime().isoTime() << "' AND typeid=" << st.typeID();
...
void ServiceSubscriber::callSubscribers(const kvalobs::kvStationInfo &si) {
  long stationID;
  ...
  if (!gate.select(
      textDataList,
      kvQueries::selectTextDataFromType(si.stationID(), si.typeID(),
                                        si.obstime()))) {
    textDataList.clear();
  }
  ...
  try {
    data2sub.reset(new DataToSubscribers(dataList, textDataList, si));
  } catch (...) {
  ...
  LOGINFO(
      "CALL Subscribers: stationID: "<< data2sub->stationid << " obstime: " << data2sub->obstime << " typeID: " << data2sub->typeid_);
...
void ServiceSubscriber::operator()() {
  ...
  while (!app.shutdown()) {
    ...
    app.sendToManager(stationInfoList, stInfoCmd->getCallback());
