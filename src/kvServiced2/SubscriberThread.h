...
class SubscriberThread : public KvDataSubscriberInfo,
 ...
 public:
  SubscriberThread(CKvalObs::CService::StatusId status,
                   const KvDataSubscriberInfo::TQc &qc,
                   const KvDataSubscriberInfo::TStations &stations,
                   ServiceApp &app, typename cmdType::_ptr_type subscriber,
                   bool persistent = false,
                   const std::string &subscriberid = "")
      : KvDataSubscriberInfo(status, qc, stations, app, subscriberid,
                             persistent),
        subscriber_(cmdType::_interface::_duplicate(subscriber)) {
  }

  SubscriberThread(const CKvalObs::CService::DataSubscribeInfo &info,
                   typename cmdType::_ptr_type subscriber, ServiceApp &app,
                   bool persistent = false,
                   const std::string &subscriberid = "")
  ...
  SubscriberThread(typename cmdType::_ptr_type subscriber, ServiceApp &app,
                   bool persistent = false,
                   const std::string &subscriberid = "")
  ...
  void subscriber(typename cmdType::_ptr_type subscriber) {
  ...
  bool sendPsData(cmdType &cmd, bool &moredata, dnmi::db::Connection *con);
