...
class SubscriberCommandBase : public dnmi::thread::CommandBase {
 ...
 public:
  ...
  DataToSubscribersPtr data() const {
    return data_;
  }
  ...
  void subscriberid(const std::string &subid) {
    subscriberid_ = subid;
  }

  ///Reset the data field with new data. 
  void data(DataToSubscribersPtr data) {
    data_ = data;
  }
