/*
  ...
  $Id: kvServiced.cc,v 1.1.2.2 2007/09/27 09:02:22 paule Exp $                                                       
...
main(int argc, char** argv)
{
  ...
  if(dnmi::file::isRunningPidFile(pidfile, error)){
    if(error){
      LOGFATAL("An error occured while reading the pidfile:" << endl
	       ...
	       << endl << "kvServiced is not running. " << 
	       "If it is running and there is problems. Kill kvServiced and"
	       << endl << "restart it." << endl << endl);
      return 1;
    }else{
      LOGFATAL("Is kvServiced allready running?" << endl
  ...
  try {                                    
    ...
    KvServiceExtImpl *serviceImpl = new KvServiceExtImpl(app);
  
    PortableServer::ObjectId_var mgrImplIid=poa->activate_object(dataReadyImpl);
    PortableServer::ObjectId_var serviceImplIid=poa->activate_object(serviceImpl);
    {
	...
	return 1;
      ...
      cout << "IDL object kvServiceDataReady IOR = '" << sior 
	   << "'" << endl;
    ...
    app.notifyAllKvHintSubscribers(true);
    app.loadAllPersistentSubscribers();
  ...
  app.notifyAllKvHintSubscribers(false);
  serviceSubscriberThread.join();
  objReaperThread.join();
  app.subscribers.joinAllSubscribers();
