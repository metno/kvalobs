...
{
  	try{
  		LOGDEBUG("KvDataSubscriberInfo::put: new data for <" << subscriberid() << ">");
...
operator<<(std::ostream& os, const KvDataSubscriberInfo &c)
{
  	...
  	if(!c.qcAll()){
    	...
    	for(vector<QcId>::size_type i=0; i<c.qc().size(); i++){
      	switch(c.qc_[i]){
      	case CKvalObs::CService::QC1:  os << sep << "QC1";  break;
      	case CKvalObs::CService::QC2d: os << sep << "QC2d"; break;
      	case CKvalObs::CService::QC2m: os << sep << "QC2m"; break;
      	case CKvalObs::CService::HQC:  os << sep << "HQC";  break;
      	}
      	sep=',';
    	}
  	}else
