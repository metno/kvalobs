...
KVSYNOPD_PIDFILE=$KVALOBS/var/run/kvsynopd.pid
KVSERVICED_PIDFILE=$KVALOBS/var/run/kvServiced.pid
SUBIDFILE=$KVALOBS/var/kvsynop/datasubscriber.id
TIMEOUT=120
...
SUBID=$(cat $KVALOBS/var/kvsynop/datasubscriber.id)
...
if [ ! -f $KVALOBS/var/kvalobs/service/subscribers/$SUBID.sub ]; then
    date
    ...
    $KVALOBS/bin/kvsynopd -1 > /dev/null  2>&1 &
else
    echo "$(date) Ok!"
fi
