/*
  ...
  $Id: kvServiceImpl.cc,v 1.1.2.4 2007/09/27 09:02:40 paule Exp $                                                       
		    ...
		    kvDataNotifySubscriber_ptr sub)
{
  ...
  try{
    ...
    dnsub=new KvDataNotifySubscriber(info, ptr, app);
  }
  catch(...){
    CORBA::release(ptr);
	      ...
	      kvDataSubscriber_ptr     sub)
{
  ...
  try{
    ...
    dsub=new KvDataSubscriber(info, ptr, app);
  }
  catch(...){
    CORBA::release(ptr);
    return CORBA::string_dup("");
...
subscribeKvHint(CKvalObs::CService::kvHintSubscriber_ptr sub)
{
    ...
    subid=app.subscribers.createHintSubscriber(ptr);
	...
	DataIterator_out     it)
{
  ...
  //CODE:
  //We must add code for garbage collection of DataIterators that don't behave.
	     ...
	     CKvalObs::CService::ModelDataIterator_out it)
{
  ...
  //CODE:
  //We must add code for garbage collection of ModelDataIterators that don't behave.
