/*
  ...
  $Id: kvQABaseDBConnection.h,v 1.21.2.2 2007/09/27 09:02:37 paule Exp $                                                       
...
public:
  ...
  struct obs_data{
    miutil::miTime time;
    std::vector<kvalobs::kvData> data;
  };
  /// all text_data parameters from one timestep
  struct text_data{
    miutil::miTime time;
    std::vector<kvalobs::kvTextData> data;
  };
  /// all model_data parameters from one timestep
  struct model_data{
    miutil::miTime time;
    std::vector<kvalobs::kvModelData> data;
  };
...
public:
  ...
  bool dbOk() const {return connection_ok;}
