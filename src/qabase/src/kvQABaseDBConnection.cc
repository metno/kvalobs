/*
  ...
  $Id: kvQABaseDBConnection.cc,v 1.44.2.2 2007/09/27 09:02:38 paule Exp $                                                       
...
{
  if (algo_timestamp==0) updateAlgoList();
...
bool kvQABaseDBConnection::getParameter(const std::string& name,
					kvalobs::kvParam& p)
{
  if (parameter_timestamp == 0) updateParameterList();
					  ...
					  kvalobs::kvData& param)
{
  ...
  if (!result || dlist.size() == 0) return false;
