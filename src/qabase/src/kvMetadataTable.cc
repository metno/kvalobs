/*
  ...
  $Id: kvMetadataTable.cc,v 1.13.6.2 2007/09/27 09:02:38 paule Exp $                                                       
...
#include "../include/kvMetadataTable.h"
				...
				std::string& data)
{
  ...
  for (int i=0; vp!=vas.end(); i++,vp++){
    ...
    if (i == 0){
      lrows.clear();
      for (int j=0; j<value_[vp->name].size(); j++){
	lrows.push_back(j);
      }
    }
				    ...
				    std::list<kvMetadataTable>& lmt)
{
  ...
  for (int i=0; i<vs.size(); i++){
	...
	int icomment;// remove comments from metadata! (Why...?!?)
