/*
  ...
  $Id: QaWorkCommand.cc,v 1.5.6.2 2007/09/27 09:02:38 paule Exp $                                                       
...
QaWorkCommand::QaWorkCommand(const CKvalObs::StationInfo &st)
  :kvalobs::StationInfoCommand(st), callback(CKvalObs::CManager::CheckedInput::_nil())
{
...
{
  for(CORBA::Long i=0; i<args.length(); i++){
...
bool 
QaWorkCommand::getKey(const std::string &key, std::string &val)const
{
...
QaWorkCommand::getCallback()const
{
