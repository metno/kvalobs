...
DataSrcApp::DataSrcApp(int argn, char **argv, int nConnections_,
                       ...
                       const char *opt[][2])
    : KvApp(argn, argv, opt),
      refMgr(CKvalObs::CManager::ManagerInput::_nil()),
      ok(false),
      shutdown_(false) {
 ...
 */
CKvalObs::CManager::ManagerInput_ptr DataSrcApp::lookUpManager(bool forceNS,
                                                               bool &usedNS) {
    ...
    if (forceNS) {
      ...
      if (CORBA::is_nil(obj))
        throw LookUpException(
            "EXCEPTION: Can't obtain a reference for 'kvManagerInput'\n           from the CORBA nameserver!");

      refMgr = CKvalObs::CManager::ManagerInput::_narrow(obj);

      if (CORBA::is_nil(refMgr))
        throw LookUpException(
            "EXCEPTION: Can't narrow reference for 'kvManagerInput'!");
...
bool DataSrcApp::sendInfoToManager(const kvalobs::kvStationInfoList &info_) {
  bool forceNS = false;
  bool usedNS = false;
  ...
  try {
    for (int i = 0; i < 2; i++) {
      ...
      try {
        ...
        LOGERROR(
            "(kvmanager) Exception CORBA::COMM_FAILURE!\n" << "Is kvManager running\n");
      } catch (...) {
...
int DataSrcApp::registerDb(int nConn) {
  ...
  LOGINFO(
      "registerDb: loading driver <" << dbMgr.fixDriverName( driver ) << ">!\n");
  ...
  for (int i = 0; i < nConn; i++) {
    ...
    while (!con && !inShutdown()) {
      ...
      if (!con) {
        LOGFATAL(
            "registerDb: Can't create connection to <" << drvId << ">\n ERROR message: " << dbMgr.getErr());
        sleep(1);
      } else {
...
bool DataSrcApp::registerParams() {
  ...
  if (!readParamsFromFile(paramFile, tmpParams)) {
    ...
    if (!readParamsFromFile(paramFile, tmpParams)) {
      IDLOGERROR(
          "param_update",
          "Cant read parameter information from file <" << paramFile << ">.");
      LOGERROR(
          "Cant read parameter information from file <" << paramFile << ">.");
    }
  }
  ...
  if (!tmpParams.empty()) {
    IDLOGINFO(
        "param_update",
        "Loaded #" << tmpParams.size() << " 'stinfosys' param definitions from file <" << paramFile << ">.");
  }
    ...
    if (res && res->size() > 0) {
      ...
      while (res->hasNext()) {
        ...
        try {
          ...
          LOGERROR(
              "registerParams: BADNUM: paramid is not a number\n" << "   paramid(" << row[0] << ")\n");
        }
      }
...
DecodeCommand*
DataSrcApp::create(const char *obsType_, const char *obs, long timoutIn_msec,
                   ErrCode &errCode, std::string &err) {
  DecoderBase *dec;
  ...
  //lookup the decoder based on obsType.
  {
    ...
    dec = decoderMgr.findDecoder(*con, paramList, typeList, obsType_, obs,
                                 myErr);
  }
...
DecodeCommand*
DataSrcApp::create(CORBA::Long report_id, const char* report_type,
                   const char* obstime, const char* data, long timoutIn_msec,
                   ErrCode &errCode, std::string &err) {
}
...
bool DataSrcApp::put(dnmi::thread::CommandBase *command) {
  try {
