...
bool DecodeCommand::executeImpl() {
  LOGDEBUG("DecodeCommand::execute: called!\n");

  try {
    kvdatainput::decodecommand::ptrRedirect.reset(
        new kvalobs::decoder::RedirectInfo());
    ...
    LOGDEBUG(
        "DecodeCommand::execute: Redirect: " << (redirect_?"redirected":"NOT redirected")<< " Result: " << result <<"\n");
  } catch (const std::exception &ex) {
    LOGERROR(
        "UNEXPECTED EXCEPTION from <"<< decoder->name() << ">\n Reason: " << ex.what());
...
void DecodeCommand::debugInfo(std::iostream &info) {
