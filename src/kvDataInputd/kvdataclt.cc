...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (verbose) {
      cerr << "Sending observation to kvDataInput!" << endl << "decoder: "
    ...
    cerr
        << "Caught system exception COMM_FAILURE -- unable to contact <kvDataInputd> at '"
        << kvserver << "'." << endl;
  } catch (CORBA::TRANSIENT & ex) {
    cerr
        << "Caught system exception TRANSIENT -- unable to contact <kvDataInputd> at '"
        << kvserver << "'." << endl;
