...
bool App::subscribeSetup() {
  notifyid = subscribeDataNotify(kvservice::KvDataSubscribeInfoHelper(),
                                 eventQue);
...
boost::posix_time::ptime App::lastFromTime(const string& filename) {
...
void App::printObsDataList(KvObsDataList& dataList) {
  ...
  string textfilename = fileTextDataList + "."
      + to_kvalobs_string(toTime.date()) + "_"
      + to_kvalobs_string(toTime.time_of_day());
  ...
  for (IKvObsDataList dit = dataList.begin(); dit != dataList.end(); dit++) {
    ...
    if (dit->dataList().size() > 0) {
      ...
      for (; it != dit->dataList().end(); it++) {
        char c = it->sensor();
        fout << it->stationID() << "|" << to_kvalobs_string(it->obstime())
             << "|" << it->original() << "|" << it->paramID() << "|"
             << to_kvalobs_string(it->tbtime()) << "|" << it->typeID() << "|"
             << c << "|" << it->level() << "|" << it->corrected() << "|"
             << it->controlinfo().flagstring() << "|"
             << it->useinfo().flagstring() << "|" << it->cfailed() << endl;
      }
    }
    ...
    if (dit->textDataList().size() > 0) {
      ...
      for (; i != dit->textDataList().end(); i++) {
        tfout << i->stationID() << "|" << to_kvalobs_string(i->obstime()) << "|"
              << i->original() << "|" << i->paramID() << "|"
              << to_kvalobs_string(i->tbtime()) << "|" << i->typeID() << endl;
      }
    }
  }
...
void App::storeToFile(const std::string& filename,
