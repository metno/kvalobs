...
DataReceiver::DataReceiver(App &app_, dnmi::thread::CommandQue &que_)
    : app(app_),
      que(que_) {
}
...
void DataReceiver::onKvDataEvent(kvservice::KvObsDataListPtr data) {
...
void DataReceiver::onKvHintEvent(bool up) {
  if (up) {
...
void DataReceiver::onKvDataNotifyEvent(kvservice::KvWhatListPtr what) {
  cerr << "KvDataNotifyEvent received!" << endl;
}

int DataReceiver::run() {
