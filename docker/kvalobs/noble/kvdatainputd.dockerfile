...
# Add bufrdecoder 
RUN apt update && apt install -y libgeo-bufr-perl  metno-bufrtables gdb tmux
...
USER ${kvuser}:${kvuser}
HEALTHCHECK --interval=300s --timeout=30s --start-period=600s --retries=10 CMD [ "/usr/local/bin/healthcheck.sh" ]
