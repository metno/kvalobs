...
if [ "$#" -eq 0 ]; then
  echo "ENTRYPOINT kvQabased"
  
  echo "Starting kvQabased. Process count $QA_PROCESS_COUNT"
  /usr/bin/kvQabased --process-count $QA_PROCESS_COUNT 2>&1 &
  kvQabased_PID=$!
  echo "kvQabased pid: $kvManagerd_PID"
  wait -n
  ec=$?

  if [ $got_exit_signal = true ]; then
    echo "Got exit signal: $got_exit_signal"
    kill_pid $kvQabased_PID
    ec=$?
    echo "Killed kvManagerd on signal. Exit code: $ec"
  else
    kill -0 $kvQabased_PID &>/dev/null || echo "kvQabased: died  ec: $ec"
  fi
