...
AC_DEFUN([METNO_REQUIRE_QT4], [
    ...
    # locate uic3, which allows compiling Qt3 .ui files for Qt4; give the user a chance to specify where it is
    AC_ARG_WITH([uic3],
	AS_HELP_STRING([--with-uic3], [Qt user interface compiler, for compiling old (version 3) .ui files]), [
	UIC3=$withval
    ], [
	AC_PATH_PROGS([UIC3], [uic3], [], [${QT4_PATH}])
    ])
    AS_IF([test x"${UIC3}" = x], [
	AC_MSG_ERROR([unable to locate uic3])
    ])
